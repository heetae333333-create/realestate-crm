...
(()=>{
  ...
  function installDraftAndSummary(type,id){
    ...
    if(saved){try{const parsed=JSON.parse(saved);if(confirm('작성 중이던 임시저장 내용이 있습니다. 이어서 작성할까요?'))restoreForm(form,parsed.data||parsed);}catch(e){}}
...
(()=>{
  ...
  function installDraft391(type,id){
    ...
    const saved=localStorage.getItem(key);
    if(saved && !form.dataset.crm391DraftAsked){
      form.dataset.crm391DraftAsked='1';
      let parsed=null; try{parsed=JSON.parse(saved)}catch(_){localStorage.removeItem(key)}
      if(parsed){
        const yes=oldConfirm('작성 중이던 임시저장 내용이 있습니다. 이어서 작성할까요?');
        if(yes && typeof restoreForm==='function') restoreForm(form,parsed.data||parsed);
  ...
  // 기존 v3.9.0 임시저장 질문이 먼저 뜬 경우, 취소 시 남은 키를 지우도록 confirm을 제한적으로 보정
  window.confirm=function(message){
    const result=oldConfirm(message);
    if(!result&&String(message).includes('작성 중이던 임시저장 내용')){
      const form=q('#modalForm'); const type=q('#modalTitle')?.textContent?.includes('고객')?'customer':'listing';
      const id=(type==='customer'?state.customers:state.listings)?.find(x=>String(x.name||x.title)===String(q('#modalBody input[name="name"],#modalBody input[name="title"]')?.value))?.id;
      localStorage.removeItem(`crm390:draft:${state.profile?.id||'anon'}:${type}:${id||'new'}`);
    }
    return result;
  };

  setTimeout(()=>window.crm390InjectAdminTools?.(),1200);
  console.info('CRM v3.9.1 10종 기능 재점검 및 임시저장/추천버튼 안정화 완료');
})();
