...
function openCustomerModal(id){
  ...
  $('#modalSubmit').onclick=async(e)=>{e.preventDefault();const fd=new FormData($('#modalForm'));const payload=Object.fromEntries(fd.entries());payload.owner_id=state.profile.id;payload.budget_max=payload.budget_max?Number(payload.budget_max):null;payload.next_follow_up_at=payload.next_follow_up_at||null;delete payload.next_contact_at;const q=id?state.client.from('customers').update(payload).eq('id',id):state.client.from('customers').insert(payload);const {error}=await q;if(error)return toast(error.message);$('#modal').close();toast('저장했습니다.');renderCustomers()};$('#modal').showModal();
}
...
function renderListingTable(rows,target,mine){
  const el=$('#'+target);el.innerHTML=rows.length?`<div class="table-wrap"><table><thead><tr><th>상태</th><th>거래</th><th>유형</th><th>매물명</th><th>지역·주소</th><th>금액</th><th>면적</th><th>입주</th><th>담당 중개사</th><th>최종 FU</th><th>예정 FU</th>${mine?'<th>관리</th>':''}</tr></thead><tbody>${rows.map(x=>`<tr><td>${badge(x.status==='available'?'거래 가능':x.status==='complete'?'거래 완료':'협의 중',x.status==='available'?'green':x.status==='complete'?'gray':'yellow')}</td><td>${escapeHtml(x.transaction_type)}</td><td>${escapeHtml(x.property_type)}</td><td><strong>${escapeHtml(x.title)}</strong>${x.is_public?'':' '+badge('비공개','red')}</td><td>${escapeHtml(x.district||'')} ${escapeHtml(x.address||'')}</td><td>${fmtMoney(x.price)}</td><td>${x.area_m2?x.area_m2+'㎡':'-'}</td><td>${moveInText(x)}</td><td>${escapeHtml(x.owner?.office_name||'')}<br><span class="muted">${escapeHtml(x.owner?.full_name||'')}</span></td><td>${fmtDate(x.last_follow_up_at||x.last_confirmed_at)}</td><td>${dueBadge(x.next_follow_up_at)}</td>${mine?`<td><div class="row-actions"><button class="success" onclick="openFollowUpModal('listing','${x.id}')">FU</button><button class="ghost" onclick="openHistoryModal('listing','${x.id}')">히스토리</button><button class="ghost" onclick="openListingModal('${x.id}')">수정</button><button class="danger" onclick="deleteListing('${x.id}')">삭제</button></div></td>`:''}</tr>`).join('')}</tbody></table></div>`:'<div class="empty">조건에 맞는 매물이 없습니다.</div>';
}
...
async function openFollowUpModal(entityType,id){
  ...
  $('#modalBody').innerHTML=`<div class="form-grid"><label>FU 일자<input name="follow_up_date" type="date" value="${today()}" required></label><label>방식<select name="contact_method"><option>전화</option><option>문자</option><option>카카오톡</option><option>방문</option><option>이메일</option><option>기타</option></select></label><label class="span-2">통화·상담 내용<textarea name="content" rows="6" placeholder="오늘 확인한 내용, 고객 반응, 다음 조치 등을 기록하세요." required></textarea></label><label>예정 FU<input name="next_follow_up_at" type="date" value="${item.next_follow_up_at?item.next_follow_up_at.slice(0,10):''}"></label></div>`;
...
async function openHistoryModal(entityType,id){
  ...
  $('#modalBody').innerHTML=(data||[]).length?`<div class="history-list">${data.map(h=>`<article class="history-item"><div class="history-head"><strong>${fmtDate(h.follow_up_date)} · ${escapeHtml(h.contact_method)}</strong><span class="muted">${escapeHtml(h.writer?.full_name||'')}</span></div><p>${escapeHtml(h.content).replace(/\n/g,'<br>')}</p>${h.next_follow_up_at?`<div class="muted">다음 FU: ${fmtDate(h.next_follow_up_at)}</div>`:''}</article>`).join('')}</div>`:'<div class="empty">아직 기록된 FU 히스토리가 없습니다.</div>';
...
window.renderView=renderView;window.openFollowUpModal=openFollowUpModal;window.openHistoryModal=openHistoryModal;window.filterCustomers=filterCustomers;window.openCustomerModal=openCustomerModal;window.deleteCustomer=deleteCustomer;window.openListingModal=openListingModal;window.deleteListing=deleteListing;window.filterNetwork=filterNetwork;window.setMemberStatus=setMemberStatus;window.previewTransfer=previewTransfer;window.executeTransfer=executeTransfer;
boot();
