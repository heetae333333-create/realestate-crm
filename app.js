...
async function printSelectedListingBrochure(customerId){
  ...
  const cards=[];for(const x of rows){const {data:photos}=await state.client.from('listing_photos').select('*').eq('listing_id',x.id).eq('is_customer_visible',true).order('sort_order');let image='';const cover=(photos||[]).find(p=>p.id===x.cover_photo_id)||(photos||[])[0];if(cover)image=await signedPhotoUrl(cover.storage_path)||'';cards.push(`<section class="property">${image?`<img src="${image}">`:''}<h2>${escapeHtml(x.title)}</h2><p><b>${escapeHtml(x.transaction_type)}</b> ${listingPriceText(x)}</p><p>${escapeHtml(x.district||'')} ${escapeHtml(x.address||'')}</p><p>면적 ${x.area_m2||'-'}㎡ · 방 ${listingRoomText(x)} · 욕실 ${x.bathroom_count??'-'}</p><p>${escapeHtml(x.description||'')}</p></section>`)}
...
openListingModal=function(id){
  ...
  <label>매도/임대인 연락처<div class="inline-field"><input name="contact_phone" value="${escapeHtml(x.contact_phone||'')}" placeholder="010-0000-0000" required><button type="button" class="ghost" onclick="crm38AddContactRow()">+ 번호 추가</button></div></label>
  ...
  <label>관리비(만원)<input name="management_fee" type="number" step="0.1" value="${x.management_fee??''}"></label><label>전용면적(㎡)<input name="area_m2" type="number" step="0.01" value="${x.area_m2||''}"></label>
  ...
  <label>입주 가능일<input id="moveInDateInput" name="move_in_date" type="date" value="${x.move_in_date||''}"></label><label class="check-label"><input id="moveInNegotiable" name="move_in_negotiable" type="checkbox" ${x.move_in_negotiable?'checked':''}> 입주일 협의 가능</label>
  <label>예정 FU<input name="next_follow_up_at" type="date" value="${x.next_follow_up_at?x.next_follow_up_at.slice(0,10):''}"></label><label>공개 여부<select name="is_public"><option value="true">공개</option><option value="false">비공개</option></select></label>
  ...
  <label class="span-2">내부 사진 추가<input id="listingPhotoFiles" type="file" accept="image/*" multiple><span class="field-help">여러 장 선택 가능 · 등록 후 사진 화면에서 한눈에 확인할 수 있습니다.</span></label><label class="span-2">상세 설명<textarea name="description" rows="5">${escapeHtml(x.description||'')}</textarea></label></div>`;
