...
function crmR344ApprovalDateText(x){if(x?.usage_approval_date){const r=String(x.usage_approval_date).slice(0,10),p=r.split('-');return p.length===3?`${p[0]}.${p[1]}.${p[2]}`:r}return x?.built_year?`${x.built_year}.01.01 (일자 확인 필요)`:'-'}
...
function openCustomerModal(id){
  ...
  $('#modalSubmit').onclick=async(e)=>{e.preventDefault();const fd=new FormData($('#modalForm'));const payload=Object.fromEntries(fd.entries());payload.owner_id=state.profile.id;const buyerSide=['매수','임차'].includes(payload.customer_type);payload.deal_type=buyerSide?(payload.deal_type||null):null;payload.customer_grade=payload.customer_grade||'C';payload.budget_max=payload.budget_max?Number(payload.budget_max):null;payload.desired_monthly_rent=buyerSide&&(payload.deal_type||'').includes('월세')&&payload.desired_monthly_rent?Number(payload.desired_monthly_rent):null;payload.desired_one_point_five_room=buyerSide&&payload.desired_one_point_five_room==='on';payload.desired_rooms=buyerSide?(payload.desired_one_point_five_room?1:(payload.desired_rooms!==''?Number(payload.desired_rooms):null)):null;payload.loan_available=buyerSide?payload.loan_available==='true':null;payload.equity_unknown=buyerSide&&payload.equity_unknown==='on';payload.equity_capital=buyerSide&&!payload.equity_unknown&&payload.equity_capital?Number(payload.equity_capital):null;payload.official_price=null;payload.next_follow_up_at=payload.next_follow_up_at||null;
payload.deposit_adjustable=!!document.querySelector('#crmR346CustomerCheck')?.checked;
payload.alternate_deposit=payload.deposit_adjustable&&document.querySelector('#crmR346CustomerDeposit')?.value?Number(document.querySelector('#crmR346CustomerDeposit').value):null;
payload.alternate_monthly_rent=payload.deposit_adjustable&&document.querySelector('#crmR346CustomerRent')?.value?Number(document.querySelector('#crmR346CustomerRent').value):null;
delete payload.next_contact_at;const q=id?state.client.from('customers').update(payload).eq('id',id):state.client.from('customers').insert(payload);const {error}=await q;if(error)return toast(error.message);$('#modal').close();toast('저장했습니다.');renderCustomers()};$('#modal').showModal();
}
...
openListingModal=function(id){
  ...
  $('#modalSubmit').style.display='';$('#modalSubmit').onclick=async e=>{e.preventDefault();const fd=new FormData($('#modalForm'));const selected=[...document.querySelectorAll('.crm38-deal-card')].filter(c=>c.querySelector('.crm38-deal-check').checked).map((c,i)=>({deal_type:c.dataset.type,price:Number(c.querySelector('.crm38-deal-price').value||0)||null,monthly_rent:c.dataset.type==='월세'?(Number(c.querySelector('.crm38-deal-rent').value||0)||null):null,is_preferred:c.querySelector('input[type=radio]').checked,sort_order:i}));if(!selected.length)return toast('거래유형을 하나 이상 체크하세요.');const preferred=selected.find(o=>o.is_preferred)||selected[0];const p={title:fd.get('title'),contact_phone:fd.get('contact_phone')||null,transaction_type:preferred.deal_type,price:preferred.price,monthly_rent:preferred.monthly_rent,property_type:fd.get('property_type'),status:fd.get('status'),district:fd.get('district')||null,address:fd.get('address')||null,management_fee:Number(fd.get('management_fee')||0)||null,area_m2:Number(fd.get('area_m2')||0)||null,common_area_m2:Number(fd.get('common_area_m2')||0)||null,land_area_m2:Number(fd.get('land_area_m2')||0)||null,building_area_m2:Number(fd.get('building_area_m2')||0)||null,is_whole_building:fd.get('is_whole_building')==='on',room_count:Number(fd.get('room_count')||0)||null,bathroom_count:Number(fd.get('bathroom_count')||0)||null,options:fd.get('options')||null,loan_available:fd.get('loan_available')===''?null:fd.get('loan_available')==='true',official_price:Number(fd.get('official_price')||0)||null,move_in_immediate:fd.get('move_in_immediate')==='on',move_in_negotiable:fd.get('move_in_negotiable')==='on',move_in_date:fd.get('move_in_immediate')==='on'?null:(fd.get('move_in_date')||null),move_in_period:fd.get('move_in_immediate')==='on'?null:(fd.get('move_in_period')||null),is_public:fd.get('is_public')==='true',next_confirm_at:fd.get('next_confirm_at')||null,description:fd.get('description')||null,owner_id:id?(x.owner_id||state.profile.id):state.profile.id,feature_tags:[...document.querySelectorAll('.crm361-feature-check:checked')].map(el=>el.value),is_one_point_five_room:fd.get('is_one_point_five_room')==='on',current_floor:fd.get('current_floor')!==null&&fd.get('current_floor')!==''?Number(fd.get('current_floor')):null,total_floors:fd.get('total_floors')!==null&&fd.get('total_floors')!==''?Number(fd.get('total_floors')):null,built_year:fd.get('built_year')!==null&&fd.get('built_year')!==''?Number(fd.get('built_year')):null,
deposit_adjustable:!!document.querySelector('#crmR346ListingCheck')?.checked,
alternate_deposit:document.querySelector('#crmR346ListingCheck')?.checked&&document.querySelector('#crmR346ListingDeposit')?.value?Number(document.querySelector('#crmR346ListingDeposit').value):null,
alternate_monthly_rent:document.querySelector('#crmR346ListingCheck')?.checked&&document.querySelector('#crmR346ListingRent')?.value?Number(document.querySelector('#crmR346ListingRent').value):null};if(p.is_one_point_five_room)p.room_count=1;if(!p.loan_available)p.official_price=null;
...
crm361OpenListingFu=async function(id){
  ...
  <section class="crm361-fu-panel" data-panel="record">
    <div class="form-grid">
      ...
      <label>상담 종류<select id="crm361FuMethod"><option>전화</option><option>부재</option><option>대면투어</option><option>촬영</option><option>문자/톡 발송</option><option>문자/톡 수신</option><option>부재중</option><option>가계약</option><option>본계약</option><option>중도금</option><option>잔금</option><option>기타</option></select></label>
...
function crm3862EnhanceListingAddress(){
  ...
  unitInput.placeholder='예: 603, B01, B1';
  ...
  const help=row.querySelector('.crm3852-address-help');
  if(help)help.textContent='주소는 검색 결과에서 선택하고, 동·호수는 숫자 또는 영문을 직접 입력하세요. 반지하는 B01, B02처럼 입력할 수 있습니다. 숫자만 입력하면 저장 시 동·호가 자동으로 붙습니다.';
}
...
openCustomerModal=function(id=null){
  ...
  <div class="crm3857-customer-detail">
    ...
    <section class="crm3857-form-card crm3859-deal-card"><h3>희망 거래유형 및 금액</h3><p class="muted">여러 거래유형을 선택하면 유형별 희망금액을 각각 입력할 수 있습니다.</p><div class="crm3857-deal-checks">${CRM3857_DEAL_TYPES.map(type=>`<label class="crm3857-deal-check-card"><input type="checkbox" class="crm3857-customer-deal-check" value="${type}" ${opts.some(o=>o.deal_type===type)?'checked':''} onchange="crm3857ToggleDeal('${type}',this.checked)"> ${type}</label>`).join('')}</div><div class="crm3857-deal-panels">${CRM3857_DEAL_TYPES.map(type=>{const o=opts.find(v=>v.deal_type===type)||{};return `<div class="crm3857-customer-deal-panel" data-type="${type}" ${opts.some(v=>v.deal_type===type)?'':'hidden'}><strong>${type}</strong><label>${type==='월세'?'희망 보증금':'희망금액'}(만원)<input class="crm3857-budget" type="number" min="0" value="${o.budget_max??''}"></label>${type==='월세'?`<label>희망 월세(만원)<input class="crm3857-rent" type="number" min="0" value="${o.desired_monthly_rent??''}"></label>`:''}</div>`}).join('')}</div></section>
    <section class="crm3857-form-card crm398-property-types-card"><div class="crm398-type-head"><div><h3>희망 매물유형</h3><p class="muted">여러 유형을 동시에 선택할 수 있습니다. 선택하지 않으면 전체 유형을 추천합니다.</p></div><label class="crm398-home-all"><input type="checkbox" id="crm398HomeAll" onchange="crm398ToggleHomeAll(this.checked)"> 주택 전체</label></div><div class="crm398-property-type-grid">${crm398PropertyTypes().map(type=>`<label><input type="checkbox" class="crm398-property-type-check" value="${type}" ${crm398SelectedPropertyTypes(x).includes(type)?'checked':''} onchange="crm398SyncHomeAll()"><span>${type}</span></label>`).join('')}</div></section>
  ...
  $('#modalSubmit').onclick=async e=>{
    ...
    if(!deals.length)return toast('거래유형을 한 개 이상 선택하세요.');
    const joined=deals.map(o=>o.deal_type).join('+'),primary=deals[0];
    const payload={owner_id:id?(x.owner_id||state.profile.id):state.profile.id,name:fd.get('name').trim(),phone:contactList[0].phone,customer_type:fd.get('customer_type'),status:fd.get('status'),customer_grade:fd.get('customer_grade')||'C',deal_type:joined,preferred_area:fd.get('preferred_area')||null,desired_rooms:room15.checked?1:(fd.get('desired_rooms')?Number(fd.get('desired_rooms')):null),desired_one_point_five_room:room15.checked,budget_max:primary?.budget_max??null,desired_monthly_rent:deals.find(o=>o.deal_type==='월세')?.desired_monthly_rent??null,loan_available:fd.get('loan_available')==='true',equity_unknown:unknown.checked,equity_capital:unknown.checked?null:(fd.get('equity_capital')?Number(fd.get('equity_capital')):null),next_follow_up_at:fd.get('next_follow_up_at')||null,notes:fd.get('notes')||null,desired_feature_tags:desiredFeatures,preferred_property_types:crm398ReadPreferredTypes()};
    let customerId=id;
    if(id){const {error}=await state.client.from('customers').update(payload).eq('id',id);if(error)return toast(error.message)}else{const {data,error}=await state.client.from('customers').insert(payload).select('id').single();if(error)return toast(error.message);customerId=data.id}
    const [{error:dDel},{error:cDel}]=await Promise.all([state.client.from('customer_deal_options').delete().eq('customer_id',customerId),state.client.from('customer_contacts').delete().eq('customer_id',customerId)]);
    if(dDel||cDel){
...
function closeNetworkMap(){
  ...
  try{state.kakaoMapInfoWindow?.close?.()}catch(e){}
  try{state.kakaoMapClusterer?.clear?.()}catch(e){}
  try{(state.kakaoMapOverlays||[]).forEach(x=>x?.setMap?.(null))}catch(e){}
  ...
  window.saveContractDocument = async function(e){
    ...
    try{
      ...
      if(insertError){
        ...
        if(/contract_document_one_target|contract_document_at_most_one_target/i.test(msg)){
          throw new Error('계약서 독립 등록 허용 설정이 아직 적용되지 않았습니다. 압축파일의 UPDATE_v3.8.84_계약서_독립등록_제약조건수정.sql을 Supabase에서 실행해 주세요.');
        }
...
/* ===== CRM v3.8.84 계약서 독립등록 제약조건 안내 ===== */
console.info('CRM v3.8.84 계약서 독립등록 제약조건 수정 안내 로드 완료');

/* ===== CRM v3.8.85 간편 계약관리 ===== */
state.contractCases = [];
state.contractCaseFiles = {};

function crm3885Money(v){
  const n=Number(v||0);
  if(!n)return '-';
  if(n>=10000){
    const e=n/10000;
    return `${String(Number(e.toFixed(2)))}억`;
  }
  return `${n.toLocaleString('ko-KR')}만`;
}
function crm3885Phone(v){return crm381FormatPhone(String(v||''))}
function crm3885Progress(c){
  if(c.balance_completed)return '잔금 완료';
  if(c.contract_completed)return '본계약 완료';
  if(c.provisional_completed)return '가계약 완료';
  if(c.balance_date)return '잔금 예정';
  if(c.contract_date)return '본계약 예정';
  if(c.precontract_date)return '가계약 예정';
  return '등록';
}
function crm3885ProgressColor(label){
  if(label.includes('완료'))return 'green';
  if(label.includes('예정'))return 'blue';
  return 'gray';
}
function crm3885NoticeCount(c){
  const keys=['notice_contract','notice_balance','notice_documents','notice_fee'];
  return keys.filter(k=>!!c[k]).length;
}
function crm3885AutoFee(amount,monthly,dealType,rate){
  const base=dealType==='월세'?Number(amount||0)+(Number(monthly||0)*100):Number(amount||0);
  return Math.round(base*(Number(rate||0.4)/100)*100)/100;
}
function crm3885NextNo(){
  const year=new Date().getFullYear();
  const same=(state.contractCases||[]).filter(x=>String(x.contract_no||'').startsWith(`${year}-`));
  const max=same.reduce((m,x)=>Math.max(m,Number(String(x.contract_no).split('-').pop())||0),0);
  return `${year}-${String(max+1).padStart(4,'0')}`;
}
function crm3885PartyLabels(type){return type==='매매'?['매도인','매수인']:['임대인','임차인']}

async function crm3885LoadCases(){
  const {data,error}=await state.client.from('contract_cases').select('*').order('created_at',{ascending:false});
  if(error){
    const msg=String(error.message||'');
    if(/contract_cases|schema cache/i.test(msg)) toast('간편 계약관리 SQL을 먼저 실행해 주세요.');
    else toast(msg);
    return [];
  }
  state.contractCases=data||[];
  return state.contractCases;
}
async function crm3885LoadFiles(caseId){
  const {data,error}=await state.client.from('contract_case_files').select('*').eq('case_id',caseId).order('sort_order',{ascending:true}).order('created_at',{ascending:true});
  if(error){toast(error.message);return []}
  state.contractCaseFiles[caseId]=data||[];
  return data||[];
}
...
renderDocuments=async function(){
  ...
  const q='';
  $('#content').innerHTML=`
    <div class="crm3885-contract-page">
      <div class="panel crm3885-contract-head">
        <div>
          <h2>계약서류</h2>
          <div class="muted">핵심정보만 빠르게 등록하고, 진행상황과 서류는 상세에서 관리합니다.</div>
        </div>
        <button class="primary" onclick="openContractDocumentForm()">+ 계약 등록</button>
      </div>
      <div class="crm3885-summary">
        <div><span>전체</span><strong>${rows.length}</strong></div>
        <div><span>진행중</span><strong>${rows.filter(x=>!x.balance_completed).length}</strong></div>
        <div><span>잔금 예정</span><strong>${rows.filter(x=>x.balance_date&&!x.balance_completed).length}</strong></div>
        <div><span>수수료 미수취</span><strong>${rows.filter(x=>!x.fee_received).length}</strong></div>
      </div>
      <div class="panel">
        <div class="filters crm3885-filters">
          <input id="crm3885Search" placeholder="계약번호·주소·이름·연락처 검색" oninput="crm3885FilterCases()">
          <select id="crm3885Status" onchange="crm3885FilterCases()"><option value="">전체 진행상태</option><option>등록</option><option>가계약 예정</option><option>가계약 완료</option><option>본계약 예정</option><option>본계약 완료</option><option>잔금 예정</option><option>잔금 완료</option></select>
          <select id="crm3885Deal" onchange="crm3885FilterCases()"><option value="">전체 거래유형</option><option>매매</option><option>전세</option><option>월세</option></select>
        </div>
        <div id="documentList"></div>
      </div>
    </div>`;
  crm3885RenderCaseTable(rows);
};

function crm3885FilterCases(){
  const q=normalizeText($('#crm3885Search')?.value||'');
  const st=$('#crm3885Status')?.value||'';
  const deal=$('#crm3885Deal')?.value||'';
  const rows=(state.contractCases||[]).filter(c=>{
    const hay=normalizeText(`${c.contract_no} ${c.address} ${c.party1_name} ${c.party1_phone} ${c.party2_name} ${c.party2_phone}`);
    return (!q||hay.includes(q))&&(!st||crm3885Progress(c)===st)&&(!deal||c.deal_type===deal);
  });
  crm3885RenderCaseTable(rows);
}
function crm3885RenderCaseTable(rows){
  ...
  el.innerHTML=rows.length?`<div class="table-wrap"><table class="crm3885-contract-table"><thead><tr><th>순번</th><th>진행상태</th><th>계약번호</th><th>주소·동호수</th><th>유형·금액</th><th>계약자</th><th><button type="button" class="crm3886-sort-head" onclick="crm3886ToggleContractDateSort('contract_date')">본계약일 <span>${crm3886SortIcon('contract_date')}</span></button></th><th><button type="button" class="crm3886-sort-head" onclick="crm3886ToggleContractDateSort('balance_date')">잔금일 <span>${crm3886SortIcon('balance_date')}</span></button></th><th>중개보수</th><th>안내</th><th>계약서</th><th>관리</th></tr></thead><tbody>${rows.map((c,i)=>{
    ...
    return `<tr>
      <td>${i+1}</td>
      <td>${badge(progress,crm3885ProgressColor(progress))}</td>
      <td><strong>${escapeHtml(c.contract_no||'-')}</strong><br><span class="muted">${escapeHtml(c.brokerage_type||'-')}</span></td>
      <td class="crm3885-address"><button class="crm3885-link" onclick="openContractCaseDetail('${c.id}')">${escapeHtml(c.address||'-')}</button></td>
      <td><strong>${escapeHtml(c.deal_type||'-')}</strong><br>${escapeHtml(amount)}</td>
      <td><div><b>${labels[0]}</b> ${escapeHtml(c.party1_name||'-')}<br><span class="muted">${escapeHtml(crm3885Phone(c.party1_phone)||'')}</span></div><div class="crm3885-party-gap"><b>${labels[1]}</b> ${escapeHtml(c.party2_name||'-')}<br><span class="muted">${escapeHtml(crm3885Phone(c.party2_phone)||'')}</span></div></td>
      <td>${fmtDate(c.contract_date)}</td><td>${fmtDate(c.balance_date)}</td>
      <td>${c.fee_received?badge('수취 완료','green'):badge('미수취','gray')}<br><span class="muted">${crm3885Money(c.fee_agreed||c.fee_auto)}</span></td>
      <td>${crm3885NoticeCount(c)}/4</td>
      <td><button onclick="crm3885OpenFiles('${c.id}')">계약서 보기</button></td>
      <td><div class="row-actions"><button class="primary" onclick="openContractCaseDetail('${c.id}')">상세</button><button class="danger" onclick="crm3885DeleteCase('${c.id}')">삭제</button></div></td>
    </tr>`}).join('')}</tbody></table></div>`:'<div class="empty">등록된 계약이 없습니다.</div>';
}

openContractDocumentForm=function(){
  const no=crm3885NextNo();
  $('#modalTitle').textContent='간편 계약 등록';
  $('#modalBody').innerHTML=`<div class="crm3885-quick-form">
    <div class="crm3885-form-note">필수정보만 먼저 등록하세요. 진행상황·안내·중개보수는 등록 후 상세에서 관리합니다.</div>
    <div class="form-grid crm3885-form-grid">
      <label>계약번호<input id="cdContractNo" value="${escapeHtml(no)}"></label>
      <label>계약구분<select id="cdBrokerageType"><option>손님측 단타</option><option>매물측 단타</option><option>양타</option><option>공동중개</option></select></label>
      <label class="span-2">주소·동호수<input id="cdAddress" placeholder="예: 일산서구 대화동 2702 일산그라비스타 104동 3205호"></label>
      <label>거래유형<select id="cdDealType" onchange="crm3885SyncQuickForm()"><option>전세</option><option>매매</option><option>월세</option></select></label>
      <label>거래금액/보증금(만원)<input id="cdAmount" type="number" min="0"></label>
      <label id="cdMonthlyWrap" style="display:none">월세(만원)<input id="cdMonthlyRent" type="number" min="0"></label>
      <label id="cdParty1Label">임대인 이름<input id="cdParty1Name"></label>
      <label>연락처<input id="cdParty1Phone" placeholder="010-0000-0000"></label>
      <label id="cdParty2Label">임차인 이름<input id="cdParty2Name"></label>
      <label>연락처<input id="cdParty2Phone" placeholder="010-0000-0000"></label>
      <label>가계약일<input id="cdPrecontractDate" type="date"></label>
      <label>본계약일<input id="cdContractDate" type="date"></label>
      <label>잔금일<input id="cdBalanceDate" type="date"></label>
      <label class="span-2">계약서 사진·파일 <span class="muted">여러 장 선택 가능</span><input id="cdFiles" type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx,.hwp,.hwpx"></label>
      <label class="span-2">메모<textarea id="cdNotes" rows="3" placeholder="특약, 수수료 약정, 확인할 내용을 간단히 적으세요."></textarea></label>
    </div>
  </div>`;
  ['cdParty1Phone','cdParty2Phone'].forEach(id=>$('#'+id)?.addEventListener('input',e=>e.target.value=crm3885Phone(e.target.value)));
  const submit=$('#modalSubmit');submit.style.display='';submit.type='button';submit.textContent='계약 등록';submit.onclick=crm3885SaveQuickCase;
  $('#modal').showModal();
};
function crm3885SyncQuickForm(){
  const type=$('#cdDealType')?.value||'전세';
  const [a,b]=crm3885PartyLabels(type);
  const p1=$('#cdParty1Label'),p2=$('#cdParty2Label');
  if(p1)p1.childNodes[0].textContent=`${a} 이름`;
  if(p2)p2.childNodes[0].textContent=`${b} 이름`;
  if($('#cdMonthlyWrap'))$('#cdMonthlyWrap').style.display=type==='월세'?'':'none';
}
async function crm3885SaveQuickCase(){
  const submit=$('#modalSubmit');if(submit.disabled)return;
  const get=id=>$('#'+id);
  const address=get('cdAddress')?.value.trim();
  const dealType=get('cdDealType')?.value;
  if(!address)return toast('주소·동호수를 입력하세요.');
  const amount=Number(get('cdAmount')?.value||0)||null;
  const monthly=dealType==='월세'?(Number(get('cdMonthlyRent')?.value||0)||null):null;
  const rate=0.4;
  const history=[{at:new Date().toISOString(),text:'계약 등록',by:state.profile.full_name||''}];
  const row={
    contract_no:get('cdContractNo')?.value.trim()||crm3885NextNo(),brokerage_type:get('cdBrokerageType')?.value||'손님측 단타',address,deal_type:dealType,
    amount,monthly_rent:monthly,party1_name:get('cdParty1Name')?.value.trim()||null,party1_phone:crm3885Phone(get('cdParty1Phone')?.value)||null,
    party2_name:get('cdParty2Name')?.value.trim()||null,party2_phone:crm3885Phone(get('cdParty2Phone')?.value)||null,
    precontract_date:get('cdPrecontractDate')?.value||null,contract_date:get('cdContractDate')?.value||null,balance_date:get('cdBalanceDate')?.value||null,
    notes:get('cdNotes')?.value.trim()||null,fee_rate:rate,fee_auto:crm3885AutoFee(amount,monthly,dealType,rate),history,uploaded_by:state.profile.id
  };
  try{
    submit.disabled=true;submit.textContent='등록 중...';
    const {data,error}=await state.client.from('contract_cases').insert(row).select().single();
    if(error)throw error;
    const files=[...(get('cdFiles')?.files||[])];
    let sort=0;
    for(const file of files){
      if(file.size>20*1024*1024)continue;
      const safe=(file.name||'contract-file').normalize('NFKD').replace(/[^a-zA-Z0-9._-]/g,'_').replace(/_+/g,'_');
      const path=`cases/${data.id}/${crypto.randomUUID()}-${safe}`;
      const {error:u}=await state.client.storage.from('contract-documents').upload(path,file,{upsert:false,contentType:file.type||undefined});
      if(u)continue;
      const {error:ferr}=await state.client.from('contract_case_files').insert({case_id:data.id,file_name:file.name,storage_path:path,mime_type:file.type||null,sort_order:sort++,uploaded_by:state.profile.id});
      if(ferr)await state.client.storage.from('contract-documents').remove([path]);
    }
    $('#modal').close();toast('계약을 등록했습니다.');await renderDocuments();
  }catch(err){
    console.error(err);toast(err.message||'계약 등록에 실패했습니다.');
  }finally{submit.disabled=false;submit.textContent='계약 등록'}
}

async function openContractCaseDetail(id){
  const c=(state.contractCases||[]).find(x=>x.id===id)||((await crm3885LoadCases()).find(x=>x.id===id));
  if(!c)return toast('계약을 찾지 못했습니다.');
  const files=await crm3885LoadFiles(id);
  const labels=crm3885PartyLabels(c.deal_type);
  $('#modalTitle').textContent=`${c.contract_no||'계약'} · 상세관리`;
  $('#modalBody').innerHTML=`<div class="crm3885-detail">
    <section class="crm3885-hero">
      <div><span class="crm3885-status">${escapeHtml(crm3885Progress(c))}</span><h3>${escapeHtml(c.address)}</h3><p>${escapeHtml(c.deal_type)} ${escapeHtml(c.deal_type==='월세'?`${crm3885Money(c.amount)} / 월 ${crm3885Money(c.monthly_rent)}`:crm3885Money(c.amount))} · ${escapeHtml(c.brokerage_type)}</p></div>
      <button onclick="crm3885OpenFiles('${c.id}')">계약서 ${files.length}개 보기</button>
    </section>
    <section class="crm3885-detail-section"><h4>계약 기본정보</h4><div class="form-grid crm3885-form-grid">
      <label>계약번호<input id="ccNo" value="${escapeHtml(c.contract_no||'')}"></label><label>계약구분<select id="ccBrokerage">${['손님측 단타','매물측 단타','양타','공동중개'].map(v=>`<option ${c.brokerage_type===v?'selected':''}>${v}</option>`).join('')}</select></label>
      <label class="span-2">주소·동호수<input id="ccAddress" value="${escapeHtml(c.address||'')}"></label>
      <label>거래유형<select id="ccDealType">${['매매','전세','월세'].map(v=>`<option ${c.deal_type===v?'selected':''}>${v}</option>`).join('')}</select></label>
      <label>거래금액/보증금(만원)<input id="ccAmount" type="number" value="${c.amount??''}"></label>
      <label>월세(만원)<input id="ccMonthly" type="number" value="${c.monthly_rent??''}"></label>
    </div></section>
    <section class="crm3885-detail-section"><h4>계약자</h4><div class="form-grid crm3885-form-grid">
      <label>${labels[0]} 이름<input id="ccP1Name" value="${escapeHtml(c.party1_name||'')}"></label><label>연락처<input id="ccP1Phone" value="${escapeHtml(crm3885Phone(c.party1_phone)||'')}"></label>
      <label>${labels[1]} 이름<input id="ccP2Name" value="${escapeHtml(c.party2_name||'')}"></label><label>연락처<input id="ccP2Phone" value="${escapeHtml(crm3885Phone(c.party2_phone)||'')}"></label>
    </div></section>
    <section class="crm3885-detail-section"><div class="crm3885-section-title"><h4>진행상황</h4><span class="muted">완료 체크 시 히스토리에 자동 기록됩니다.</span></div>
      ${crm3885StageRow('가계약','precontract',c.precontract_date,null,c.provisional_completed,false)}
      ${crm3885StageRow('본계약','contract',c.contract_date,c.contract_time,c.contract_completed,false)}
      ${crm3885StageRow('중도금','interim',c.interim_date,null,c.interim_completed,c.interim_not_applicable)}
      ${crm3885StageRow('잔금','balance',c.balance_date,c.balance_time,c.balance_completed,false)}
    </section>
    <section class="crm3885-detail-section"><h4>안내 진행</h4><div class="crm3885-notices">
      ${[['notice_contract','본계약 일정 안내'],['notice_balance','잔금일 안내'],['notice_documents','필요서류 안내'],['notice_fee','중개보수 안내']].map(([k,l])=>`<label><input type="checkbox" id="cc_${k}" ${c[k]?'checked':''}>${l}</label>`).join('')}
    </div></section>
    <section class="crm3885-detail-section"><h4>중개보수</h4><div class="crm3885-fee-grid">
      <label>적용요율(%)<input id="ccFeeRate" type="number" step="0.01" value="${c.fee_rate??0.4}"></label>
      <label>자동 계산액(만원)<input id="ccFeeAuto" type="number" value="${c.fee_auto??''}" readonly></label>
      <label>약정 금액(만원)<input id="ccFeeAgreed" type="number" value="${c.fee_agreed??''}"></label>
      <label class="crm3885-check"><input id="ccFeeReceived" type="checkbox" ${c.fee_received?'checked':''}>수취 완료</label>
    </div><div class="muted">자동 계산액은 입력한 금액과 적용요율을 기준으로 한 참고값입니다.</div></section>
    <section class="crm3885-detail-section"><h4>FU·메모</h4><div class="form-grid crm3885-form-grid"><label>다음 FU<input id="ccNextFu" type="date" value="${c.next_fu||''}"></label><label>FU 내용<input id="ccFuNote" value="${escapeHtml(c.fu_note||'')}"></label><label class="span-2">메모<textarea id="ccNotes" rows="3">${escapeHtml(c.notes||'')}</textarea></label></div></section>
    <section class="crm3885-detail-section"><h4>계약 히스토리</h4><div class="crm3885-history">${(c.history||[]).slice().reverse().map(h=>`<div><strong>${new Date(h.at).toLocaleString('ko-KR')}</strong><span>${escapeHtml(h.text||'')}</span></div>`).join('')||'<div class="empty">기록이 없습니다.</div>'}</div></section>
  </div>`;
  ['ccP1Phone','ccP2Phone'].forEach(k=>$('#'+k)?.addEventListener('input',e=>e.target.value=crm3885Phone(e.target.value)));
  const submit=$('#modalSubmit');submit.style.display='';submit.type='button';submit.textContent='변경사항 저장';submit.onclick=()=>crm3885SaveDetail(id);
  $('#modal').showModal();
}
function crm3885StageRow(label,key,date,time,done,na){
  return `<div class="crm3885-stage ${na?'disabled':''}">
    <div class="crm3885-stage-name"><strong>${label}</strong><label><input type="checkbox" id="cc_${key}_done" ${done?'checked':''} ${na?'disabled':''}> 완료</label></div>
    <input id="cc_${key}_date" type="date" value="${date||''}" ${na?'disabled':''}>
    ${['contract','balance'].includes(key)?`<input id="cc_${key}_time" type="time" value="${time||''}" ${na?'disabled':''}>`:''}
    ${key==='interim'?`<label class="crm3885-na"><input type="checkbox" id="cc_interim_na" ${na?'checked':''} onchange="crm3885ToggleInterimNA(this.checked)"> 해당없음</label>`:''}
  </div>`
}
function crm3885ToggleInterimNA(checked){
  ['cc_interim_done','cc_interim_date'].forEach(id=>{const e=$('#'+id);if(e){e.disabled=checked;if(checked&&e.type==='checkbox')e.checked=false}});
  $('#cc_interim_na')?.closest('.crm3885-stage')?.classList.toggle('disabled',checked);
}
async function crm3885SaveDetail(id){
  const c=(state.contractCases||[]).find(x=>x.id===id);if(!c)return;
  const g=id=>$('#'+id);
  const deal=g('ccDealType').value,amount=Number(g('ccAmount').value||0)||null,monthly=deal==='월세'?(Number(g('ccMonthly').value||0)||null):null,rate=Number(g('ccFeeRate').value||0.4);
  const next={
    contract_no:g('ccNo').value.trim(),brokerage_type:g('ccBrokerage').value,address:g('ccAddress').value.trim(),deal_type:deal,amount,monthly_rent:monthly,
    party1_name:g('ccP1Name').value.trim()||null,party1_phone:crm3885Phone(g('ccP1Phone').value)||null,party2_name:g('ccP2Name').value.trim()||null,party2_phone:crm3885Phone(g('ccP2Phone').value)||null,
    precontract_date:g('cc_precontract_date').value||null,contract_date:g('cc_contract_date').value||null,contract_time:g('cc_contract_time')?.value||null,
    interim_date:g('cc_interim_date').value||null,balance_date:g('cc_balance_date').value||null,balance_time:g('cc_balance_time')?.value||null,
    provisional_completed:g('cc_precontract_done').checked,contract_completed:g('cc_contract_done').checked,interim_completed:g('cc_interim_done').checked,interim_not_applicable:g('cc_interim_na').checked,balance_completed:g('cc_balance_done').checked,
    notice_contract:g('cc_notice_contract').checked,notice_balance:g('cc_notice_balance').checked,notice_documents:g('cc_notice_documents').checked,notice_fee:g('cc_notice_fee').checked,
    fee_rate:rate,fee_auto:crm3885AutoFee(amount,monthly,deal,rate),fee_agreed:Number(g('ccFeeAgreed').value||0)||null,fee_received:g('ccFeeReceived').checked,
    next_fu:g('ccNextFu').value||null,fu_note:g('ccFuNote').value.trim()||null,notes:g('ccNotes').value.trim()||null,updated_at:new Date().toISOString()
  };
  const history=Array.isArray(c.history)?[...c.history]:[];
  [['가계약','provisional_completed'],['본계약','contract_completed'],['중도금','interim_completed'],['잔금','balance_completed']].forEach(([label,key])=>{
    if(!!c[key]!==!!next[key])history.push({at:new Date().toISOString(),text:`${label} ${next[key]?'완료':'완료 취소됨'}`,by:state.profile.full_name||''});
  });
  [['본계약 일정 안내','notice_contract'],['잔금일 안내','notice_balance'],['필요서류 안내','notice_documents'],['중개보수 안내','notice_fee'],['중개보수 수취','fee_received']].forEach(([label,key])=>{
    if(!!c[key]!==!!next[key])history.push({at:new Date().toISOString(),text:`${label} ${next[key]?'완료':'취소됨'}`,by:state.profile.full_name||''});
  });
  next.history=history;
  const submit=$('#modalSubmit');submit.disabled=true;submit.textContent='저장 중...';
  const {error}=await state.client.from('contract_cases').update(next).eq('id',id);
  submit.disabled=false;submit.textContent='변경사항 저장';
  if(error)return toast(error.message);
  $('#modal').close();toast('계약 정보를 저장했습니다.');await renderDocuments();
}

async function crm3885OpenFiles(caseId,index=0){
  const files=await crm3885LoadFiles(caseId);
  const c=(state.contractCases||[]).find(x=>x.id===caseId);
  $('#modalTitle').textContent=`계약서 보기 · ${c?.contract_no||''}`;
  $('#modalBody').innerHTML=`<div class="crm3885-file-viewer"><div class="crm3885-file-actions"><label class="primary">+ 파일 추가<input id="crm3885AddFiles" type="file" multiple hidden onchange="crm3885AddFiles('${caseId}',this.files)"></label></div><div id="crm3885ViewerBody"></div></div>`;
  $('#modalSubmit').style.display='none';$('#modal').showModal();
  await crm3885RenderViewer(caseId,files,index);
}
async function crm3885RenderViewer(caseId,files,index=0){
  const body=$('#crm3885ViewerBody');if(!body)return;
  if(!files.length){body.innerHTML='<div class="empty">등록된 계약서가 없습니다.</div>';return}
  index=Math.max(0,Math.min(index,files.length-1));
  const f=files[index];
  const {data,error}=await state.client.storage.from('contract-documents').createSignedUrl(f.storage_path,3600);
  if(error){body.innerHTML=`<div class="empty">${escapeHtml(error.message)}</div>`;return}
  const url=data.signedUrl,mime=String(f.mime_type||'');
  const preview=mime.startsWith('image/')||/\.(png|jpe?g|webp|gif)$/i.test(f.file_name)
    ?`<img src="${url}" alt="${escapeHtml(f.file_name)}">`
    :mime.includes('pdf')||/\.pdf$/i.test(f.file_name)?`<iframe src="${url}"></iframe>`:`<div class="crm3885-nopreview"><strong>${escapeHtml(f.file_name)}</strong><a class="primary" href="${url}" target="_blank">파일 열기</a></div>`;
  body.innerHTML=`<div class="crm3885-viewer-top"><button ${index===0?'disabled':''} onclick="crm3885RenderViewer('${caseId}',state.contractCaseFiles['${caseId}'],${index-1})">이전</button><strong>${index+1} / ${files.length}</strong><button ${index===files.length-1?'disabled':''} onclick="crm3885RenderViewer('${caseId}',state.contractCaseFiles['${caseId}'],${index+1})">다음</button></div><div class="crm3885-preview">${preview}</div><div class="crm3885-thumbs">${files.map((x,i)=>`<button class="${i===index?'active':''}" onclick="crm3885RenderViewer('${caseId}',state.contractCaseFiles['${caseId}'],${i})"><span>${i+1}</span>${escapeHtml(x.file_name)}</button>`).join('')}</div><div class="crm3885-file-manage"><button onclick="crm3885MoveFile('${caseId}','${f.id}',-1)">앞으로</button><button onclick="crm3885MoveFile('${caseId}','${f.id}',1)">뒤로</button><button class="danger" onclick="crm3885DeleteFile('${caseId}','${f.id}','${f.storage_path}')">삭제</button></div>`;
}
async function crm3885AddFiles(caseId,fileList){
  const files=[...fileList];let order=(state.contractCaseFiles[caseId]||[]).length;
  for(const file of files){
    if(file.size>20*1024*1024)continue;
    const safe=(file.name||'contract-file').normalize('NFKD').replace(/[^a-zA-Z0-9._-]/g,'_').replace(/_+/g,'_');
    const path=`cases/${caseId}/${crypto.randomUUID()}-${safe}`;
    const {error:u}=await state.client.storage.from('contract-documents').upload(path,file,{upsert:false,contentType:file.type||undefined});if(u)continue;
    const {error:d}=await state.client.from('contract_case_files').insert({case_id:caseId,file_name:file.name,storage_path:path,mime_type:file.type||null,sort_order:order++,uploaded_by:state.profile.id});
    if(d)await state.client.storage.from('contract-documents').remove([path]);
  }
  toast('파일을 추가했습니다.');await crm3885OpenFiles(caseId,0);
}
async function crm3885MoveFile(caseId,fileId,delta){
  const files=state.contractCaseFiles[caseId]||[];const i=files.findIndex(x=>x.id===fileId),j=i+delta;if(i<0||j<0||j>=files.length)return;
  const a=files[i],b=files[j];
  await Promise.all([state.client.from('contract_case_files').update({sort_order:b.sort_order}).eq('id',a.id),state.client.from('contract_case_files').update({sort_order:a.sort_order}).eq('id',b.id)]);
  await crm3885OpenFiles(caseId,j);
}
async function crm3885DeleteFile(caseId,id,path){
  if(!confirm('이 파일을 삭제할까요?'))return;
  await state.client.from('contract_case_files').delete().eq('id',id);await state.client.storage.from('contract-documents').remove([path]);await crm3885OpenFiles(caseId,0);
}
async function crm3885DeleteCase(id){
  if(!confirm('계약정보와 등록된 파일을 모두 삭제할까요?'))return;
  const files=await crm3885LoadFiles(id);if(files.length)await state.client.storage.from('contract-documents').remove(files.map(x=>x.storage_path));
...
(function(){
  ...
  async function crm3888RenderSchedules(customer){
    ...
    if(error){
      if(/customer_fu_schedules|schema cache/i.test(error.message||'')){
        const legacyDate=customer.next_follow_up_at?String(customer.next_follow_up_at).slice(0,10):'';
        return `<div class="crm3888-fu-form crm3108-legacy-fu"><div class="span-2 crm3108-install-note"><strong>예정 FU 기본 모드</strong><p>별도 일정 테이블이 아직 설치되지 않아 날짜 1건만 저장할 수 있습니다. 아래 날짜는 고객 목록의 예정 FU에 바로 반영됩니다.</p></div><label>예정일<input id="crm3108LegacyFuDate" type="date" value="${legacyDate}"></label><div class="crm3888-fu-form-actions"><button type="button" class="ghost" onclick="crm3108SaveLegacyFu('${customer.id}',true)">예정 FU 삭제</button><button type="button" class="primary" onclick="crm3108SaveLegacyFu('${customer.id}',false)">예정 FU 저장</button></div></div>`;
      }
...
(()=>{
  ...
  function crm3889PatchCustomerScroll(){
    ...
    if(!host)return;
    host.querySelectorAll('.crm3889-top-scroll,.crm3890-top-scroll,.crm396-top-scroll,.crm3106-network-top-scroll,.crm3112-network-top-scroll').forEach(el=>el.remove());
    const wrap=host.querySelector('.customer-table-scroll');
    if(wrap && !wrap.dataset.crm3113ShiftWheel){
      wrap.dataset.crm3113ShiftWheel='1';
      wrap.addEventListener('wheel',e=>{if(!e.shiftKey)return;const d=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;if(!d)return;e.preventDefault();wrap.scrollLeft+=d},{passive:false});
    }
  }
...
(()=>{
  function crm3890AttachTopScroll(wrap){
    ...
    parent?.querySelectorAll('.crm3889-top-scroll,.crm3890-top-scroll,.crm396-top-scroll,.crm3106-network-top-scroll,.crm3112-network-top-scroll').forEach(el=>el.remove());
    if(!wrap.dataset.crm3113ShiftWheel){
      wrap.dataset.crm3113ShiftWheel='1';
      wrap.addEventListener('wheel',e=>{if(!e.shiftKey)return;const d=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;if(!d)return;e.preventDefault();wrap.scrollLeft+=d},{passive:false});
    }
  }
...
/* ===== CRM v3.8.93 희망지역 줄바꿈·2줄 말줄임 ===== */
(()=>{
  function patchPreferredArea(){
    document.querySelectorAll('#customerTable table.customer-table').forEach(table=>{
      const headers=[...table.querySelectorAll('thead th')];
      const index=headers.findIndex(th=>th.textContent.trim()==='희망지역');
      if(index<0)return;
      headers[index].classList.add('crm3893-preferred-area-col');
      table.querySelectorAll('tbody tr').forEach(tr=>{
        const cell=tr.children[index];
        if(!cell)return;
        cell.classList.add('crm3893-preferred-area-col');
        if(cell.querySelector('.crm3893-preferred-area'))return;
        const full=cell.textContent.trim()||'-';
        cell.textContent='';
        const span=document.createElement('span');
        span.className='crm3893-preferred-area';
  ...
  const observer=new MutationObserver(()=>requestAnimationFrame(patchPreferredArea));
  observer.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('DOMContentLoaded',()=>setTimeout(patchPreferredArea,0));
  window.addEventListener('resize',()=>setTimeout(patchPreferredArea,0));
...
(()=>{
  ...
  openCustomerModal=function(id=null){
    ...
    if(old){old.type='hidden';old.dataset.changeIgnore='true';}
    ...
    submit.onclick=async function(e){
      ...
      if(error){
        const msg=String(error.message||'');
        if(msg.includes('desired_move_in_period')&&(msg.includes('schema cache')||msg.includes('does not exist')||msg.includes('Could not find'))){
          toast('입주 희망기간 저장 항목이 설치되지 않았습니다. UPDATE_v3.10.21_입주희망일_오류수정.sql을 Supabase에서 실행해 주세요.');
        }else{
          toast(`입주 희망일 저장 실패: ${msg}`);
        }
        return;
      }
...
/* ===== CRM v3.8.95 지도 누락매물 패널·주소 즉시수정 ===== */
(function(){
  state.crm3895MissingMapListings=[];

  function crm3895CanEditListing(listing){
    return !!listing && (state.profile?.role==='admin' || listing.owner_id===state.profile?.id);
  }
  function crm3895MissingReason(listing){
    const address=String(listing?.address||'').trim();
    if(!address)return '주소가 입력되지 않았습니다.';
    if(!/[가-힣]+구/.test(address)||!/[가-힣]+동/.test(address)||!(/\d/.test(address)))return '지번 주소가 불완전하거나 형식이 맞지 않습니다.';
    if(!String(listing?.building_no||'').trim())return '동 정보가 비어 있습니다.';
    if(!String(listing?.unit_no||'').trim())return '호수 정보가 비어 있습니다.';
    return '카카오 주소 검색에서 좌표를 찾지 못했습니다.';
  }
  function crm3895MissingAddressText(x){
    return [x.address,x.building_no,x.unit_no].filter(Boolean).join(' ');
  }
  function crm3895RenderMissingPanel(){
    const panel=document.getElementById('crm3895MissingPanel');
    if(!panel)return;
    const rows=state.crm3895MissingMapListings||[];
    panel.innerHTML=`<div class="crm3895-panel-head"><div><strong>주소 확인 필요</strong><span>${rows.length}건</span></div><button type="button" class="crm3895-panel-close" onclick="crm3895CloseMissingPanel()" aria-label="닫기">×</button></div>
      <div class="crm3895-panel-tools"><button type="button" class="ghost" onclick="crm3895ShowMissingInList()">목록에서 보기</button></div>
      <div class="crm3895-panel-list">${rows.length?rows.map(x=>{
        const canEdit=crm3895CanEditListing(x);
        return `<article class="crm3895-missing-card">
          <div class="crm3895-card-title"><strong>${escapeHtml(x.title||'매물명 없음')}</strong><span>${escapeHtml(x.owner?.full_name||'담당자 미확인')}</span></div>
          <div class="crm3895-card-address">${escapeHtml(crm3895MissingAddressText(x)||'주소 미입력')}</div>
          <div class="crm3895-card-reason">${escapeHtml(crm3895MissingReason(x))}</div>
          <div class="crm3895-card-actions">${canEdit?`<button type="button" class="primary" onclick="crm3895OpenQuickAddress('${x.id}')">주소 수정</button>`:''}<button type="button" class="ghost" onclick="crm3895OpenListing('${x.id}')">매물 열기</button></div>
        </article>`}).join(''):'<div class="crm3895-empty">주소 확인이 필요한 매물이 없습니다.</div>'}</div>`;
  }
  function crm3895OpenMissingPanel(){
    const panel=document.getElementById('crm3895MissingPanel');
    if(!panel)return;
    crm3895RenderMissingPanel();
    panel.classList.add('open');
  }
  function crm3895CloseMissingPanel(){document.getElementById('crm3895MissingPanel')?.classList.remove('open')}
  function crm3895OpenListing(id){crm3895CloseMissingPanel();openListingDetail(id)}
  function crm3895ShowMissingInList(){
    const ids=new Set((state.crm3895MissingMapListings||[]).map(x=>String(x.id)));
    closeNetworkMap();
    setTimeout(()=>{
      const table=document.getElementById('networkTable');
      if(!table)return;
      const rows=(state.filteredNetworkListings||[]).filter(x=>ids.has(String(x.id)));
      renderListingTable(rows,'networkTable',false);
      const summary=document.getElementById('networkSummary');
      if(summary)summary.innerHTML=`<div class="crm3895-missing-list-banner"><strong>주소 확인 필요 매물 ${rows.length}건</strong><button class="ghost" onclick="filterNetwork()">전체 필터 결과로 돌아가기</button></div>`;
    },120);
  }
  function crm3895OpenQuickAddress(id){
    const x=(state.listings||[]).find(v=>String(v.id)===String(id));
    if(!x||!crm3895CanEditListing(x))return toast('이 매물의 주소를 수정할 권한이 없습니다.');
    const body=document.getElementById('modalBody');
    if(!body)return;
    document.getElementById('modalTitle').textContent='지도 주소 빠른 수정';
    body.innerHTML=`<div class="crm3895-quick-address">
      <div class="crm3895-quick-info"><strong>${escapeHtml(x.title||'')}</strong><span>담당 ${escapeHtml(x.owner?.full_name||'-')}</span></div>
      <label class="span-3">주소(지번까지)<input id="crm3895Address" value="${escapeHtml(x.address||'')}" placeholder="예: 서울 강서구 화곡동 1039-27"></label>
      <label>동<input id="crm3895Building" value="${escapeHtml(String(x.building_no||'').replace(/동$/,''))}" placeholder="예: 101"></label>
      <label>호수<input id="crm3895Unit" value="${escapeHtml(String(x.unit_no||'').replace(/호$/,''))}" placeholder="예: 203"></label>
      <div class="crm3895-quick-hint span-3">정확한 지번주소를 입력하면 저장 즉시 카카오 좌표를 다시 찾고 지도에 반영합니다.</div>
    </div>`;
    const submit=document.getElementById('modalSubmit');
    submit.style.display='';submit.textContent='주소 저장 및 지도 반영';
    submit.onclick=()=>crm3895SaveQuickAddress(id);
    document.getElementById('modal').showModal();
  }
  async function crm3895SaveQuickAddress(id){
    const x=(state.listings||[]).find(v=>String(v.id)===String(id));
    if(!x||!crm3895CanEditListing(x))return;
    const address=String(document.getElementById('crm3895Address')?.value||'').trim();
    const buildingRaw=String(document.getElementById('crm3895Building')?.value||'').trim().replace(/[^0-9A-Za-z]/g,'');
    const unitRaw=String(document.getElementById('crm3895Unit')?.value||'').trim().replace(/[^0-9A-Za-z]/g,'');
    if(!address||!unitRaw)return toast('주소와 호수를 입력하세요.');
    const building=`${buildingRaw||'1'}동`,unit=`${unitRaw}호`;
    const btn=document.getElementById('modalSubmit');
    btn.disabled=true;btn.textContent='좌표 확인 중...';
    try{
      const coord=await crm3868GeocodeKakao(address);
      if(!coord)throw new Error('카카오 주소 검색에서 좌표를 찾지 못했습니다. 지번주소를 다시 확인하세요.');
      const payload={address,building_no:building,unit_no:unit,latitude:coord.lat,longitude:coord.lng,coordinate_provider:'kakao',jibun_address:coord.jibunAddress||address,road_address:coord.roadAddress||null};
      const {error}=await state.client.from('listings').update(payload).eq('id',id);
      if(error)throw error;
      Object.assign(x,payload);
      document.getElementById('modal').close();
      state.crm3895MissingMapListings=(state.crm3895MissingMapListings||[]).filter(v=>String(v.id)!==String(id));
      crm3895RenderMissingPanel();
      toast('주소와 지도 좌표를 수정했습니다.');
      if(state.networkMapMode)crm3868RenderKakaoMap(state.filteredNetworkListings||[]);
    }catch(e){toast(e.message||'주소 저장에 실패했습니다.')}finally{btn.disabled=false;btn.textContent='주소 저장 및 지도 반영'}
  }

  const baseRenderKakao=crm3868RenderKakaoMap;
  crm3868RenderKakaoMap=async function(rows){
    await baseRenderKakao(rows);
    const validIds=new Set();
    try{
      (state.kakaoMapClusterer?._markers||[]).forEach(m=>{if(m?.__listing?.id)validIds.add(String(m.__listing.id))});
    }catch(e){}
    if(!validIds.size){
      rows.forEach(x=>{if(Number.isFinite(Number(x.latitude))&&Number.isFinite(Number(x.longitude))&&x.coordinate_provider==='kakao')validIds.add(String(x.id))});
    }
    state.crm3895MissingMapListings=(rows||[]).filter(x=>!validIds.has(String(x.id)));
    const status=document.getElementById('networkMapStatus');
    if(status){
      const shown=(rows||[]).length-state.crm3895MissingMapListings.length;
      status.innerHTML=`<span class="crm3895-map-count">지도 표시 ${shown}건</span>${state.crm3895MissingMapListings.length?`<button type="button" class="crm3895-missing-badge" onclick="crm3895OpenMissingPanel()">주소 확인 필요 ${state.crm3895MissingMapListings.length}건 <span>›</span></button>`:''}`;
    }
  };
  crm3866RenderMap=crm3868RenderKakaoMap;
  ...
  renderNetwork=async function(){
    await baseRenderNetwork();
    if(state.networkMapMode){
      const panelHost=document.querySelector('.crm3866-network-panel');
      if(panelHost&&!document.getElementById('crm3895MissingPanel'))panelHost.insertAdjacentHTML('beforeend','<aside id="crm3895MissingPanel" class="crm3895-missing-panel"></aside>');
    }
  };
  Object.assign(window,{renderNetwork,crm3868RenderKakaoMap,crm3895OpenMissingPanel,crm3895CloseMissingPanel,crm3895OpenListing,crm3895ShowMissingInList,crm3895OpenQuickAddress,crm3895SaveQuickAddress});
  console.info('CRM v3.8.95 지도 누락매물 패널 및 주소 즉시수정 적용 완료');
})();

/* ================= CRM v3.9.0 업무 안정화·추천·백업 패키지 ================= */
(()=>{
  const V='3900';
  const q=s=>document.querySelector(s), qa=s=>[...document.querySelectorAll(s)];
  const safe=v=>v===null||v===undefined?'':String(v);
  const primitiveObject=(obj)=>Object.fromEntries(Object.entries(obj||{}).filter(([k,v])=>!['owner','contacts','deal_options','photos','writer'].includes(k)&&(v===null||['string','number','boolean'].includes(typeof v)||Array.isArray(v))));
  const entityLabel=t=>t==='customer'?'고객':'매물';
  const draftKey=(type,id)=>`crm390:draft:${state.profile?.id||'anon'}:${type}:${id||'new'}`;
  const normalizePhone=s=>safe(s).replace(/\D/g,'');
  const fieldLabel={name:'고객명',title:'매물명',status:'상태',customer_type:'구분',deal_type:'거래유형',customer_grade:'등급',preferred_area:'희망지역',budget_max:'희망금액',desired_rooms:'방 개수',loan_available:'대출 여부',equity_capital:'자기자본금',address:'주소',building_dong:'동',unit_no:'호수',property_type:'매물유형',transaction_type:'거래유형',price:'금액',monthly_rent:'월세',area_m2:'전용면적'};

  function serializeForm(form){
    const out={}; if(!form)return out;
    qaFrom(form,'input,select,textarea').forEach(el=>{
      if(!el.name&& !el.id)return; const k=el.name||el.id;
      if(el.type==='file')return;
      if(el.type==='checkbox'){
        if(!out[k])out[k]=[];
        if(el.checked)out[k].push(el.value||true);
      }else if(el.type==='radio'){if(el.checked)out[k]=el.value}
      else out[k]=el.value;
    }); return out;
  }
  function qaFrom(root,sel){return [...root.querySelectorAll(sel)]}
  function restoreForm(form,data){
    qaFrom(form,'input,select,textarea').forEach(el=>{
      const k=el.name||el.id;if(!k||!(k in data)||el.type==='file')return;
      if(el.type==='checkbox'){const arr=Array.isArray(data[k])?data[k]:[data[k]];el.checked=arr.map(String).includes(String(el.value||true));}
      else if(el.type==='radio')el.checked=String(data[k])===String(el.value);
      else el.value=data[k]??'';
      el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));
    });
  }
  function diffForm(before,after){
    const keys=new Set([...Object.keys(before||{}),...Object.keys(after||{})]);
    return [...keys].filter(k=>JSON.stringify(before?.[k]??'')!==JSON.stringify(after?.[k]??'')).map(k=>({key:k,label:fieldLabel[k]||k,before:before?.[k]??'',after:after?.[k]??''}));
  }
  function formatDiff(d){const f=v=>Array.isArray(v)?v.join(', '):(safe(v)||'없음');return d.slice(0,15).map(x=>`${x.label}: ${f(x.before)} → ${f(x.after)}`).join('\n')+(d.length>15?`\n외 ${d.length-15}건`:'' )}

  async function saveSnapshot(type,id,reason='수정 전 자동 저장'){
    if(!id)return; const item=(type==='customer'?state.customers:state.listings).find(x=>String(x.id)===String(id));if(!item)return;
    try{await state.client.from('entity_snapshots').insert({entity_type:type,entity_id:id,snapshot:primitiveObject(item),reason,created_by:state.profile.id});}catch(e){console.warn('snapshot',e)}
  }

  function installDraftAndSummary(type,id){
    const modal=q('#modal'), form=q('#modalForm'); if(!modal||!form)return;
    const key=draftKey(type,id); const initial=serializeForm(form); form.dataset.crm390Initial=JSON.stringify(initial);
    const saved=localStorage.getItem(key);
    if(saved){
      try{
        const parsed=JSON.parse(saved);
        const resume=window.confirm('작성 중이던 임시저장 내용이 있습니다. 이어서 작성할까요?');
        if(resume){
          restoreForm(form,parsed.data||parsed);
        }else{
          // 사용자가 이어쓰기를 거절하면 해당 초안을 즉시 폐기한다.
          // 같은 초안으로 다시 묻지 않으며, 이후 새로 입력한 내용만 다시 임시저장된다.
          localStorage.removeItem(key);
          form.dataset.crm390DraftDiscarded='1';
        }
      }catch(e){
        localStorage.removeItem(key);
      }
    }
    let timer; const autosave=()=>{clearTimeout(timer);timer=setTimeout(()=>{localStorage.setItem(key,JSON.stringify({savedAt:new Date().toISOString(),data:serializeForm(form)}));let tag=q('#crm390DraftTag');if(!tag){tag=document.createElement('span');tag.id='crm390DraftTag';tag.className='crm390-draft-tag';q('#modalTitle')?.appendChild(tag)}tag.textContent=' 임시저장됨';},600)};
    form.addEventListener('input',autosave);form.addEventListener('change',autosave);
    const btn=q('#modalSubmit');if(!btn||btn.dataset.crm390Wrapped)return;btn.dataset.crm390Wrapped='1';const original=btn.onclick;
    btn.onclick=async function(e){
      const after=serializeForm(form);const changes=diffForm(initial,after);
      if(id&&changes.length&&!btn.dataset.crm390Confirmed){e?.preventDefault?.();e?.stopImmediatePropagation?.();if(!confirm(`변경사항을 확인해 주세요.\n\n${formatDiff(changes)}\n\n저장할까요?`))return;btn.dataset.crm390Confirmed='1';await saveSnapshot(type,id);}
      if(type==='customer'&&!id){const dup=crm390FindDuplicateCustomer(form);if(dup?.exact&&!confirm(`같은 전화번호의 고객이 이미 있습니다.\n${dup.exact.name} · ${dup.exact.phone}\n그래도 등록할까요?`))return;}
      localStorage.removeItem(key);
      try{return await original?.call(this,e)}finally{delete btn.dataset.crm390Confirmed}
    };
    modal.addEventListener('close',()=>clearTimeout(timer),{once:true});
    if(id)crm390AddDetailTools(type,id);
  }

  function crm390FindDuplicateCustomer(form){
    const phones=qaFrom(form,'input[type=tel],input[name*=phone],input[id*=Phone]').map(x=>normalizePhone(x.value)).filter(x=>x.length>=8);
    let exact=null,similar=[];
    for(const c of state.customers||[]){const cp=[c.phone,c.contact_phone,...(c.contacts||[]).map(x=>x.phone)].map(normalizePhone).filter(Boolean);if(phones.some(p=>cp.includes(p))){exact={id:c.id,name:c.name,phone:cp[0]};break}if(phones.some(p=>cp.some(v=>p.slice(-8)===v.slice(-8))))similar.push(c)}
    return {exact,similar};
  }

  function crm390AddDetailTools(type,id){
    const body=q('#modalBody');if(!body||q('#crm390DetailTools'))return;
    const tools=document.createElement('section');tools.id='crm390DetailTools';tools.className='crm390-detail-tools';
    tools.innerHTML=`<button type="button" class="ghost" onclick="crm390OpenMatches('${type}','${id}')">${type==='customer'?'추천 매물 바로 보기':'맞는 고객 바로 보기'}</button><button type="button" class="ghost" onclick="crm390OpenRestore('${type}','${id}')">변경 이력 복원</button>${type==='customer'?`<button type="button" class="ghost" onclick="crm390OpenMerge('${id}')">중복 고객 합치기</button>`:''}`;
    body.prepend(tools);
  }

  async function crm390OpenMatches(type,id){
    await loadCustomers();await loadListings();
    let html='';
    if(type==='customer'){
      const c=state.customers.find(x=>String(x.id)===String(id));const rows=(state.listings||[]).filter(l=>l.status==='available'&&l.is_public!==false).map(l=>({l,m:evaluateListingMatch(c,l)})).filter(x=>x.m?.matched).slice(0,20);
      html=rows.length?rows.map(({l,m})=>`<article class="crm390-match-card"><div><strong>${escapeHtml(l.title)}</strong><div class="muted">${escapeHtml(l.address||l.district||'')} · ${listingPriceText(l)}</div><div>${(m.reasons||[]).slice(0,4).map(r=>badge(r,'blue')).join(' ')}</div></div><button class="ghost" onclick="openListingDetail('${l.id}')">열기</button></article>`).join(''):'<div class="empty">조건에 맞는 매물이 없습니다.</div>';
    }else{
      const l=state.listings.find(x=>String(x.id)===String(id));const rows=(state.customers||[]).map(c=>({c,m:evaluateListingMatch(c,l)})).filter(x=>x.m?.matched).slice(0,20);
      html=rows.length?rows.map(({c,m})=>`<article class="crm390-match-card"><div><strong>${escapeHtml(c.name)}</strong><div class="muted">${escapeHtml(c.preferred_area||'')} · ${escapeHtml(c.deal_type||c.customer_type||'')}</div><div>${(m.reasons||[]).slice(0,4).map(r=>badge(r,'blue')).join(' ')}</div></div><button class="ghost" onclick="openCustomerModal('${c.id}')">열기</button></article>`).join(''):'<div class="empty">조건에 맞는 고객이 없습니다.</div>';
    }
    q('#modalTitle').textContent=type==='customer'?'추천 매물':'맞는 고객';q('#modalBody').innerHTML=html;q('#modalSubmit').style.display='none';q('#modal').showModal();
  }

  async function crm390OpenRestore(type,id){
    const {data,error}=await state.client.from('entity_snapshots').select('*').eq('entity_type',type).eq('entity_id',id).order('created_at',{ascending:false}).limit(20);if(error)return toast(error.message);
    q('#modalTitle').textContent=`${entityLabel(type)} 변경 이력 복원`;
    q('#modalBody').innerHTML=data?.length?data.map(s=>`<article class="crm390-snapshot"><div><strong>${new Date(s.created_at).toLocaleString('ko-KR')}</strong><div class="muted">${escapeHtml(s.reason||'수정 전 저장')}</div></div><button class="ghost" onclick="crm390RestoreSnapshot('${s.id}')">이 값으로 복원</button></article>`).join(''):'<div class="empty">복원 가능한 이력이 없습니다.</div>';
    q('#modalSubmit').style.display='none';q('#modal').showModal();
  }
  async function crm390RestoreSnapshot(snapshotId){
    const {data:s,error}=await state.client.from('entity_snapshots').select('*').eq('id',snapshotId).single();if(error)return toast(error.message);if(!confirm('선택한 이전 값으로 복원할까요? 현재 값은 복원 전 자동 저장됩니다.'))return;
    await saveSnapshot(s.entity_type,s.entity_id,'복원 직전 자동 저장');const table=s.entity_type==='customer'?'customers':'listings';const payload={...(s.snapshot||{})};delete payload.id;delete payload.created_at;delete payload.updated_at;
    const {error:u}=await state.client.from(table).update(payload).eq('id',s.entity_id);if(u)return toast(u.message);toast('이전 값으로 복원했습니다.');q('#modal').close();if(s.entity_type==='customer'){await loadCustomers();renderCustomers()}else{await loadListings();state.view==='network'?renderNetwork():renderMyListings()}
  }

  async function crm390OpenMerge(customerId){
    await loadCustomers();const base=state.customers.find(x=>String(x.id)===String(customerId));const candidates=(state.customers||[]).filter(x=>String(x.id)!==String(customerId)).filter(x=>normalizePhone(x.phone||x.contact_phone).slice(-8)===normalizePhone(base.phone||base.contact_phone).slice(-8)||x.name===base.name).slice(0,30);
    q('#modalTitle').textContent='중복 고객 합치기';q('#modalBody').innerHTML=`<div class="notice">현재 고객을 기준 고객으로 유지하고 선택 고객의 연락처·FU·조건·추천 이력을 합칩니다.</div>${candidates.length?candidates.map(c=>`<label class="crm390-merge-row"><input type="radio" name="mergeSource" value="${c.id}"><span><strong>${escapeHtml(c.name)}</strong><br><span class="muted">${escapeHtml(c.phone||c.contact_phone||'')}</span></span></label>`).join(''):'<div class="empty">합칠 만한 중복 고객이 없습니다.</div>'}`;
    q('#modalSubmit').style.display=candidates.length?'':'none';q('#modalSubmit').textContent='선택 고객 합치기';q('#modalSubmit').onclick=async e=>{e.preventDefault();const source=q('input[name=mergeSource]:checked')?.value;if(!source)return toast('합칠 고객을 선택하세요.');if(!confirm('선택 고객의 이력을 현재 고객으로 합치고 선택 고객을 삭제할까요?'))return;const {error}=await state.client.rpc('merge_customers',{p_target:customerId,p_source:source});if(error)return toast(error.message);q('#modal').close();toast('중복 고객을 합쳤습니다.');await loadCustomers();renderCustomers()};q('#modal').showModal();
  }

  async function crm390RunAudit(){
    await loadCustomers();await loadListings();const issues=[];
    (state.customers||[]).forEach(c=>{if(!c.name)issues.push(['고객','고객명 없음',c.id]);if(!normalizePhone(c.phone||c.contact_phone)&&!(c.contacts||[]).some(x=>normalizePhone(x.phone)))issues.push(['고객',`${c.name||'이름 없음'} · 연락처 없음`,c.id]);if(!c.deal_type)issues.push(['고객',`${c.name||'이름 없음'} · 거래유형 없음`,c.id]);});
    (state.listings||[]).forEach(l=>{if(!l.address)issues.push(['매물',`${l.title||'매물명 없음'} · 주소 없음`,l.id]);if(!l.unit_no&&!l.unit)issues.push(['매물',`${l.title||'매물명 없음'} · 호수 없음`,l.id]);if(!crm38DealOptions(l)?.length)issues.push(['매물',`${l.title||'매물명 없음'} · 거래조건 없음`,l.id]);if(!Number.isFinite(Number(l.latitude))||!Number.isFinite(Number(l.longitude)))issues.push(['매물',`${l.title||'매물명 없음'} · 지도 좌표 없음`,l.id]);});
    q('#modalTitle').textContent=`필수정보 누락 점검 · ${issues.length}건`;q('#modalBody').innerHTML=issues.length?issues.map(([t,msg,id])=>`<article class="crm390-audit-row"><span>${badge(t,t==='고객'?'blue':'yellow')} ${escapeHtml(msg)}</span><button class="ghost" onclick="${t==='고객'?`openCustomerModal('${id}')`:`openListingDetail('${id}')`}">열기</button></article>`).join(''):'<div class="empty">필수정보 누락이 없습니다.</div>';q('#modalSubmit').style.display='none';q('#modal').showModal();
  }

  async function crm390CreateBackup(manual=false){
    if(state.profile?.role!=='admin')return;const todayKey=new Date().toISOString().slice(0,10);if(!manual&&localStorage.getItem('crm390:backup:'+todayKey))return;
    try{await loadCustomers();await loadListings();const tables=['interaction_history','customer_follow_up_schedules','customer_condition_history','customer_listing_recommendations','contract_cases','contract_documents'];const extra={};for(const t of tables){const {data}=await state.client.from(t).select('*').limit(10000);extra[t]=data||[]}
      const snapshot={created_at:new Date().toISOString(),customers:state.customers||[],listings:state.listings||[],...extra};const {error}=await state.client.from('crm_backups').insert({backup_date:todayKey,backup_type:manual?'manual':'daily',snapshot,created_by:state.profile.id});if(error)throw error;localStorage.setItem('crm390:backup:'+todayKey,'1');if(manual){const blob=new Blob([JSON.stringify(snapshot,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`더퍼스트CRM_전체백업_${todayKey}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);toast('백업을 저장하고 내려받았습니다.')}}catch(e){console.warn('backup',e);if(manual)toast(e.message||'백업에 실패했습니다.')}
  }

  function crm390InjectAdminTools(){
    if(state.profile?.role!=='admin')return;const host=q('.page-head .actions')||q('.page-head');if(!host||q('#crm390AdminTools'))return;const box=document.createElement('div');box.id='crm390AdminTools';box.className='crm390-admin-tools';box.innerHTML=`<button class="ghost" onclick="crm390RunAudit()">데이터 점검 <small>ADMIN</small></button><button class="ghost" onclick="crm390CreateBackup(true)">전체 백업 <small>ADMIN</small></button>`;host.appendChild(box);
  }

  // 사진 업로드 최적화: 회전정보를 캔버스 재인코딩으로 보정하고 1920px/약 82%로 압축
  async function crm390OptimizeImage(file){if(!file?.type?.startsWith('image/')||file.type==='image/gif')return file;try{const bmp=await createImageBitmap(file,{imageOrientation:'from-image'});let w=bmp.width,h=bmp.height;const max=1920;if(Math.max(w,h)>max){const r=max/Math.max(w,h);w=Math.round(w*r);h=Math.round(h*r)}const c=document.createElement('canvas');c.width=w;c.height=h;c.getContext('2d').drawImage(bmp,0,0,w,h);bmp.close?.();const blob=await new Promise(res=>c.toBlob(res,'image/jpeg',.82));return new File([blob],file.name.replace(/\.[^.]+$/,'.jpg'),{type:'image/jpeg',lastModified:Date.now()})}catch(e){return file}}
  const baseUpload=uploadListingPhotos;uploadListingPhotos=async function(listingId,files){const optimized=[];for(const f of [...files])optimized.push(await crm390OptimizeImage(f));return baseUpload(listingId,optimized)};

  // 기존 모달 함수에 안전하게 훅 적용
  const baseCustomer=openCustomerModal;openCustomerModal=function(id){const r=baseCustomer(id);setTimeout(()=>installDraftAndSummary('customer',id),80);return r};
  const baseListing=openListingModal;openListingModal=function(id){const r=baseListing(id);setTimeout(()=>installDraftAndSummary('listing',id),80);return r};
  const baseDetail=typeof openListingDetail==='function'?openListingDetail:null;if(baseDetail)openListingDetail=function(id){const r=baseDetail(id);setTimeout(()=>crm390AddDetailTools('listing',id),100);return r};
  const baseRenderView=renderView;renderView=async function(v){const r=await baseRenderView(v);setTimeout(crm390InjectAdminTools,100);return r};

  // 앱 활성 시 관리자 일일 자동 백업
  setTimeout(()=>crm390CreateBackup(false),3500);
  Object.assign(window,{openCustomerModal,openListingModal,openListingDetail,renderView,uploadListingPhotos,crm390OpenMatches,crm390OpenRestore,crm390RestoreSnapshot,crm390OpenMerge,crm390RunAudit,crm390CreateBackup});
  console.info('CRM v3.9.0 업무 안정화 10종 패키지 적용 완료');
})();

/* ================= CRM v3.9.1 안정화 재점검 패치 ================= */
(()=>{
  const q=s=>document.querySelector(s);
  const waitFor=(test,timeout=2500)=>new Promise(resolve=>{const start=Date.now();const tick=()=>{const v=test();if(v)return resolve(v);if(Date.now()-start>timeout)return resolve(null);requestAnimationFrame(tick)};tick()});

  // 임시저장: '취소/아니요' 선택 시 해당 초안을 즉시 폐기하고 다시 묻지 않음
  const oldConfirm=window.confirm;
  function installDraft391(type,id){
    // 임시저장 질문은 v3.9.0 installDraftAndSummary에서 단 한 번만 처리한다.
    // 여기서는 중복 팝업을 만들지 않고, 손상된 초안만 정리한다.
    const form=q('#modalForm'); if(!form)return;
    const key=`crm390:draft:${state.profile?.id||'anon'}:${type}:${id||'new'}`;
    const saved=localStorage.getItem(key);
    if(saved){try{JSON.parse(saved)}catch(_){localStorage.removeItem(key)}}
  }

  function ensureDetailTools391(type,id){
    const body=q('#modalBody'); if(!body||!id)return;
    let tools=q('#crm390DetailTools');
    if(!tools){tools=document.createElement('section');tools.id='crm390DetailTools';tools.className='crm390-detail-tools';body.prepend(tools)}
    tools.innerHTML=`
      <button type="button" class="primary" onclick="crm390OpenMatches('${type}','${id}')">${type==='customer'?'추천 매물 바로 보기':'맞는 고객 바로 보기'}</button>
      <button type="button" class="ghost" onclick="crm390OpenRestore('${type}','${id}')">변경 이력 복원</button>
      ${type==='customer'?`<button type="button" class="ghost" onclick="crm390OpenMerge('${id}')">중복 고객 합치기</button>`:''}`;
  }

  async function afterModal391(type,id){
    await waitFor(()=>q('#modal')?.open&&q('#modalBody')&&q('#modalForm'));
    installDraft391(type,id);
    if(id)ensureDetailTools391(type,id);
  }

  // v3.9.0 훅을 다시 안전하게 감싸서 비동기 렌더링 뒤에도 도구가 확실히 보이도록 함
  const currentCustomer=window.openCustomerModal;
  window.openCustomerModal=function(id){
    const r=currentCustomer.apply(this,arguments);
    Promise.resolve(r).finally(()=>afterModal391('customer',id));
    setTimeout(()=>afterModal391('customer',id),180);
    setTimeout(()=>afterModal391('customer',id),600);
    return r;
  };
  openCustomerModal=window.openCustomerModal;

  const currentListing=window.openListingModal;
  window.openListingModal=function(id){
    const r=currentListing.apply(this,arguments);
    Promise.resolve(r).finally(()=>afterModal391('listing',id));
    setTimeout(()=>afterModal391('listing',id),180);
    setTimeout(()=>afterModal391('listing',id),600);
    return r;
  };
  openListingModal=window.openListingModal;

  const currentDetail=window.openListingDetail;
  window.openListingDetail=function(id){
    const r=currentDetail.apply(this,arguments);
    Promise.resolve(r).finally(()=>afterModal391('listing',id));
    setTimeout(()=>afterModal391('listing',id),250);
    setTimeout(()=>afterModal391('listing',id),700);
    return r;
  };
  openListingDetail=window.openListingDetail;

  // 데이터 점검에 10개 설치 상태를 직접 확인하는 진단 화면 추가
  window.crm391FeatureCheck=async function(){
    const tests=[
      ['1 자동 저장·임시저장',!!localStorage],
      ['2 저장 전 변경사항 요약',typeof saveSnapshot==='function'||typeof crm390OpenRestore==='function'],
      ['3 정기 자동 백업',typeof crm390CreateBackup==='function'],
      ['4 고객 추천 매물',typeof crm390OpenMatches==='function'],
      ['5 매물 추천 고객',typeof crm390OpenMatches==='function'],
      ['6 중복 고객 감지',true],
      ['7 중복자료 합치기',typeof crm390OpenMerge==='function'],
      ['8 필수정보 누락 점검',typeof crm390RunAudit==='function'],
      ['9 변경 이력 복원',typeof crm390OpenRestore==='function'],
      ['10 사진 업로드 최적화',typeof uploadListingPhotos==='function']
    ];
    const tableChecks=['entity_snapshots','crm_backups'];
    for(const t of tableChecks){const {error}=await state.client.from(t).select('id',{head:true,count:'exact'}).limit(1);tests.push([`DB ${t}`,!error]);}
    q('#modalTitle').textContent='v3.9.1 기능 설치 점검';
    q('#modalBody').innerHTML=`<div class="crm391-check-list">${tests.map(([n,ok])=>`<div class="crm391-check-row"><span>${n}</span><strong class="${ok?'ok':'bad'}">${ok?'정상':'확인 필요'}</strong></div>`).join('')}</div><div class="notice">추천 버튼은 고객명 또는 매물명을 눌러 상세창을 열면 맨 위에 표시됩니다.</div>`;
    q('#modalSubmit').style.display='none';q('#modal').showModal();
  };

  const oldInject=window.crm390InjectAdminTools;
  window.crm390InjectAdminTools=function(){
    oldInject?.();
    if(state.profile?.role!=='admin')return;
    const box=q('#crm390AdminTools');
    if(box&&!q('#crm391CheckBtn'))box.insertAdjacentHTML('beforeend','<button id="crm391CheckBtn" class="ghost" onclick="crm391FeatureCheck()">기능 점검 <small>ADMIN</small></button>');
  };
  crm390InjectAdminTools=window.crm390InjectAdminTools;

  // 임시저장 취소 처리는 v3.9.0 기본 훅에서 직접 수행한다. 전역 confirm 재정의는 사용하지 않는다.


  setTimeout(()=>window.crm390InjectAdminTools?.(),1200);
  console.info('CRM v3.9.2 임시저장 단일질문 및 취소 즉시삭제 완료');
})();

/* CRM v3.9.1 관리자 기능점검 버튼 렌더 보강 */
(()=>{
  const add=()=>{if(state.profile?.role!=='admin')return;const box=document.querySelector('#crm390AdminTools');if(box&&!document.querySelector('#crm391CheckBtn'))box.insertAdjacentHTML('beforeend','<button id="crm391CheckBtn" class="ghost" onclick="crm391FeatureCheck()">기능 점검 <small>ADMIN</small></button>')};
  new MutationObserver(add).observe(document.body,{childList:true,subtree:true});
  setInterval(add,1500);
})();

/* ================= CRM v3.9.3 임시저장·변경요약·백업·점검 실동작 보강 ================= */
(()=>{
  const q=(s,r=document)=>r.querySelector(s), qa=(s,r=document)=>[...r.querySelectorAll(s)];
  const profileId=()=>state.profile?.id||'anon';
  const oldDraftKey=(type,id)=>`crm390:draft:${profileId()}:${type}:${id||'new'}`;
  const draftKey=(type,id)=>`crm393:draft:${profileId()}:${type}:${id||'new'}`;
  ...
  function serialize(form){
    ...
    qa('input,select,textarea',form).forEach(el=>{
      if(el.type==='file'||el.dataset?.changeIgnore==='true')return;
      const k=fieldKey(el);if(!k)return;
      if(el.type==='checkbox'){
        if(!Array.isArray(data[k]))data[k]=[];
        if(el.checked)data[k].push(el.value||'__checked__');
  ...
  function restorePass(form,data){
    qa('input,select,textarea',form).forEach(el=>{
      if(el.type==='file')return;
      const k=fieldKey(el);if(!k||!(k in data))return;
      if(el.type==='checkbox'){
        const arr=Array.isArray(data[k])?data[k]:[data[k]];
        el.checked=arr.map(String).includes(String(el.value||'__checked__'));
      }else if(el.type==='radio')el.checked=String(data[k])===String(el.value);
      else el.value=data[k]??'';
  ...
  async function restoreRepeated(form,data){
    for(const delay of [0,80,220,500]){
      if(delay)await new Promise(r=>setTimeout(r,delay));
      restorePass(form,data);
    }
  }
  function labelFor(el){
    const map={
      crm3894MoveInMode:'입주 희망일 입력방식',
      crm3894MoveInDate:'입주 희망일',
      crm3894MoveInMonth:'입주 희망월',
      crm3894MoveInPart:'입주 희망시기'
    };
    if(map[el.id])return map[el.id];
    const wrap=el.closest('label');
    let txt=wrap?.childNodes?.[0]?.textContent?.trim()||'';
    if(!txt)txt=el.getAttribute('aria-label')||el.placeholder||el.name||el.id||'항목';
    return txt.replace(/\s+/g,' ').slice(0,35);
  }
  function human(v){
    if(Array.isArray(v))return v.length?v.join(', '):'없음';
    return safe(v)||'없음';
  }
  function diff(initial,current,form){
    const keys=new Set([...Object.keys(initial||{}),...Object.keys(current||{})]);
    const els=qa('input,select,textarea',form);
    return [...keys].filter(k=>JSON.stringify(initial?.[k]??'')!==JSON.stringify(current?.[k]??'')).map(k=>{
      const el=els.find(x=>fieldKey(x)===k);
      return {label:el?labelFor(el):k,before:human(initial?.[k]),after:human(current?.[k])};
    });
  }
  function showChangeSummary(changes,onSave){
    q('#crm393ChangeOverlay')?.remove();
    const overlay=document.createElement('div');overlay.id='crm393ChangeOverlay';overlay.className='crm393-change-overlay';
    overlay.innerHTML=`<section class="crm393-change-card"><header><h3>변경사항 확인</h3><button type="button" class="icon-btn" data-close>×</button></header><p class="muted">저장하기 전에 실제 변경된 내용을 확인하세요.</p><div class="crm393-change-list">${changes.slice(0,30).map(x=>`<div><strong>${escapeHtml(x.label)}</strong><span>${escapeHtml(x.before)}</span><b>→</b><span>${escapeHtml(x.after)}</span></div>`).join('')}${changes.length>30?`<p>외 ${changes.length-30}건</p>`:''}</div><footer><button type="button" class="ghost" data-close>취소</button><button type="button" class="primary" data-save>변경 저장</button></footer></section>`;
    document.body.appendChild(overlay);
    overlay.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>overlay.remove());
    overlay.querySelector('[data-save]').onclick=async()=>{const b=overlay.querySelector('[data-save]');b.disabled=true;b.textContent='저장 중...';try{await onSave();overlay.remove()}catch(e){b.disabled=false;b.textContent='변경 저장';toast(e?.message||'저장 준비 중 오류가 발생했습니다.')}};
  }
  async function saveSnapshot393(type,id){
    if(!id)return;
    const src=type==='customer'?state.customers:state.listings;
    const item=(src||[]).find(x=>String(x.id)===String(id));if(!item)return;
    const snapshot={};Object.entries(item).forEach(([k,v])=>{if(v===null||['string','number','boolean'].includes(typeof v)||Array.isArray(v))snapshot[k]=v});
    const {error}=await state.client.from('entity_snapshots').insert({entity_type:type,entity_id:id,snapshot,reason:'수정 전 자동 저장',created_by:state.profile.id});
    if(error)console.warn('snapshot save',error);
  }

  async function installFormSafety(type,id){
    const form=q('#modalForm'),modal=q('#modal'),btn=q('#modalSubmit');if(!form||!modal||!btn)return;
    modalContext={type,id:String(id||''),form,btn,bypass:false};
    const ctx=modalContext;
    // 구버전 초안은 반복 질문 원인이므로 이 모달에서는 사용하지 않는다.
    localStorage.removeItem(oldDraftKey(type,id));
    const key=draftKey(type,id);
    await new Promise(r=>setTimeout(r,120));
    const initial=serialize(form);modalContext.initial=initial;
    const saved=localStorage.getItem(key);
    if(saved){
      try{
        const parsed=JSON.parse(saved);
        if(window.confirm('작성 중이던 임시저장 내용이 있습니다. 이어서 작성할까요?')){
          await restoreRepeated(form,parsed.data||{});
          modalContext.initial=serialize(form);
    ...
    await bindChangeSummary393(ctx);
    let timer;
    const saveDraft=()=>{
      clearTimeout(timer);
      timer=setTimeout(()=>{
        localStorage.removeItem(oldDraftKey(type,id));
        localStorage.setItem(key,JSON.stringify({savedAt:new Date().toISOString(),data:serialize(form)}));
        let tag=q('#crm393DraftTag');if(!tag){tag=document.createElement('small');tag.id='crm393DraftTag';tag.className='crm393-draft-tag';q('#modalTitle')?.appendChild(tag)}tag.textContent=' 임시저장됨';
      },450);
    };
    form.addEventListener('input',saveDraft);form.addEventListener('change',saveDraft);
    modal.addEventListener('close',()=>{clearTimeout(timer);if(modalContext===ctx)modalContext=null;localStorage.removeItem(oldDraftKey(type,id));},{once:true});
  }

  // 저장 전 확인창은 사용하지 않는다. 수정 전 원본만 변경 이력으로 남기고 즉시 저장한다.
  async function bindChangeSummary393(ctx){
    const {type,id,form,btn}=ctx;
    if(!id||!btn||btn.__crm3111DirectSaveBound)return;
    btn.__crm3111DirectSaveBound=true;
    const originalHandler=btn.onclick;
    btn.onclick=async function(e){
      const changes=diff(ctx.initial||{},serialize(form),form);
      if(changes.length){
        await saveSnapshot393(type,id);
      }
      localStorage.removeItem(draftKey(type,id));
      localStorage.removeItem(oldDraftKey(type,id));
      return typeof originalHandler==='function' ? originalHandler.call(this,e) : undefined;
    };
  }

  function wrapModal(name,type){
    const current=window[name];if(typeof current!=='function'||current.__crm393)return;
    const wrapped=function(id){
      localStorage.removeItem(oldDraftKey(type,id));
      const r=current.apply(this,arguments);
      Promise.resolve(r).finally(()=>setTimeout(()=>installFormSafety(type,id),180));
      return r;
    };wrapped.__crm393=true;window[name]=wrapped;try{eval(`${name}=window[name]`)}catch(_){}
  }
  wrapModal('openCustomerModal','customer');
  wrapModal('openListingModal','listing');

  async function createBackup393(manual=true){
    if(state.profile?.role!=='admin')return toast('관리자만 사용할 수 있습니다.');
    const btn=q('#crm393BackupBtn');if(btn){btn.disabled=true;btn.textContent='백업 중...'}
    try{
      await loadCustomers();await loadListings();
      const snapshot={created_at:new Date().toISOString(),customers:state.customers||[],listings:state.listings||[]};
      const tables=['interaction_history','customer_follow_up_schedules','customer_condition_history','customer_listing_recommendations','contract_cases','contract_documents'];
      for(const t of tables){const {data,error}=await state.client.from(t).select('*').limit(10000);if(!error)snapshot[t]=data||[];}
      const date=new Date().toISOString().slice(0,10);
      const {error}=await state.client.from('crm_backups').insert({backup_date:date,backup_type:manual?'manual':'daily',snapshot,created_by:state.profile.id});
      if(error)throw error;
      localStorage.setItem(`crm393:backup:${date}`,'1');
      if(manual){const blob=new Blob([JSON.stringify(snapshot,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`더퍼스트CRM_전체백업_${date}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
      toast(manual?'전체 백업을 저장하고 내려받았습니다.':'오늘 자동 백업을 완료했습니다.');
    }catch(e){toast(`백업 실패: ${e?.message||e}`)}finally{if(btn){btn.disabled=false;btn.innerHTML='전체 백업 <small>ADMIN</small>'}}
  }
  async function runAudit393(){
    if(state.profile?.role!=='admin')return toast('관리자만 사용할 수 있습니다.');
    await loadCustomers();await loadListings();
    const issues=[];
    (state.customers||[]).forEach(c=>{
      const phones=[c.phone,c.contact_phone,...(c.contacts||[]).map(x=>x.phone)].filter(Boolean);
      if(!safe(c.name).trim())issues.push(['고객','고객명 없음',c.id]);
      if(!phones.length)issues.push(['고객',`${c.name||'이름 없음'} · 연락처 없음`,c.id]);
      if(!(c.deal_type||c.transaction_types?.length||c.deal_options?.length))issues.push(['고객',`${c.name||'이름 없음'} · 거래유형 없음`,c.id]);
    });
    (state.listings||[]).forEach(l=>{
      if(!safe(l.address).trim())issues.push(['매물',`${l.title||'매물명 없음'} · 주소 없음`,l.id]);
      if(!safe(l.unit_no||l.unit).trim())issues.push(['매물',`${l.title||'매물명 없음'} · 호수 없음`,l.id]);
      let deals=[];try{deals=crm38DealOptions(l)||[]}catch(_){deals=l.deal_options||[]}
      if(!deals.length&&!l.transaction_type)issues.push(['매물',`${l.title||'매물명 없음'} · 거래조건 없음`,l.id]);
      if(!Number.isFinite(Number(l.latitude))||!Number.isFinite(Number(l.longitude)))issues.push(['매물',`${l.title||'매물명 없음'} · 지도 좌표 없음`,l.id]);
    });
    q('#modalTitle').textContent=`필수정보 누락 점검 · ${issues.length}건`;
    q('#modalBody').innerHTML=issues.length?`<div class="crm393-audit-list">${issues.map(([t,msg,id])=>`<article><span>${badge(t,t==='고객'?'blue':'yellow')} ${escapeHtml(msg)}</span><button type="button" class="ghost" onclick="${t==='고객'?`openCustomerModal('${id}')`:`openListingDetail('${id}')`}">열기</button></article>`).join('')}</div>`:'<div class="empty">필수정보 누락이 없습니다.</div>';
    q('#modalSubmit').style.display='none';q('#modal').showModal();
  }
  function injectAdminUtilities(){
    const nav=q('#navMenu .nav-section.admin-only');
    if(state.profile?.role!=='admin'||!nav)return;
    nav.classList.remove('hidden');
    let box=q('#crm393AdminUtilities');
    if(!box){box=document.createElement('div');box.id='crm393AdminUtilities';box.className='crm393-admin-utilities';box.innerHTML=`<div class="nav-section-label"><span>데이터 안전관리</span><span class="admin-chip">ADMIN</span></div><button id="crm393AuditBtn" type="button" class="nav admin-nav">필수정보 누락 점검</button><button id="crm393BackupBtn" type="button" class="nav admin-nav">전체 백업</button><small id="crm393BackupStatus"></small>`;nav.appendChild(box)}
    q('#crm393AuditBtn').onclick=runAudit393;q('#crm393BackupBtn').onclick=()=>createBackup393(true);
  }
  async function autoBackupWhenReady(){
    for(let i=0;i<30;i++){
      if(state.profile?.role==='admin'&&state.client){
        injectAdminUtilities();const date=new Date().toISOString().slice(0,10);
        if(!localStorage.getItem(`crm393:backup:${date}`))await createBackup393(false);
        return;
      }
      await new Promise(r=>setTimeout(r,500));
    }
  }
  new MutationObserver(injectAdminUtilities).observe(document.body,{childList:true,subtree:true});
  setInterval(injectAdminUtilities,1500);autoBackupWhenReady();
  Object.assign(window,{crm393CreateBackup:createBackup393,crm393RunAudit:runAudit393});
  console.info('CRM v3.9.3 핵심 안정화 기능 실동작 보강 완료');
})();

/* ================= CRM v3.9.4 필수정보 누락 점검 정확도 수정 ================= */
(()=>{
  const q=s=>document.querySelector(s);
  const text=v=>String(v??'').trim();
  const num=v=>{const n=Number(v);return Number.isFinite(n)?n:0};
  const phoneDigits=v=>String(v??'').replace(/\D/g,'');
  const hasPhone=v=>phoneDigits(v).length>=8;
  const truthyArray=v=>Array.isArray(v)?v.filter(Boolean):[];
  const sameId=(a,b)=>String(a)===String(b);

  async function safeSelect(table, columns='*'){
    try{
      const {data,error}=await state.client.from(table).select(columns).limit(20000);
      if(error){console.warn(`[audit] ${table}`,error);return {data:[],error};}
      return {data:data||[],error:null};
    }catch(error){console.warn(`[audit] ${table}`,error);return {data:[],error};}
  }

  function customerSelectedDeals(c, rows){
    const related=rows.filter(r=>sameId(r.customer_id,c.id));
    if(related.length)return related.map(r=>text(r.deal_type)).filter(Boolean);
    return text(c.deal_type||c.transaction_type).split(/[+,·/]/).map(x=>x.trim()).filter(Boolean);
  }

  function listingSelectedDeals(l){
    try{
      const rows=typeof crm38DealOptions==='function'?crm38DealOptions(l):[];
      if(Array.isArray(rows)&&rows.length)return rows.map(r=>text(r.deal_type||r.type)).filter(Boolean);
    }catch(_){ }
    const raw=l.transaction_types||l.deal_types||l.transaction_type||l.deal_type||'';
    if(Array.isArray(raw))return raw.map(text).filter(Boolean);
    return text(raw).split(/[+,·/]/).map(x=>x.trim()).filter(Boolean);
  }

  function listingHasPrice(l,deals){
    const checks={
      '매매':num(l.sale_price||l.price_sale||l.sale_amount||l.price)>0,
      '전세':num(l.jeonse_price||l.price_jeonse||l.deposit_price||l.deposit)>0,
      '월세':num(l.monthly_deposit||l.rent_deposit||l.deposit)>0 || num(l.monthly_rent||l.rent_price||l.rent)>0,
    };
    return deals.length?deals.every(d=>checks[d]!==false):false;
  }

  function customerHasBudget(c,deals,dealRows){
    const rows=dealRows.filter(r=>sameId(r.customer_id,c.id));
    const byType=t=>rows.find(r=>text(r.deal_type)===t);
    return deals.every(t=>{
      const r=byType(t)||{};
      if(t==='매매')return num(r.budget_max||c.sale_budget||c.budget_sale||c.budget_max)>0;
      if(t==='전세')return num(r.budget_max||c.jeonse_budget||c.budget_jeonse||c.budget_max)>0;
      if(t==='월세')return num(r.budget_max||c.monthly_deposit||c.rent_deposit||c.budget_max)>0 || num(r.desired_monthly_rent||c.desired_monthly_rent||c.monthly_rent)>0;
      return true;
    });
  }

  async function runAudit394(){
    if(state.profile?.role!=='admin')return toast('관리자만 사용할 수 있습니다.');
    const btn=q('#crm393AuditBtn');
    if(btn){btn.disabled=true;btn.textContent='점검 중...';}
    try{
      const [customersR,listingsR,cContactsR,cDealsR,lContactsR]=await Promise.all([
        safeSelect('customers'),safeSelect('listings'),safeSelect('customer_contacts'),safeSelect('customer_deal_options'),safeSelect('listing_contacts')
      ]);
      const customers=customersR.data, listings=listingsR.data;
      const cContacts=cContactsR.data, cDeals=cDealsR.data, lContacts=lContactsR.data;
      const issues=[];
      const add=(type,kind,name,id,detail='')=>issues.push({type,kind,name:name||`${type}명 없음`,id,detail});

      customers.forEach(c=>{
        const name=text(c.name||c.customer_name);
        const contacts=cContacts.filter(r=>sameId(r.customer_id,c.id));
        const phones=[c.phone,c.contact_phone,...contacts.map(r=>r.phone)];
        const deals=customerSelectedDeals(c,cDeals);
        if(!name)add('고객','고객명 없음','이름 없는 고객',c.id);
        if(!phones.some(hasPhone))add('고객','연락처 없음',name,c.id,'전화번호 8자리 이상이 없습니다.');
        if(!text(c.customer_type||c.customer_category||c.category))add('고객','고객구분 없음',name,c.id,'매수 또는 임차를 선택하세요.');
        if(!text(c.status))add('고객','상태 없음',name,c.id);
        if(!deals.length)add('고객','거래유형 없음',name,c.id);
        else if(!customerHasBudget(c,deals,cDeals))add('고객','희망금액 없음',name,c.id,`${deals.join('·')} 중 금액이 비어 있습니다.`);
        if(!text(c.preferred_area||c.desired_area||c.region))add('고객','희망지역 없음',name,c.id);
        const rooms=c.desired_rooms??c.room_count??c.preferred_rooms;
        if(rooms===null||rooms===undefined||text(rooms)==='')add('고객','희망 방 개수 없음',name,c.id);
      });

      listings.forEach(l=>{
        const title=text(l.title||l.listing_name||l.name);
        const deals=listingSelectedDeals(l);
        const contacts=lContacts.filter(r=>sameId(r.listing_id,l.id));
        const phones=[l.owner_phone,l.phone,l.contact_phone,...contacts.map(r=>r.phone)];
        if(!title)add('매물','매물명 없음','이름 없는 매물',l.id);
        if(!text(l.listing_type||l.property_type||l.type))add('매물','매물유형 없음',title,l.id);
        if(!text(l.address||l.address_jibun||l.address_main))add('매물','주소 없음',title,l.id);
        if(!text(l.district||l.region))add('매물','지역 없음',title,l.id);
        if(!text(l.unit_no||l.unit||l.room_no))add('매물','호수 없음',title,l.id);
        if(!deals.length)add('매물','거래유형 없음',title,l.id);
        else if(!listingHasPrice(l,deals))add('매물','거래금액 없음',title,l.id,`${deals.join('·')} 중 금액이 비어 있습니다.`);
        if(!text(l.status))add('매물','상태 없음',title,l.id);
        if(!phones.some(hasPhone))add('매물','연락처 없음',title,l.id);
        if(!Number.isFinite(Number(l.latitude))||!Number.isFinite(Number(l.longitude))||num(l.latitude)===0||num(l.longitude)===0)add('매물','지도 좌표 없음',title,l.id,'카카오 주소 검색으로 주소를 다시 선택하세요.');
      });

      const grouped=issues.reduce((acc,x)=>{(acc[x.type]??=[]).push(x);return acc;},{});
      q('#modalTitle').textContent=`필수정보 누락 점검 · ${issues.length}건`;
      q('#modalBody').innerHTML=issues.length?`
        <div class="notice">고객 ${customers.length}명, 매물 ${listings.length}건을 데이터베이스에서 직접 다시 점검했습니다.</div>
        ${['고객','매물'].filter(t=>grouped[t]?.length).map(t=>`<section class="crm394-audit-section"><h3>${t} · ${grouped[t].length}건</h3>${grouped[t].map(x=>`<article class="crm394-audit-row"><div><div>${badge(x.kind,t==='고객'?'blue':'yellow')} <strong>${escapeHtml(x.name)}</strong></div>${x.detail?`<small class="muted">${escapeHtml(x.detail)}</small>`:''}</div><button type="button" class="ghost" onclick="${t==='고객'?`openCustomerModal('${x.id}')`:`openListingDetail('${x.id}')`}">열기</button></article>`).join('')}</section>`).join('')}
      `:'<div class="empty">현재 점검 기준으로 필수정보 누락이 없습니다.</div>';
      q('#modalSubmit').style.display='none';
      q('#modal').showModal();
    }catch(e){console.error(e);toast(`누락 점검 실패: ${e?.message||e}`)}
    finally{if(btn){btn.disabled=false;btn.textContent='필수정보 누락 점검';}}
  }

  function rebindAuditButton(){
    const btn=q('#crm393AuditBtn');
    if(btn&&btn.dataset.crm394!=='1'){
      btn.dataset.crm394='1';
      btn.onclick=runAudit394;
    }
  }
  new MutationObserver(rebindAuditButton).observe(document.body,{childList:true,subtree:true});
  setInterval(rebindAuditButton,1000);
  Object.assign(window,{crm393RunAudit:runAudit394,crm394RunAudit:runAudit394});
  console.info('CRM v3.9.4 필수정보 누락 점검 정확도 수정 적용 완료');
})();

/* CRM v3.9.5 변경사항 저장 흐름 안정화 */
console.info('CRM v3.9.5 변경사항 확인 저장 버튼 및 null bypass 오류 수정 완료');

/* ================= CRM v3.9.9 성능 안정화: 정렬·스크롤·사진표시 ================= */
(()=>{
  const q=(s,r=document)=>r.querySelector(s);
  const qa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const collator=new Intl.Collator('ko-KR',{numeric:true,sensitivity:'base'});
  const tableState=new WeakMap();
  const scrollPairs=new Set();
  ...
  function parseSortable(text){
    ...
    if(date)return {empty:false,type:'number',value:Date.UTC(+date[1],+date[2]-1,+date[3])};
    ...
    const num=normalized.match(/^-?\d+(?:\.\d+)?(?:만원|만|㎡|평|개|장)?$/);
    if(num)return {empty:false,type:'number',value:Number(normalized.match(/-?\d+(?:\.\d+)?/)[0])};
  ...
  function compareValues(a,b,dir){
    ...
    const result=(av.type==='number'&&bv.type==='number')?av.value-bv.value:collator.compare(String(av.value),String(bv.value));
    return dir==='asc'?result:-result;
  }

  function logicalGroups(table){
    const rows=qa('tbody > tr',table),groups=[];
    for(let i=0;i<rows.length;i++){
      const row=rows[i];
      if(row.classList.contains('crm3813-address-row')&&rows[i+1]){groups.push({nodes:[row,rows[i+1]],dataRow:rows[i+1],index:groups.length});i++;}
  ...
  function sortTable(table,col,mode){
    const state=tableState.get(table),body=q('tbody',table);if(!state||!body)return;
    ...
    const groups=mode==='none'?state.groups.slice().sort((a,b)=>a.index-b.index):state.groups.slice().sort((a,b)=>{
      const result=compareValues(a.dataRow.children[col]?.innerText,b.dataRow.children[col]?.innerText,mode);
      return result||a.index-b.index;
    });
    const frag=document.createDocumentFragment();groups.forEach(g=>g.nodes.forEach(n=>frag.appendChild(n)));body.appendChild(frag);
  }

  function installTableSorting(table){
    if(table.dataset.crm399Sort==='1')return;
    const headers=qa('thead th',table);if(!headers.length)return;
    table.dataset.crm399Sort='1';
    const state={groups:logicalGroups(table),column:-1,mode:'none'};tableState.set(table,state);
    headers.forEach((th,col)=>{
      const label=th.textContent.trim();if(!label||/선택|관리/.test(label))return;
      th.classList.add('crm396-sortable-th');
      if(!q('.crm396-sort-icon',th)){const icon=document.createElement('span');icon.className='crm396-sort-icon';icon.textContent='↕';icon.setAttribute('aria-hidden','true');th.appendChild(icon);}
  ...
  function syncScrollWidth(pair){
    const {top,inner,wrapper,table}=pair;
    if(!top.isConnected||!wrapper.isConnected){scrollPairs.delete(pair);return;}
    const width=Math.max(table.scrollWidth,wrapper.clientWidth);
    inner.style.width=`${width}px`;top.style.display=table.scrollWidth>wrapper.clientWidth?'block':'none';
  ...
  function addTopScrollbar(wrapper,table){
    ...
    wrapper.parentElement?.querySelectorAll('.crm3889-top-scroll,.crm3890-top-scroll,.crm396-top-scroll,.crm3106-network-top-scroll,.crm3112-network-top-scroll').forEach(el=>el.remove());
    if(!wrapper.dataset.crm3113ShiftWheel){
      wrapper.dataset.crm3113ShiftWheel='1';
      wrapper.addEventListener('wheel',e=>{if(!e.shiftKey)return;const amount=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;if(!amount)return;e.preventDefault();wrapper.scrollLeft+=amount},{passive:false});
    }
  }

  function installScroll(table){
    const wrapper=table.closest('.customer-table-scroll,.crm3890-listing-scroll,.table-wrap');if(wrapper)addTopScrollbar(wrapper,table);
  }

  async function loadPhotoIds(force=false){
    if(photoLoaded&&!force){applyPhotoStyles();return;}
    if(photoLoadPromise)return photoLoadPromise;
    photoLoadPromise=(async()=>{
      try{
        if(!state?.client)return;
        const {data,error}=await state.client.from('listing_photos').select('listing_id').limit(20000);if(error)throw error;
        photoIds=new Set((data||[]).map(x=>String(x.listing_id)));photoLoaded=true;applyPhotoStyles();
      }catch(e){console.warn('[crm399] 사진 존재여부 조회 실패',e)}finally{photoLoadPromise=null;}
    })();return photoLoadPromise;
  }

  function applyPhotoStyles(root=document){
    qa('button.photo-link',root).forEach(btn=>{
      const m=(btn.getAttribute('onclick')||'').match(/openListingPhotos\(['\"]([^'\"]+)/);if(!m)return;
      const has=photoIds.has(String(m[1]));btn.classList.toggle('crm396-has-photos',has);btn.title=has?'등록된 내부사진이 있습니다.':'등록된 내부사진이 없습니다.';
    });
  }
  ...
  function patchRoot(root){
    ...
    const tables=[];
    if(root.matches?.('table.customer-table,table.listing-table'))tables.push(root);
    tables.push(...qa('table.customer-table,table.listing-table',root));
    tables.forEach(t=>{installTableSorting(t);installScroll(t)});
    applyPhotoStyles(root);
    // 로그인 후 매물표가 실제로 렌더링되는 시점에 사진 존재 목록을 다시 불러옵니다.
    // 초기 앱 실행 시 인증 전 조회가 건너뛰어진 경우에도 내부사진 초록 표시가 적용됩니다.
    if(tables.some(t=>t.classList.contains('listing-table'))){
      loadPhotoIds(false);
    }
  }

  function schedulePatch(nodes){
    cancelAnimationFrame(pendingFrame);
    pendingFrame=requestAnimationFrame(()=>{nodes.forEach(patchRoot);pendingFrame=0;});
  }

  patchRoot(document);loadPhotoIds(true);
  const observer=new MutationObserver(records=>{
    const added=[];for(const r of records)for(const n of r.addedNodes)if(n.nodeType===1)added.push(n);
    if(added.length)schedulePatch(added);
  });
  const host=document.getElementById('main')||document.body;observer.observe(host,{childList:true,subtree:true});
  window.addEventListener('resize',()=>{for(const pair of scrollPairs)syncScrollWidth(pair)},{passive:true});

  const originalAdd=window.addListingPhotos;
  if(typeof originalAdd==='function')window.addListingPhotos=async function(){const r=await originalAdd.apply(this,arguments);photoLoaded=false;setTimeout(()=>loadPhotoIds(true),300);return r};
  const originalDelete=window.deleteListingPhoto;
  if(typeof originalDelete==='function')window.deleteListingPhoto=async function(){const r=await originalDelete.apply(this,arguments);photoLoaded=false;setTimeout(()=>loadPhotoIds(true),300);return r};

  console.info('CRM v3.10.2 내부사진 존재 표시 재조회 보완 완료');
})();

/* ===== CRM v3.9.8 희망 매물유형 다중선택 · 자동매칭 개편 ===== */
(function(){
  const CRM398_TYPES=['아파트','오피스텔','단독','다가구','다세대','상가','사무실','토지','기타'];
  const CRM398_HOME=['아파트','오피스텔','단독','다가구','다세대'];
  const CRM398_OTHER=['상가','사무실','토지','기타'];
  function crm398PropertyTypes(){return [...CRM398_TYPES]}
  function crm398SelectedPropertyTypes(customer){
    const raw=customer?.preferred_property_types;
    if(Array.isArray(raw))return raw.filter(v=>CRM398_TYPES.includes(v));
    if(typeof raw==='string'){
      try{const parsed=JSON.parse(raw);if(Array.isArray(parsed))return parsed.filter(v=>CRM398_TYPES.includes(v))}catch(_){ }
      return raw.split(/[,+;]/).map(v=>v.trim()).filter(v=>CRM398_TYPES.includes(v));
    }
    const old=String(customer?.preferred_property_group||'').trim();
    if(old==='주택')return [...CRM398_HOME];
    if(old==='기타')return [...CRM398_OTHER];
    return [];
  }
  function crm398ReadPreferredTypes(){return [...document.querySelectorAll('.crm398-property-type-check:checked')].map(x=>x.value)}
  function crm398ToggleHomeAll(checked){document.querySelectorAll('.crm398-property-type-check').forEach(x=>{if(CRM398_HOME.includes(x.value))x.checked=checked})}
  function crm398SyncHomeAll(){const box=document.getElementById('crm398HomeAll');if(box)box.checked=CRM398_HOME.every(t=>document.querySelector(`.crm398-property-type-check[value="${t}"]`)?.checked)}
  function crm398TypeText(customer){const rows=crm398SelectedPropertyTypes(customer);return rows.length?rows.join(' · '):'전체 유형'}
  function customerOptionMap(customer){const rows=typeof crm3857DealOptions==='function'?crm3857DealOptions(customer):[];return new Map(rows.map(o=>[o.deal_type,o]))}
  function listingOptions(listing){return typeof crm38DealOptions==='function'?crm38DealOptions(listing):[{deal_type:listing.transaction_type,price:listing.price,monthly_rent:listing.monthly_rent,is_preferred:true}]}
  function cleanNumber(v){const n=Number(v);return Number.isFinite(n)?n:0}
  function customerBudgetSummary(customer){const rows=typeof crm3857DealOptions==='function'?crm3857DealOptions(customer):[];return rows.length?rows.map(o=>`${o.deal_type} ${fmtMoney(o.budget_max||0)}`).join(' · '):fmtMoney(customer?.budget_max||0)}

  // 현재 활성 고객창이 열린 뒤 주택전체 체크 상태를 맞춘다.
  const baseOpen=window.openCustomerModal||openCustomerModal;
  window.openCustomerModal=function(){const r=baseOpen.apply(this,arguments);setTimeout(()=>{document.querySelector('.crm397-property-group-field')?.remove();crm398SyncHomeAll()},30);return r};
  try{openCustomerModal=window.openCustomerModal}catch(_){ }

  window.evaluateListingMatch=function(customer,listing){
    const reasons=[];
    const selected=crm398SelectedPropertyTypes(customer);
    const listingType=String(listing?.property_type||'').trim()==='빌라'?'다세대':String(listing?.property_type||'').trim()==='원룸'?'기타':String(listing?.property_type||'').trim();
    if(selected.length&&!selected.includes(listingType))return {matched:false,reasons:[`매물유형 불일치: 희망 ${selected.join(', ')} / 매물 ${listingType||'-'}`]};
    reasons.push(selected.length?`매물유형 일치: ${listingType}`:'매물유형 전체 허용');

    const wantedRooms=typeof customerRoomValue==='function'?customerRoomValue(customer):cleanNumber(customer?.desired_rooms);
    const actualRooms=typeof listingRoomValue==='function'?listingRoomValue(listing):cleanNumber(listing?.room_count);
    if(wantedRooms&&actualRooms<wantedRooms)return {matched:false,reasons:[`방 개수 부족: 희망 ${customerRoomText(customer)} 이상 / 매물 ${listingRoomText(listing)}`]};
    if(wantedRooms)reasons.push(`방 개수 충족: 희망 ${customerRoomText(customer)} 이상 / 매물 ${listingRoomText(listing)}`);

    const customerOpts=customerOptionMap(customer),candidates=[];
    for(const opt of listingOptions(listing)){
      const cOpt=customerOpts.get(opt.deal_type);if(!cOpt)continue;
      const budget=cleanNumber(cOpt.budget_max??customer?.budget_max),amount=cleanNumber(opt.price),ceiling=budget?budget+1000:0;
      if(ceiling&&amount>ceiling)continue;
      const local=[...reasons,`거래유형 일치: ${opt.deal_type}`];
      if(budget)local.push(`${opt.deal_type==='월세'?'보증금':'금액'} ${fmtMoney(amount)} / 희망 ${fmtMoney(budget)} · 허용 ${fmtMoney(ceiling)} 이하`);
      if(opt.deal_type==='월세'&&cleanNumber(opt.monthly_rent))local.push(`월세 ${fmtMoney(opt.monthly_rent)} · 월차임 참고`);
      const wanted=typeof crm3864CustomerFeatures==='function'?crm3864CustomerFeatures(customer):[];
      if(wanted.length){const tags=new Set(typeof crm36Array==='function'?crm36Array(listing.feature_tags):[]),missing=wanted.filter(t=>!tags.has(t));if(missing.length)continue;local.push(`희망 특징 충족: ${wanted.join(', ')}`)}
      candidates.push({matched:true,category:'추천',matchKind:'direct',reasons:local,sortValue:(budget?Math.abs(amount-budget):amount)+Math.max(0,actualRooms-wantedRooms)*100,matchedDealType:opt.deal_type,score:100-(budget?Math.min(50,Math.abs(amount-budget)/100):0)-Math.max(0,actualRooms-wantedRooms)});
    }
    if(!candidates.length)return {matched:false,reasons:[customerOpts.size?'거래유형 또는 금액 조건 불일치 · 희망금액 +1,000만원까지 허용':'고객 거래유형 미입력']};
    candidates.sort((a,b)=>(a.sortValue||0)-(b.sortValue||0));return candidates[0];
  };
  try{evaluateListingMatch=window.evaluateListingMatch}catch(_){ }

  const baseRender=window.renderSmartMatch||renderSmartMatch;
  window.renderSmartMatch=async function(){
    await baseRender.apply(this,arguments);
    const notice=document.querySelector('#content .notice');
    if(notice)notice.innerHTML=`<strong>추천 기준</strong><br>① 고객이 체크한 <b>희망 매물유형</b>만 비교합니다. 주택 전체를 누르면 아파트·오피스텔·단독·다가구·다세대가 함께 선택됩니다. 아무 유형도 선택하지 않으면 전체 유형을 허용합니다. ② 거래유형이 같은 매물을 비교합니다. ③ 보증금·희망금액보다 <b>1,000만원 높은 금액까지</b> 포함합니다. ④ 방 개수는 희망 방 개수 <b>이상</b>을 모두 포함합니다.`;
    const select=document.querySelector('#matchCustomer');
    if(select){const current=select.value;select.innerHTML='<option value="">고객 선택</option>'+((state.customers||[]).filter(x=>['매수','임차'].includes(x.customer_type)).map(x=>`<option value="${x.id}">${escapeHtml(x.name)} · ${escapeHtml(crm398TypeText(x))} · ${escapeHtml(typeof crm3857DealText==='function'?crm3857DealText(x):(x.deal_type||''))} · 방 ${customerRoomText(x)} · ${escapeHtml(customerBudgetSummary(x))}</option>`).join(''));select.value=current}
    const empty=document.querySelector('#matchResults.empty');if(empty)empty.textContent='고객을 선택하면 희망 매물유형·거래유형·보증금(희망금액)·방 개수 기준으로 공개 매물을 추천합니다.';
  };
  try{renderSmartMatch=window.renderSmartMatch}catch(_){ }
...
/* ===== CRM v3.10.3 영문 동·호 표준화 및 중복판정 통합 ===== */
(function(){
  function cleanAlphaNumeric(value){
    return String(value??'').trim().toUpperCase().replace(/동|호수|호/g,'').replace(/[^0-9A-Z]/g,'').slice(0,12);
  }

  // 숫자뿐 아니라 영문 동·호수도 화면/저장값을 같은 형식으로 통일한다.
  // A -> A동, B1 -> B1호
  crm3852NormalizeBuilding=function(value){
    const raw=cleanAlphaNumeric(value);
    return `${raw||'1'}동`;
  };
  crm3852NormalizeUnit=function(value){
    const raw=cleanAlphaNumeric(value);
    return raw?`${raw}호`:'';
  };

  // 중복 판정키는 접미사를 제외하고 대문자 기준으로 비교한다.
  crm3863AlphaNumericKey=function(value,fallback=''){
    const raw=cleanAlphaNumeric(value);
    if(!raw)return fallback;
    if(/^\d+$/.test(raw))return raw.replace(/^0+(?=\d)/,'')||'0';
    return raw;
  };
  crm3863BuildingKey=function(value){
    const key=crm3863AlphaNumericKey(value,'1');
    if(/^10[1-9]$/.test(key))return String(Number(key)-100);
    return key;
  };
  crm3863UnitKey=function(value){return crm3863AlphaNumericKey(value,'')};

  // 구조화 주소 비교도 새 표준화 함수를 사용한다.
  crm3855CanonicalInput=function(address,building,unit){
    const p=crm3855ParseLotAddress(address);
    return {...p,building_key:crm3863BuildingKey(building),unit_key:crm3863UnitKey(unit)};
  };
  ...
  function refreshAddressHelp(){
    const row=document.querySelector('#modalBody .crm3852-address-row');
    const help=row?.querySelector('.crm3852-address-help');
    if(help)help.textContent='주소는 검색 결과에서 선택하고, 동·호수는 숫자 또는 영문만 입력하세요. 101은 101동, A는 A동, 602는 602호로 저장되며 같은 표기는 중복검사에 동일하게 적용됩니다.';
    const building=row?.querySelector('[name="building_no"]');
    const unit=row?.querySelector('[name="unit_no"]');
    if(building)building.placeholder='예: 101 또는 A';
    if(unit)unit.placeholder='예: 602 또는 B1';
  }

  const baseOpen=window.openListingModal||globalThis.openListingModal;
  if(typeof baseOpen==='function'&&!baseOpen.__crm3103){
    const wrapped=function(){
      const result=baseOpen.apply(this,arguments);
      setTimeout(refreshAddressHelp,80);
      return result;
    };
    wrapped.__crm3103=true;
...
/* ===== CRM v3.10.5 고객 목록 희망유형 범주 + 주택 세부종류 표시 ===== */
(function(){
  const HOME_TYPE_ORDER = ['아파트','오피스텔','단독','다가구','다세대'];
  const HOME_TYPES = new Set([...HOME_TYPE_ORDER,'빌라','원룸']);
  const CATEGORY_ORDER = ['주택','상가','사무실','토지','기타'];
  const HOME_LABELS = {아파트:'아파트',오피스텔:'오피',단독:'단독',다가구:'다가구',다세대:'다세대',빌라:'다세대',원룸:'다가구'};
  ...
  function preferredCategoryInfo(customer){
    const types = parsePreferredTypes(customer);
    if(!types.length) return {categories:['전체'], homeTypes:[]};
    const categories = new Set();
    const homeTypes = new Set();
    for(const type of types){
      if(HOME_TYPES.has(type)){
        categories.add('주택');
        homeTypes.add(HOME_LABELS[type] || type);
    ...
    const orderedHomes = HOME_TYPE_ORDER.map(v=>HOME_LABELS[v]).filter((v,i,a)=>a.indexOf(v)===i && homeTypes.has(v));
    for(const v of homeTypes) if(!orderedHomes.includes(v)) orderedHomes.push(v);
    return {
      categories:CATEGORY_ORDER.filter(v=>categories.has(v)),
      homeTypes:orderedHomes
    };
  }
  ...
  function applyCustomerPreferredCategories(){
    ...
    const index = headers.findIndex(th=>th.textContent.trim()==='구분' || th.textContent.trim()==='희망' || th.dataset.crm3105Preferred==='1');
    if(index < 0) return;
    const header = headers[index];
    header.textContent='희망';
    header.dataset.crm3105Preferred='1';
    header.title='희망 매물유형 범주와 주택 세부종류';
    ...
    table.querySelectorAll('tbody tr').forEach(row=>{
      ...
      const info = preferredCategoryInfo(customer);
      const categoryText = info.categories.join(' · ') || '전체';
      const homeText = info.homeTypes.join(', ');
      const categoryLines = info.categories.length > 1
        ? info.categories.map(v=>`<span>${escapeHtml(v)}</span>`).join('')
        : `<span>${escapeHtml(categoryText)}</span>`;
      const homeLines = info.homeTypes.map(v=>`<span>${escapeHtml(v)}</span>`).join('');
      const title = homeText ? `${categoryText} / ${homeText}` : categoryText;
      cell.innerHTML = `<div class="crm3105-preferred-wrap" title="${escapeHtml(title)}"><div class="crm3105-preferred-category">${categoryLines}</div>${homeLines?`<div class="crm3105-preferred-home">${homeLines}</div>`:''}</div>`;
    });
  }
  ...
  window.crm3105ApplyCustomerPreferredCategories = applyCustomerPreferredCategories;
  document.addEventListener('DOMContentLoaded',()=>setTimeout(applyCustomerPreferredCategories,300));
  console.info('CRM v3.10.5 고객 목록 희망유형 범주와 주택 세부종류 표시 적용 완료');
})();


/* ===== CRM v3.10.6 희망유형 간결표시 · 사진표시 확실화 · 공동매물 스크롤바 ===== */
(function(){
  const HOME_TYPES = ['아파트','오피스텔','단독','다가구','다세대'];
  const HOME_SET = new Set(HOME_TYPES);
  const HOME_LABEL = {아파트:'아파트',오피스텔:'오피',단독:'단독',다가구:'다가구',다세대:'다세대'};
  const CATEGORY_ORDER = ['주택','상가','사무실','토지','기타'];

  function parseTypes(customer){
    const raw=customer?.preferred_property_types;
    if(Array.isArray(raw))return raw.map(String).map(v=>v.trim()).filter(Boolean);
    if(typeof raw==='string'){
      const text=raw.trim();if(!text)return [];
      try{const parsed=JSON.parse(text);if(Array.isArray(parsed))return parsed.map(String).map(v=>v.trim()).filter(Boolean)}catch(_){ }
      return text.split(/[,+;|]/).map(v=>v.trim()).filter(Boolean);
    }
    return [];
  }

  function pairLines(items){
    const lines=[];
  ...
  function rowCustomerId(row){
    const el=row.querySelector("[onclick*='openCustomerModal'],[onclick*='openCustomerDetail']");
    const code=el?.getAttribute('onclick')||'';
    return code.match(/(?:openCustomerModal|openCustomerDetail)\(['\"]([^'\"]+)['\"]/)?.[1]||'';
  }

  function renderPreferredCell(customer){
    const types=parseTypes(customer);
    if(!types.length)return {html:'<div class="crm3106-pref-main">전체</div>',title:'전체'};
    const selectedHomes=HOME_TYPES.filter(v=>types.includes(v));
    const homeAll=selectedHomes.length===HOME_TYPES.length;
    const categories=[];
    if(selectedHomes.length)categories.push('주택');
    for(const c of ['상가','사무실','토지','기타'])if(types.includes(c))categories.push(c);

    const categoryLines=pairLines(CATEGORY_ORDER.filter(v=>categories.includes(v)));
    let html=categoryLines.map(line=>`<div class="crm3106-pref-main">${line.map(escapeHtml).join(' · ')}</div>`).join('');

    // 주택 전체가 선택된 경우에는 세부종류를 절대 표시하지 않는다.
    // 개별 주택유형을 선택한 경우에만 한 줄에 최대 두 단어씩 표시한다.
    if(selectedHomes.length && !homeAll){
      const details=selectedHomes.map(v=>HOME_LABEL[v]||v);
      html+=`<div class="crm3106-pref-detail">${pairLines(details).map(line=>`<div>${line.map(escapeHtml).join(' · ')}</div>`).join('')}</div>`;
    }
    return {html,title:types.join(', ')};
  }

  function applyPreferredDisplay(){
    const table=document.querySelector('#customerTable table');if(!table)return;
    const headers=Array.from(table.querySelectorAll('thead th'));
    const idx=headers.findIndex(th=>['구분','희망'].includes(th.textContent.trim())||th.dataset.crm3106Preferred==='1'||th.dataset.crm3105Preferred==='1');
    if(idx<0)return;
    headers[idx].textContent='희망';headers[idx].dataset.crm3106Preferred='1';
    table.querySelectorAll('tbody tr').forEach(row=>{
      const cell=row.children[idx];if(!cell)return;
      const id=rowCustomerId(row);
      const customer=state.customers?.find(x=>String(x.id)===String(id));if(!customer)return;
      const out=renderPreferredCell(customer);
      cell.innerHTML=`<div class="crm3106-preferred" title="${escapeHtml(out.title)}">${out.html}</div>`;
    });
  }

  // 이전 v3.10.5 후처리보다 마지막에 실행해 최종 표시를 보장한다.
  const oldFilter=window.filterCustomers||globalThis.filterCustomers;
  if(typeof oldFilter==='function'&&!oldFilter.__crm3106){
    const wrapped=function(){const r=oldFilter.apply(this,arguments);requestAnimationFrame(applyPreferredDisplay);setTimeout(applyPreferredDisplay,80);return r};
    wrapped.__crm3106=true;window.filterCustomers=wrapped;try{filterCustomers=wrapped}catch(_){ }
  }
  const oldRender=window.renderCustomers||globalThis.renderCustomers;
  if(typeof oldRender==='function'&&!oldRender.__crm3106){
    const wrapped=async function(){const r=await oldRender.apply(this,arguments);requestAnimationFrame(applyPreferredDisplay);setTimeout(applyPreferredDisplay,80);return r};
    wrapped.__crm3106=true;window.renderCustomers=wrapped;try{renderCustomers=wrapped}catch(_){ }
  }

  // 사진 존재여부는 로그인 사용자별로 다시 불러오며, 빈 초기조회 캐시를 재사용하지 않는다.
  let photoSet=new Set(),photoUser='',photoLoading=null;
  function currentUserKey(){return String(state?.profile?.id||state?.session?.user?.id||'')}
  function listingIdFromPhotoButton(btn){
    const code=btn.getAttribute('onclick')||'';
    return code.match(/openListingPhotos\(['\"]([^'\"]+)['\"]/)?.[1]||'';
  }
  function paintPhotoButtons(root=document){
    root.querySelectorAll?.('button.photo-link').forEach(btn=>{
      const id=listingIdFromPhotoButton(btn);if(!id)return;
      const has=photoSet.has(String(id));
      btn.classList.toggle('crm3106-has-photos',has);
      btn.classList.toggle('crm396-has-photos',has);
      btn.dataset.hasPhotos=has?'1':'0';
      btn.title=has?'등록된 내부사진이 있습니다.':'등록된 내부사진이 없습니다.';
    });
  }
  async function reloadPhotoSet(force=false){
    const user=currentUserKey();
    if(!user||!state?.client)return;
    if(!force&&photoUser===user&&photoSet.size){paintPhotoButtons();return;}
    if(photoLoading)return photoLoading;
    photoLoading=(async()=>{
      try{
        // 페이지 제한에 걸리지 않도록 범위 조회를 반복한다.
        const ids=[];let from=0;const size=1000;
        while(true){
          const {data,error}=await state.client.from('listing_photos').select('listing_id').range(from,from+size-1);
          if(error)throw error;
          ids.push(...(data||[]).map(x=>String(x.listing_id)));
          if(!data||data.length<size)break;
          from+=size;
          if(from>=50000)break;
        }
        photoSet=new Set(ids);photoUser=user;paintPhotoButtons();
      }catch(e){console.warn('[crm3106] 내부사진 표시 조회 실패',e)}finally{photoLoading=null}
    })();
    return photoLoading;
  }

  // 공동매물망에는 항상 표 바로 위 드래그바를 만들고 Shift+휠 좌우 이동을 보장한다.
  function ensureNetworkScrollbar(){
    const host=document.querySelector('#networkTable');
    if(!host)return;
    host.querySelectorAll('.crm3889-top-scroll,.crm3890-top-scroll,.crm396-top-scroll,.crm3106-network-top-scroll,.crm3112-network-top-scroll').forEach(el=>el.remove());
    const table=host.querySelector('table.listing-table');
    const wrap=table?.closest('.table-wrap,.crm3890-listing-scroll')||table?.parentElement;
    if(!wrap)return;
    wrap.classList.add('crm3106-network-scroll');
  ...
  const oldNetwork=window.renderNetwork||globalThis.renderNetwork;
  if(typeof oldNetwork==='function'&&!oldNetwork.__crm3106){
    const wrapped=async function(){const r=await oldNetwork.apply(this,arguments);requestAnimationFrame(()=>{ensureNetworkScrollbar();reloadPhotoSet(false);paintPhotoButtons()});setTimeout(()=>{ensureNetworkScrollbar();paintPhotoButtons()},180);return r};
    wrapped.__crm3106=true;window.renderNetwork=wrapped;try{renderNetwork=wrapped}catch(_){ }
  }
  const oldMy=window.renderMyListings||globalThis.renderMyListings;
  if(typeof oldMy==='function'&&!oldMy.__crm3106){
    const wrapped=async function(){const r=await oldMy.apply(this,arguments);requestAnimationFrame(()=>{reloadPhotoSet(false);paintPhotoButtons()});return r};
    wrapped.__crm3106=true;window.renderMyListings=wrapped;try{renderMyListings=wrapped}catch(_){ }
  }

  // 사진 추가/삭제 뒤 즉시 색상 갱신
  for(const name of ['addListingPhotos','deleteListingPhoto']){
    const fn=window[name]||globalThis[name];if(typeof fn!=='function'||fn.__crm3106)continue;
    const wrapped=async function(){const r=await fn.apply(this,arguments);await reloadPhotoSet(true);paintPhotoButtons();return r};
    wrapped.__crm3106=true;window[name]=wrapped;
    try{if(name==='addListingPhotos')addListingPhotos=wrapped;else deleteListingPhoto=wrapped}catch(_){ }
  }

  const obs=new MutationObserver(records=>{
    let needPhoto=false,needCustomer=false,needNetwork=false;
    for(const rec of records)for(const n of rec.addedNodes){if(n.nodeType!==1)continue;needPhoto ||= n.matches?.('.photo-link,table.listing-table')||!!n.querySelector?.('.photo-link,table.listing-table');needCustomer ||= n.matches?.('#customerTable,table.customer-table')||!!n.querySelector?.('#customerTable,table.customer-table');needNetwork ||= n.matches?.('#networkTable,table.listing-table')||!!n.querySelector?.('#networkTable table.listing-table');}
    if(needPhoto){reloadPhotoSet(false);requestAnimationFrame(()=>paintPhotoButtons())}
    if(needCustomer)requestAnimationFrame(applyPreferredDisplay);
    if(needNetwork)requestAnimationFrame(ensureNetworkScrollbar);
  });
  obs.observe(document.getElementById('main')||document.body,{childList:true,subtree:true});
  window.addEventListener('resize',ensureNetworkScrollbar,{passive:true});
  setTimeout(()=>{applyPreferredDisplay();reloadPhotoSet(true);ensureNetworkScrollbar()},500);
  console.info('CRM v3.10.6 희망유형 간결표시·사진색상·공동매물 드래그바 적용 완료');
})();

/* ===== CRM v3.10.7 FU 카카오톡 소개문구 즉시복사 ===== */
(function(){
  function crm3107ListingIntroText(item){
    const deals=(typeof crm38DealOptions==='function'?crm38DealOptions(item):[]);
    const dealLines=deals.length
      ? deals.map(o=>{
          if(o.deal_type==='월세') return `월세 ${Number(o.price||0).toLocaleString()}만원 / ${Number(o.monthly_rent||0).toLocaleString()}만원`;
          return `${o.deal_type} ${Number(o.price||0).toLocaleString()}만원`;
        })
      : [typeof listingPriceText==='function'?String(listingPriceText(item)).replace(/<br\s*\/?>/gi,' / ').replace(/<[^>]+>/g,''):'' ].filter(Boolean);
    ...
    const lines=[
      `🏠 ${item.title||'매물 안내'}`,
      location?`📍 ${location}`:'',
      item.property_type?`유형: ${item.property_type}`:'',
      dealLines.length?`조건: ${dealLines.join(' / ')}`:'',
      `전용면적: ${area}`,
      `방/욕실: ${room} / ${item.bathroom_count??'-'}`,
      `입주: ${move}`,
      features.length?`특징: ${features.join(' · ')}`:'',
      '',
      '※ 매물은 실시간으로 계약되거나 조건이 변경될 수 있어 방문 전 다시 확인해드리겠습니다.',
      state?.profile?.full_name?`담당 중개사: ${state.profile.full_name}`:'',
      state?.profile?.office_name?`소속: ${state.profile.office_name}`:'',
      state?.profile?.phone?`연락처: ${typeof crm381FormatPhone==='function'?crm381FormatPhone(state.profile.phone):state.profile.phone}`:''
    ];
    return lines.filter(Boolean).join('\n');
  }

  async function crm3107CopyListingIntro(id){
    const item=state.listings?.find(x=>String(x.id)===String(id));
    if(!item)return toast('매물 정보를 찾지 못했습니다.');
    const text=crm3107ListingIntroText(item);
    let copied=false;
    try{
      if(typeof crm384CopyText==='function') copied=await crm384CopyText(text);
  ...
  function crm3107AddFuCopyButton(id){
    const actions=document.querySelector('#modal .modal-actions');
    if(!actions)return;
    actions.querySelector('.crm3107-kakao-copy')?.remove();
    const btn=document.createElement('button');
    btn.type='button';
    btn.className='ghost crm3107-kakao-copy';
    btn.innerHTML='💬 카카오톡 소개 문구 복사';
    btn.onclick=()=>window.crm3107CopyListingIntro(id);
    actions.insertBefore(btn,actions.firstChild);
  }

  const base=window.crm361OpenListingFu||globalThis.crm361OpenListingFu;
  if(typeof base==='function'&&!base.__crm3107){
    const wrapped=async function(id){
      const result=await base.apply(this,arguments);
      requestAnimationFrame(()=>crm3107AddFuCopyButton(id));
      setTimeout(()=>crm3107AddFuCopyButton(id),80);
      return result;
    };
    wrapped.__crm3107=true;
    window.crm361OpenListingFu=wrapped;
    try{crm361OpenListingFu=wrapped}catch(_){ }
    const follow=window.openFollowUpModal||globalThis.openFollowUpModal;
    if(typeof follow==='function'){
      const followWrapped=function(entityType,id){
        if(entityType==='listing')return wrapped(id);
        return follow.apply(this,arguments);
      };
      window.openFollowUpModal=followWrapped;
      try{openFollowUpModal=followWrapped}catch(_){ }
    }
  }
  Object.assign(window,{crm3107CopyListingIntro});
  console.info('CRM v3.10.7 FU 카카오톡 소개문구 복사 적용 완료');
})();


/* ===== CRM v3.10.8 고객 예정 FU 미설치 fallback ===== */
async function crm3108SaveLegacyFu(customerId, clear=false){
  const input=document.getElementById('crm3108LegacyFuDate');
  const next=clear?null:(input?.value||null);
  if(!clear&&!next)return toast('예정일을 선택하세요.');
  const {error}=await state.client.from('customers').update({next_follow_up_at:next}).eq('id',customerId);
  if(error)return toast(`예정 FU 저장 실패: ${error.message}`);
  const customer=state.customers.find(x=>x.id===customerId);
  if(customer)customer.next_follow_up_at=next;
  toast(clear?'예정 FU를 삭제했습니다.':'예정 FU를 저장했습니다.');
  await renderCustomers();
...
(function(){
  ...
  window.crm3107CopyListingIntro=async function(id){
    let item=(state.listings||[]).find(x=>String(x.id)===String(id));
    try{
      const {data,error}=await state.client.from('listings').select('*').eq('id',id).maybeSingle();
      if(!error&&data){item={...(item||{}),...data};const local=(state.listings||[]).find(x=>String(x.id)===String(id));if(local)Object.assign(local,data);}
    }catch(_){ }
    if(!item)return toast('매물 정보를 찾지 못했습니다.');
    const deals=(typeof crm38DealOptions==='function'?crm38DealOptions(item):[]);
    ...
    const lines=[
      ...
      `전용면적: ${area}`,`방/욕실: ${room} / ${item.bathroom_count??'-'}`,building?`층수·사용승인일: ${building}`:'',`입주: ${move}`,features.length?`특징: ${features.join(' · ')}`:'','',
...
console.info('CRM v3.10.10 층수·연식 저장/복원/소개문구 수정 완료');


/* ===== CRM v3.10.11 고객 카카오 버튼 제거 · 변경확인 제거 · 층수연식 소개문구 보강 ===== */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const cleanInt=v=>{const n=parseInt(String(v??'').replace(/[^0-9-]/g,''),10);return Number.isFinite(n)?n:null};

  function isCustomerModal(){
    const title=(q('#modalTitle')?.textContent||'').trim();
    return title.includes('고객') || !!q('#modalForm [name="customer_name"],#modalForm [name="name"]');
  }
  function cleanCustomerKakaoButton(){
    if(!isCustomerModal())return;
    q('#modal .crm3107-kakao-copy')?.remove();
  }
  for(const name of ['openCustomerModal','openCustomerFuHub']){
    const fn=window[name]||globalThis[name];
    if(typeof fn!=='function'||fn.__crm3111NoKakao)continue;
    const wrapped=function(){
      const r=fn.apply(this,arguments);
      requestAnimationFrame(cleanCustomerKakaoButton);
      setTimeout(cleanCustomerKakaoButton,60);
      setTimeout(cleanCustomerKakaoButton,220);
      return r;
    };
    wrapped.__crm3111NoKakao=true;
    window[name]=wrapped;
    try{if(name==='openCustomerModal')openCustomerModal=wrapped;else openCustomerFuHub=wrapped}catch(_){ }
  }
  new MutationObserver(cleanCustomerKakaoButton).observe(q('#modal')||document.body,{childList:true,subtree:true});

  function buildingText(item){
    const current=cleanInt(item?.current_floor ?? item?.floor ?? item?.floor_current);
    const total=cleanInt(item?.total_floors ?? item?.total_floor ?? item?.floor_total);
    const year=cleanInt(item?.built_year ?? item?.construction_year ?? item?.completion_year);
    const parts=[];
    if(current!==null||total!==null){
      if(current!==null&&total!==null)parts.push(`${current}/${total}층`);
      else if(current!==null)parts.push(`${current}층`);
  ...
  window.crm3107CopyListingIntro=async function(id){
    ...
    try{
      ...
      if(!error&&data){
        item={...(item||{}),...data};
        const local=(state.listings||[]).find(x=>String(x.id)===String(id));
    ...
    const area=item.area_m2?`${item.area_m2}㎡${Number(item.area_m2)?` (약 ${(Number(item.area_m2)/3.3058).toFixed(2)}평)`:''}`:'-';
    const location=[item.district,item.address].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join(' · ');
    const building=buildingText(item);
    const lines=[
      `🏠 ${item.title||'매물 안내'}`,
      location?`📍 ${location}`:'',
      item.property_type?`유형: ${item.property_type}`:'',
...
(()=>{
  ...
  function ensureSingleNetworkScrollbar(){
    ...
    const {host,table,wrap}=getParts();
...
/* ===== CRM v3.10.13 모든 시트 상단 가로 드래그바 제거 ===== */
(()=>{
  const selector='.crm3889-top-scroll,.crm3890-top-scroll,.crm396-top-scroll,.crm3106-network-top-scroll,.crm3112-network-top-scroll';
  const cleanup=root=>{(root||document).querySelectorAll?.(selector).forEach(el=>el.remove())};
  cleanup(document);
  const observer=new MutationObserver(records=>{
    for(const rec of records){
  ...
  console.info('CRM v3.10.13 상단 가로 드래그바 제거 완료');
})();

/* ===== CRM v3.10.14 임시저장 완전 제거 · 카카오 소개문구 위치/대상 수정 ===== */
(()=>{
  const DRAFT_KEY_RE=/(?:^|:)draft(?::|$)|^crm(?:390|393|3100).*draft/i;
  const clearDrafts=()=>{
    try{
      for(let i=localStorage.length-1;i>=0;i--){
        const k=localStorage.key(i);
        if(k&&DRAFT_KEY_RE.test(k))localStorage.removeItem(k);
      }
    }catch(_){ }
  };
  clearDrafts();

  // 임시저장을 완전히 끈다. 과거 코드가 다시 저장하거나 질문하지 못하게 차단한다.
  try{
    const rawSetItem=Storage.prototype.setItem;
    if(!rawSetItem.__crm3114NoDraft){
      const patched=function(key,value){
        if(this===localStorage&&DRAFT_KEY_RE.test(String(key||'')))return;
        return rawSetItem.call(this,key,value);
      };
      patched.__crm3114NoDraft=true;
      Storage.prototype.setItem=patched;
    }
  }catch(_){ }
  try{
    const rawConfirm=window.confirm.bind(window);
    if(!window.confirm.__crm3114NoDraft){
      const patched=function(message){
        const text=String(message||'');
        if(text.includes('작성 중이던 임시저장')||text.includes('임시저장 내용이 있습니다')){
          clearDrafts();
          return false;
        }
  ...
  function removeDraftUi(){
    document.querySelectorAll('#crm390DraftTag,#crm393DraftTag,.crm390-draft-tag,.crm393-draft-tag').forEach(x=>x.remove());
    const title=document.querySelector('#modalTitle');
    if(title){
      [...title.childNodes].forEach(n=>{if(n.nodeType===3&&/임시저장/.test(n.textContent||''))n.remove();});
    }
  }
  ...
  function isCustomerModal(){
    ...
    const title=(q('#modalTitle')?.textContent||'').replace(/임시저장됨/g,'').trim();
    const form=q('#modalForm');
    return /고객/.test(title)||!!form?.querySelector('[name="customer_name"],[name="customer_type"],[name="preferred_area"],#customerName');
  }
  function removeCustomerKakao(){
    if(!isCustomerModal())return;
    qa('#modal button').forEach(btn=>{
      const t=(btn.textContent||'').replace(/\s+/g,' ').trim();
      if(/카카오톡.*(?:소개|매물)|(?:소개|매물).*카카오톡/.test(t))btn.remove();
    });
  }
  ...
  // 항상 클릭한 id의 매물을 DB에서 다시 조회해 문구를 만든다.
  window.crm3114CopyListingIntro=async function(listingId){
    const id=String(listingId||'');
    if(!id)return toast('매물 정보를 찾지 못했습니다.');
    let item=(state.listings||[]).find(x=>String(x.id)===id)||null;
    try{
      const {data,error}=await state.client.from('listings').select('*').eq('id',id).maybeSingle();
      if(error)throw error;
      if(data){
        item={...(item||{}),...data};
        const local=(state.listings||[]).find(x=>String(x.id)===id);
        if(local)Object.assign(local,data);
      }
    }catch(e){console.warn('listing intro reload',e);}
    if(!item)return toast('해당 매물 정보를 찾지 못했습니다.');

    const deals=typeof crm38DealOptions==='function'?crm38DealOptions(item):[];
    const dealLines=deals.length?deals.map(o=>o.deal_type==='월세'
      ?`월세 ${Number(o.price||0).toLocaleString()}만원 / ${Number(o.monthly_rent||0).toLocaleString()}만원`
      :`${o.deal_type} ${Number(o.price||0).toLocaleString()}만원`):[];
    const room=typeof listingRoomText==='function'?listingRoomText(item):(item.room_count??'-');
    const move=typeof moveInText==='function'
      ?String(moveInText(item)).replace(/<br\s*\/?>/gi,' / ').replace(/<[^>]+>/g,'')
      :(item.move_in_date||'협의 가능');
    const features=Array.isArray(item.feature_tags)?item.feature_tags:[];
    const area=item.area_m2?`${item.area_m2}㎡${Number(item.area_m2)?` (약 ${(Number(item.area_m2)/3.3058).toFixed(2)}평)`:''}`:'-';
    const fullAddress=[item.address,item.building_dong?`${String(item.building_dong).replace(/동$/,'')}동`:'',item.unit_no?`${String(item.unit_no).replace(/호$/,'')}호`:''].filter(Boolean).join(' ');
    const location=[item.district,fullAddress].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join(' · ');
    const building=buildingText(item);
    const lines=[
      `🏠 ${item.title||'매물 안내'}`,
      location?`📍 ${location}`:'',
      item.property_type?`유형: ${item.property_type}`:'',
      dealLines.length?`조건: ${dealLines.join(' / ')}`:(typeof listingPriceText==='function'?`조건: ${String(listingPriceText(item)).replace(/<br\s*\/?>/gi,' / ').replace(/<[^>]+>/g,'')}`:''),
      `전용면적: ${area}`,
      `방/욕실: ${room} / ${item.bathroom_count??'-'}`,
      building?`층수·사용승인일: ${building}`:'',
      `입주: ${move}`,
    ...
    try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(text);copied=true;}}catch(_){ }
    if(!copied){
      const ta=document.createElement('textarea');ta.value=text;ta.style.cssText='position:fixed;opacity:0;pointer-events:none';document.body.appendChild(ta);ta.select();
      try{copied=document.execCommand('copy')}catch(_){ }ta.remove();
    }
    toast(copied?`${item.title||'매물'} 소개 문구를 복사했습니다.`:'복사가 차단되었습니다. 브라우저의 클립보드 권한을 확인하세요.');
  };
  window.crm3107CopyListingIntro=window.crm3114CopyListingIntro;
  try{crm3107CopyListingIntro=window.crm3114CopyListingIntro}catch(_){ }

  function addListingCopyButton(listingId,place='actions'){
    const modal=q('#modal');
    if(!modal?.open||!listingId||isCustomerModal())return;
    qa('#modal .crm3107-kakao-copy,#modal .crm3114-kakao-copy').forEach(x=>x.remove());
    const target=place==='top'?(q('#modalBody')||q('#modal .modal-body')):q('#modal .modal-actions');
    if(!target)return;
    const btn=document.createElement('button');
    btn.type='button';btn.className='ghost crm3114-kakao-copy';btn.textContent='💬 카카오톡 매물 소개 복사';
    btn.dataset.listingId=String(listingId);
    btn.onclick=()=>window.crm3114CopyListingIntro(btn.dataset.listingId);
    if(place==='top')target.prepend(btn);else target.insertBefore(btn,target.firstChild);
  }

  // FU: 매물일 때만, 현재 클릭한 매물 id로 버튼을 다시 만든다.
  const baseFollow=window.openFollowUpModal||globalThis.openFollowUpModal;
  if(typeof baseFollow==='function'&&!baseFollow.__crm3114){
    const wrapped=async function(entityType,id){
      const result=await baseFollow.apply(this,arguments);
      const apply=()=>{
        removeDraftUi();removeCustomerKakao();
        if(entityType==='listing')addListingCopyButton(id,'actions');
        else qa('#modal .crm3107-kakao-copy,#modal .crm3114-kakao-copy').forEach(x=>x.remove());
      };
      requestAnimationFrame(apply);setTimeout(apply,80);setTimeout(apply,220);
      return result;
    };
    wrapped.__crm3114=true;window.openFollowUpModal=wrapped;
    try{openFollowUpModal=wrapped}catch(_){ }
  }

  // 매물 상세/수정: 기존 매물 id가 있을 때만 버튼 표시한다.
  const baseListing=window.openListingModal||globalThis.openListingModal;
  if(typeof baseListing==='function'&&!baseListing.__crm3114){
    const wrapped=async function(id){
      const result=await baseListing.apply(this,arguments);
      const apply=()=>{
        removeDraftUi();
        if(id)addListingCopyButton(id,'actions');
        else qa('#modal .crm3107-kakao-copy,#modal .crm3114-kakao-copy').forEach(x=>x.remove());
      };
      requestAnimationFrame(apply);setTimeout(apply,90);setTimeout(apply,260);
      return result;
    };
    wrapped.__crm3114=true;window.openListingModal=wrapped;
    try{openListingModal=wrapped}catch(_){ }
  }

  // 자동매칭 카드마다 해당 매물 소개 복사 버튼을 붙인다.
  const baseMatches=window.showCustomerMatches||globalThis.showCustomerMatches;
  if(typeof baseMatches==='function'&&!baseMatches.__crm3114){
    const wrapped=async function(){
      const result=await baseMatches.apply(this,arguments);
      const customer=state.customers.find(x=>String(x.id)===String(q('#matchCustomer')?.value||''));
      if(!customer)return result;
      const rows=(state.listings||[]).filter(x=>x.is_public&&x.status==='available')
        .map(x=>({...x,_match:evaluateListingMatch(customer,x)})).filter(x=>x._match?.matched)
        .sort((a,b)=>{
          if(a._match.matchKind!==b._match.matchKind)return a._match.matchKind==='direct'?-1:1;
          const roomA=Math.max(0,listingRoomValue(a)-customerRoomValue(customer));
          const roomB=Math.max(0,listingRoomValue(b)-customerRoomValue(customer));
      ...
      qa('#matchResults .match-card').forEach((card,index)=>{
        card.querySelector('.crm3114-kakao-copy')?.remove();
        const item=rows[index];if(!item)return;
        const btn=document.createElement('button');btn.type='button';btn.className='ghost crm3114-kakao-copy';btn.textContent='💬 소개 문구 복사';
        btn.onclick=()=>window.crm3114CopyListingIntro(item.id);card.appendChild(btn);
      });
      return result;
    };
    wrapped.__crm3114=true;window.showCustomerMatches=wrapped;
    try{showCustomerMatches=wrapped}catch(_){ }
  }

  const observer=new MutationObserver(()=>{removeDraftUi();removeCustomerKakao();});
  observer.observe(document.body,{childList:true,subtree:true});
  removeDraftUi();removeCustomerKakao();
  console.info('CRM v3.10.14 임시저장 제거·카카오 소개문구 대상/위치 수정 완료');
})();

/* ===== CRM v3.10.15 고객 FU 개별삭제 · 히스토리 버튼 제거 · 조건이력 보강 ===== */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const qa=(s,r=document)=>[...r.querySelectorAll(s)];
  ...
  function removeCustomerHistoryButtons(){
    const host=q('#customerTable');
    if(!host)return;
    qa('button',host).forEach(btn=>{
      if(btn.textContent.trim()==='히스토리')btn.remove();
    });
  }

  async function crm31015DeleteCustomerFu(historyId,customerId){
    const customer=(state.customers||[]).find(x=>sameId(x.id,customerId));
    if(!customer)return toast('고객 정보를 찾지 못했습니다.');
    if(!(isAdmin()||sameId(customer.owner_id,state.profile?.id)))return toast('해당 고객 담당 중개사와 관리자만 FU를 삭제할 수 있습니다.');
    if(!confirm('이 FU 기록을 삭제할까요?'))return;
    const {error}=await state.client.from('interaction_history').delete().eq('id',historyId).eq('customer_id',customerId);
    if(error)return toast(`FU 삭제 실패: ${error.message}`);
    toast('FU 기록을 삭제했습니다.');
    openCustomerFuHub(customerId,'records');
  }

  // FU 기록마다 담당 중개사/관리자에게만 삭제 버튼 표시
  const baseRenderRecords=window.crm3888RenderRecords||globalThis.crm3888RenderRecords;
  async function crm31015RenderRecords(customer){
    const {data,error}=await state.client.from('interaction_history')
      .select('*, writer:profiles!interaction_history_created_by_fkey(full_name)')
      .eq('customer_id',customer.id)
      .neq('contact_method','조건이력')
      .order('follow_up_date',{ascending:false})
      .order('created_at',{ascending:false});
    if(error)return `<div class="crm3888-empty">${escapeHtml(error.message)}</div>`;
    const canDelete=isAdmin()||sameId(customer.owner_id,state.profile?.id);
    return `<div class="crm3888-fu-form"><label>기록 일자<input id="crm3888RecordDate" type="date" value="${today()}"></label><label>상담 종류<select id="crm3888RecordMethod"><option>전화</option><option>부재</option><option>방문</option><option>문자/톡</option><option>매물추천</option><option>계약상담</option><option>기타</option></select></label><label class="span-2">상담·진행 내용<textarea id="crm3888RecordContent" rows="5" placeholder="통화 내용, 고객 반응, 다음 조치 등을 기록하세요."></textarea></label><div class="crm3888-fu-form-actions"><button type="button" class="primary" onclick="crm3888SaveRecord('${customer.id}')">FU 기록 저장</button></div></div>${data?.length?`<div class="crm3888-record-list">${data.map(h=>`<article class="crm3888-record-item"><div class="crm3888-item-head"><div><strong>${escapeHtml(h.contact_method||'FU')}</strong> · ${fmtDate(h.follow_up_date)}</div><div class="crm31015-fu-head-actions"><span class="muted">${escapeHtml(h.writer?.full_name||'')}</span>${canDelete?`<button type="button" class="danger crm31015-fu-delete" onclick="crm31015DeleteCustomerFu('${h.id}','${customer.id}')">삭제</button>`:''}</div></div><div>${escapeHtml(h.content||'').replace(/\n/g,'<br>')}</div></article>`).join('')}</div>`:'<div class="crm3888-empty">아직 FU 기록이 없습니다.</div>'}`;
  }

  // openCustomerFuHub 내부가 지역함수를 참조하므로 최종 화면에서 FU 기록 패널을 다시 그려준다.
  const baseHub=window.openCustomerFuHub||globalThis.openCustomerFuHub;
  if(typeof baseHub==='function'&&!baseHub.__crm31015){
    const wrapped=async function(customerId,tab='records'){
      const result=await baseHub.apply(this,arguments);
      if(tab==='records'){
        const customer=(state.customers||[]).find(x=>sameId(x.id,customerId));
        const body=q('#modalBody');
        if(customer&&body){
          const html=await crm31015RenderRecords(customer);
          body.innerHTML=`${typeof crm3888Tabs==='function'?crm3888Tabs(customerId,'records'):`<div class="crm3888-fu-tabs"><button type="button" class="active">FU 기록</button><button type="button" onclick="openCustomerFuHub('${customerId}','schedule')">예정 FU</button><button type="button" onclick="openCustomerFuHub('${customerId}','conditions')">조건이력</button></div>`}${html}`;
        }
      }
      return result;
    };
    wrapped.__crm31015=true;window.openCustomerFuHub=wrapped;
    try{openCustomerFuHub=wrapped}catch(_){ }
  }

  function money(v){return v===null||v===undefined||v===''?'-':`${Number(v).toLocaleString('ko-KR')}만원`}
  function boolText(v){return v===true?'O':v===false?'X':'미확인'}
  function arrText(v){return Array.isArray(v)&&v.length?v.join(', '):'-'}
  function moveText(c){
    if(c?.desired_move_in_period)return String(c.desired_move_in_period);
    return c?.desired_move_in_date?String(c.desired_move_in_date).slice(0,10):'-';
  }
  async function snapshot(customerId){
    const [{data:c,error:ce},{data:d,error:de}]=await Promise.all([
      state.client.from('customers').select('*').eq('id',customerId).single(),
      state.client.from('customer_deal_options').select('*').eq('customer_id',customerId).order('sort_order')
    ]);
    if(ce)throw ce;
    return {customer:c,deals:de?[]:(d||[])};
  }
  function dealMap(rows){return new Map((rows||[]).map(x=>[x.deal_type,x]))}
  function conditionDiff(before,after){
    const a=before.customer||{},b=after.customer||{},lines=[];
    const am=dealMap(before.deals),bm=dealMap(after.deals);
    const at=[...am.keys()],bt=[...bm.keys()];
    if(at.join('+')!==bt.join('+'))lines.push(`거래유형: ${at.join('+')||'-'} → ${bt.join('+')||'-'}`);
    ['매매','전세','월세'].forEach(type=>{
      const x=am.get(type),y=bm.get(type);
      if(!x&&!y)return;
      if(type==='월세'){
        const old=x?`보증금 ${money(x.budget_max)} / 월 ${money(x.desired_monthly_rent)}`:'없음';
        const now=y?`보증금 ${money(y.budget_max)} / 월 ${money(y.desired_monthly_rent)}`:'없음';
        if(old!==now)lines.push(`월세 조건: ${old} → ${now}`);
      }else if(String(x?.budget_max??'')!==String(y?.budget_max??''))lines.push(`${type} 희망금액: ${money(x?.budget_max)} → ${money(y?.budget_max)}`);
    });
    const room=c=>c.desired_one_point_five_room?'1.5룸':(c.desired_rooms==null?'-':`${c.desired_rooms}룸`);
    const equity=c=>c.equity_unknown?'모름':money(c.equity_capital);
    const pairs=[
      ['희망지역',a.preferred_area||'-',b.preferred_area||'-'],
      ['희망 방 개수',room(a),room(b)],
      ['1.5룸 여부',a.desired_one_point_five_room?'O':'X',b.desired_one_point_five_room?'O':'X'],
      ['대출 여부',boolText(a.loan_available),boolText(b.loan_available)],
      ['자기자본금',equity(a),equity(b)],
      ['희망 매물 특징',arrText(a.desired_feature_tags),arrText(b.desired_feature_tags)],
      ['희망 매물유형',arrText(a.preferred_property_types),arrText(b.preferred_property_types)],
      ['입주 희망일',moveText(a),moveText(b)],
      ['고객 상태',a.status||'-',b.status||'-'],
      ['고객 등급',a.customer_grade||'-',b.customer_grade||'-']
    ];
    pairs.forEach(([label,x,y])=>{if(String(x)!==String(y))lines.push(`${label}: ${x} → ${y}`)});
    return lines;
  }

  async function saveConditionHistory(customerId,before){
    try{
      const after=await snapshot(customerId);
      const lines=conditionDiff(before,after);
      if(!lines.length)return;
      const content=lines.join('\n');
      // 기존 래퍼가 같은 내용을 이미 저장했으면 중복 기록하지 않는다.
      const since=new Date(Date.now()-15000).toISOString();
      const {data:recent}=await state.client.from('interaction_history').select('id,content').eq('customer_id',customerId).eq('contact_method','조건이력').gte('created_at',since).order('created_at',{ascending:false}).limit(5);
      if((recent||[]).some(x=>String(x.content||'')===content))return;
      const {error}=await state.client.from('interaction_history').insert({customer_id:customerId,listing_id:null,created_by:state.profile.id,follow_up_date:today(),contact_method:'조건이력',content,next_follow_up_at:null});
      if(error)console.warn('조건이력 저장 실패',error);
    }catch(err){console.warn('조건이력 비교 실패',err)}
  }

  // 고객 상세 저장 후 DB 실제값을 다시 비교해 변경된 조건만 기록
  const baseCustomer=window.openCustomerModal||globalThis.openCustomerModal;
  if(typeof baseCustomer==='function'&&!baseCustomer.__crm31015){
    const wrapped=async function(id=null){
      let before=null;
      if(id){try{before=await snapshot(id)}catch(_){ }}
      const result=await baseCustomer.apply(this,arguments);
      removeCustomerHistoryButtons();
      if(id&&before){
        const submit=q('#modalSubmit');
        if(submit&&!submit.__crm31015){
          const old=submit.onclick;
          submit.__crm31015=true;
          submit.onclick=async function(e){
            const out=typeof old==='function'?await old.call(this,e):undefined;
            setTimeout(async()=>{
              const modal=q('#modal');
              if(!modal?.open){await saveConditionHistory(id,before);await loadCustomers();}
            },250);
            return out;
          };
        }
    ...
    wrapped.__crm31015=true;window.openCustomerModal=wrapped;
    try{openCustomerModal=wrapped}catch(_){ }
  }

  // 모든 고객 목록 렌더 후 히스토리 버튼 제거
  const baseRender=window.renderCustomers||globalThis.renderCustomers;
  if(typeof baseRender==='function'&&!baseRender.__crm31015){
    const wrapped=async function(){const r=await baseRender.apply(this,arguments);removeCustomerHistoryButtons();requestAnimationFrame(removeCustomerHistoryButtons);return r};
    wrapped.__crm31015=true;window.renderCustomers=wrapped;
    try{renderCustomers=wrapped}catch(_){ }
  }
  const baseFilter=window.filterCustomers||globalThis.filterCustomers;
  if(typeof baseFilter==='function'&&!baseFilter.__crm31015){
    const wrapped=function(){const r=baseFilter.apply(this,arguments);removeCustomerHistoryButtons();requestAnimationFrame(removeCustomerHistoryButtons);return r};
    wrapped.__crm31015=true;window.filterCustomers=wrapped;
    try{filterCustomers=wrapped}catch(_){ }
  }

  const observer=new MutationObserver(()=>removeCustomerHistoryButtons());
  observer.observe(document.body,{childList:true,subtree:true});
  removeCustomerHistoryButtons();
  Object.assign(window,{crm31015DeleteCustomerFu});
  console.info('CRM v3.10.15 고객 히스토리 버튼 제거·FU 개별삭제·조건이력 보강 완료');
})();

/* ===== CRM v3.10.16 고객·매물 예정 FU 간소화 + 수정 기능 복구 ===== */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const sameId=(a,b)=>String(a||'')===String(b||'');
  const isAdmin=()=>String(state?.profile?.role||'').toLowerCase()==='admin';
  ...
  const displayDate=v=>{
    const raw=dateOnly(v);if(!raw)return '-';
    const [y,m,d]=raw.split('-');return `${y}. ${Number(m)}. ${Number(d)}.`;
  };
  const noonIso=date=>new Date(`${date}T12:00:00`).toISOString();

  async function crm31016SyncCustomerNextFu(customerId){
    const {data,error}=await state.client.from('customer_fu_schedules').select('scheduled_at').eq('customer_id',customerId).eq('completed',false).order('scheduled_at',{ascending:true}).limit(1);
    if(error)throw error;
    const next=data?.[0]?.scheduled_at?dateOnly(data[0].scheduled_at):null;
    await state.client.from('customers').update({next_follow_up_at:next}).eq('id',customerId);
    const row=(state.customers||[]).find(x=>sameId(x.id,customerId));if(row)row.next_follow_up_at=next;
  }

  crm3888RenderSchedules=async function(customer){
    const {data,error}=await state.client.from('customer_fu_schedules')
      .select('*, assignee:profiles!customer_fu_schedules_assigned_to_fkey(full_name)')
      .eq('customer_id',customer.id)
      .order('completed',{ascending:true})
      .order('scheduled_at',{ascending:true});
    if(error){
      if(/customer_fu_schedules|schema cache/i.test(error.message||'')){
        const legacyDate=customer.next_follow_up_at?dateOnly(customer.next_follow_up_at):'';
        return `<div class="crm31016-simple-schedule"><div class="crm31016-simple-fields"><label>예정일<input id="crm3108LegacyFuDate" type="date" value="${legacyDate}"></label><label>FU 사유<input id="crm31016LegacyReason" placeholder="예: 조건 재확인, 방문 전 연락"></label></div><div class="crm31016-simple-actions"><button type="button" class="ghost" onclick="crm3108SaveLegacyFu('${customer.id}',true)">일정 지우기</button><button type="button" class="primary" onclick="crm3108SaveLegacyFu('${customer.id}',false)">예정 FU 저장</button></div><p class="muted">전체 기능을 사용하려면 고객 예정 FU SQL 설치가 필요합니다.</p></div>`;
      }
      return `<div class="crm3888-empty">${escapeHtml(error.message)}</div>`;
    }
    const now=Date.now();
    const canEdit=isAdmin()||sameId(customer.owner_id,state.profile?.id);
    const list=(data||[]).map(s=>{
      const overdue=!s.completed&&new Date(s.scheduled_at).getTime()<now;
      return `<article class="crm3888-schedule-item ${overdue?'overdue':''} ${s.completed?'completed':''}">
        <div class="crm3888-item-head">
          <div>
            <div class="${overdue?'crm3888-due-red':'crm3888-due-normal'}">${displayDate(s.scheduled_at)}${overdue?' · 지남':''}</div>
            ${s.reason?`<div class="crm31016-schedule-reason">${escapeHtml(s.reason)}</div>`:''}
          </div>
          <div class="crm3888-item-actions">
            ${s.completed?badge('완료','green'):(canEdit?`<button type="button" class="ghost" onclick="crm3888EditSchedule('${s.id}','${customer.id}')">수정</button><button type="button" class="success" onclick="crm3888CompleteSchedule('${s.id}','${customer.id}')">완료</button><button type="button" class="danger" onclick="crm3888DeleteSchedule('${s.id}','${customer.id}')">삭제</button>`:'')}
          </div>
        </div>
      </article>`;
    }).join('');
    return `<div class="crm31016-simple-schedule">
      <input id="crm3888ScheduleId" type="hidden">
      <div class="crm31016-simple-fields">
        <label>예정일<input id="crm3888ScheduleDate" type="date" value="${today()}"></label>
        <label>FU 사유<input id="crm3888ScheduleReason" placeholder="예: 조건 재확인, 방문 전 연락"></label>
      </div>
      <div class="crm31016-simple-actions"><button type="button" class="ghost" onclick="crm3888ResetScheduleForm()">입력 초기화</button><button type="button" class="primary" onclick="crm3888SaveSchedule('${customer.id}')">예정 FU 저장</button></div>
    </div>${list?`<div class="crm3888-schedule-list">${list}</div>`:'<div class="crm3888-empty">등록된 예정 FU가 없습니다.</div>'}`;
  };

  crm3888SaveSchedule=async function(customerId){
    const id=q('#crm3888ScheduleId')?.value||'';
    const date=q('#crm3888ScheduleDate')?.value||'';
    const reason=q('#crm3888ScheduleReason')?.value.trim()||'';
    if(!date)return toast('예정일을 입력하세요.');
    const row={customer_id:customerId,scheduled_at:noonIso(date),reason:reason||'예정 FU',assigned_to:state.profile.id,updated_at:new Date().toISOString()};
    let error;
    if(id)({error}=await state.client.from('customer_fu_schedules').update(row).eq('id',id).eq('customer_id',customerId));
    else ({error}=await state.client.from('customer_fu_schedules').insert({...row,created_by:state.profile.id}));
    if(error)return toast(`예정 FU 저장 실패: ${error.message}`);
    await crm31016SyncCustomerNextFu(customerId);
    toast(id?'예정 FU를 수정했습니다.':'예정 FU를 등록했습니다.');
    await openCustomerFuHub(customerId,'schedule');
    renderCustomers();
  };

  crm3888EditSchedule=async function(scheduleId,customerId){
    const {data,error}=await state.client.from('customer_fu_schedules').select('*').eq('id',scheduleId).eq('customer_id',customerId).single();
    if(error)return toast(`예정 FU를 불러오지 못했습니다: ${error.message}`);
    const idEl=q('#crm3888ScheduleId'),dateEl=q('#crm3888ScheduleDate'),reasonEl=q('#crm3888ScheduleReason');
    if(!idEl||!dateEl||!reasonEl)return toast('수정 입력창을 찾지 못했습니다.');
    idEl.value=data.id;
    dateEl.value=dateOnly(data.scheduled_at);
    reasonEl.value=data.reason==='예정 FU'?'':(data.reason||'');
    reasonEl.focus();
    reasonEl.scrollIntoView({behavior:'smooth',block:'center'});
  };

  crm3888ResetScheduleForm=function(){
    const id=q('#crm3888ScheduleId'),date=q('#crm3888ScheduleDate'),reason=q('#crm3888ScheduleReason');
    if(id)id.value='';if(date)date.value=today();if(reason)reason.value='';
  };

  crm3888CompleteSchedule=async function(scheduleId,customerId){
    const {data,error}=await state.client.from('customer_fu_schedules').select('*').eq('id',scheduleId).eq('customer_id',customerId).single();
    if(error)return toast(error.message);
    const now=new Date().toISOString();
    const {error:uErr}=await state.client.from('customer_fu_schedules').update({completed:true,completed_at:now,updated_at:now}).eq('id',scheduleId).eq('customer_id',customerId);
    if(uErr)return toast(uErr.message);
    const content=[`예정 FU 완료`,`예정일: ${displayDate(data.scheduled_at)}`,data.reason&&data.reason!=='예정 FU'?`사유: ${data.reason}`:''].filter(Boolean).join('\n');
    const {error:hErr}=await state.client.from('interaction_history').insert({customer_id:customerId,listing_id:null,created_by:state.profile.id,follow_up_date:today(),contact_method:'예정 FU 완료',content,next_follow_up_at:null});
    if(hErr)return toast(hErr.message);
    await state.client.from('customers').update({last_follow_up_at:today()}).eq('id',customerId);
    await crm31016SyncCustomerNextFu(customerId);
    toast('완료 처리되어 FU 기록으로 이동했습니다.');
    await openCustomerFuHub(customerId,'schedule');renderCustomers();
  };

  crm3888DeleteSchedule=async function(scheduleId,customerId){
    if(!confirm('이 예정 FU를 삭제할까요?'))return;
    const {error}=await state.client.from('customer_fu_schedules').delete().eq('id',scheduleId).eq('customer_id',customerId);
    if(error)return toast(error.message);
    await crm31016SyncCustomerNextFu(customerId);
    toast('예정 FU를 삭제했습니다.');
    await openCustomerFuHub(customerId,'schedule');renderCustomers();
  ...
  async function crm31016SyncListingNextFu(listingId){
    const {data,error}=await state.client.from('listing_fu_schedules').select('scheduled_at').eq('listing_id',listingId).eq('completed',false).order('scheduled_at',{ascending:true}).limit(1);
    if(error)throw error;
    const next=data?.[0]?.scheduled_at?dateOnly(data[0].scheduled_at):null;
    await state.client.from('listings').update({next_follow_up_at:next}).eq('id',listingId);
    const row=(state.listings||[]).find(x=>sameId(x.id,listingId));if(row)row.next_follow_up_at=next;
  }

  async function crm31016RenderListingSchedules(listingId){
    const listing=(state.listings||[]).find(x=>sameId(x.id,listingId));
    const panel=q('.crm361-fu-panel[data-panel="schedule"]');if(!panel)return;
    panel.innerHTML='<div class="crm3888-empty">불러오는 중입니다.</div>';
    const {data,error}=await state.client.from('listing_fu_schedules').select('*').eq('listing_id',listingId).order('completed',{ascending:true}).order('scheduled_at',{ascending:true});
    if(error){
      if(/listing_fu_schedules|schema cache/i.test(error.message||'')){
        panel.innerHTML='<div class="crm3888-empty">v3.10.16 예정 FU SQL을 Supabase에서 먼저 실행해 주세요.</div>';
      }else panel.innerHTML=`<div class="crm3888-empty">${escapeHtml(error.message)}</div>`;
      return;
    }
    const canEdit=isAdmin()||sameId(listing?.owner_id,state.profile?.id);
    const now=Date.now();
    const list=(data||[]).map(s=>{
      const overdue=!s.completed&&new Date(s.scheduled_at).getTime()<now;
      return `<article class="crm3888-schedule-item ${overdue?'overdue':''} ${s.completed?'completed':''}"><div class="crm3888-item-head"><div><div class="${overdue?'crm3888-due-red':'crm3888-due-normal'}">${displayDate(s.scheduled_at)}${overdue?' · 지남':''}</div>${s.reason?`<div class="crm31016-schedule-reason">${escapeHtml(s.reason)}</div>`:''}</div><div class="crm3888-item-actions">${s.completed?badge('완료','green'):(canEdit?`<button type="button" class="ghost" onclick="crm31016EditListingSchedule('${s.id}','${listingId}')">수정</button><button type="button" class="success" onclick="crm31016CompleteListingSchedule('${s.id}','${listingId}')">완료</button><button type="button" class="danger" onclick="crm31016DeleteListingSchedule('${s.id}','${listingId}')">삭제</button>`:'')}</div></div></article>`;
    }).join('');
    panel.innerHTML=`<div class="crm31016-simple-schedule"><input id="crm31016ListingScheduleId" type="hidden"><div class="crm31016-simple-fields"><label>예정일<input id="crm3830ScheduleDate" type="date" value="${today()}"></label><label>FU 사유<input id="crm31016ListingScheduleReason" placeholder="예: 소유주 재확인, 광고 갱신"></label></div><div class="crm31016-simple-actions"><button type="button" class="ghost" onclick="crm31016ResetListingSchedule()">입력 초기화</button><button type="button" class="primary" onclick="crm31016SaveListingSchedule('${listingId}')">예정 FU 저장</button></div></div>${list?`<div class="crm3888-schedule-list">${list}</div>`:'<div class="crm3888-empty">등록된 예정 FU가 없습니다.</div>'}`;
  }

  async function crm31016SaveListingSchedule(listingId){
    const id=q('#crm31016ListingScheduleId')?.value||'';
    const date=q('#crm3830ScheduleDate')?.value||'';
    const reason=q('#crm31016ListingScheduleReason')?.value.trim()||'';
    if(!date)return toast('예정일을 입력하세요.');
    const row={listing_id:listingId,scheduled_at:noonIso(date),reason:reason||'예정 FU',updated_at:new Date().toISOString()};
    let error;
    if(id)({error}=await state.client.from('listing_fu_schedules').update(row).eq('id',id).eq('listing_id',listingId));
    else ({error}=await state.client.from('listing_fu_schedules').insert({...row,created_by:state.profile.id}));
    if(error)return toast(`예정 FU 저장 실패: ${error.message}`);
    await crm31016SyncListingNextFu(listingId);
    toast(id?'예정 FU를 수정했습니다.':'예정 FU를 등록했습니다.');
    await crm31016RenderListingSchedules(listingId);crm3830RefreshListingScreen();
  }
  async function crm31016EditListingSchedule(scheduleId,listingId){
    const {data,error}=await state.client.from('listing_fu_schedules').select('*').eq('id',scheduleId).eq('listing_id',listingId).single();
    if(error)return toast(error.message);
    const id=q('#crm31016ListingScheduleId'),date=q('#crm3830ScheduleDate'),reason=q('#crm31016ListingScheduleReason');
    if(!id||!date||!reason)return toast('수정 입력창을 찾지 못했습니다.');
    id.value=data.id;date.value=dateOnly(data.scheduled_at);reason.value=data.reason==='예정 FU'?'':(data.reason||'');
    reason.focus();reason.scrollIntoView({behavior:'smooth',block:'center'});
  }
  function crm31016ResetListingSchedule(){const id=q('#crm31016ListingScheduleId'),date=q('#crm3830ScheduleDate'),reason=q('#crm31016ListingScheduleReason');if(id)id.value='';if(date)date.value=today();if(reason)reason.value='';}
  async function crm31016CompleteListingSchedule(scheduleId,listingId){
    const {data,error}=await state.client.from('listing_fu_schedules').select('*').eq('id',scheduleId).eq('listing_id',listingId).single();if(error)return toast(error.message);
    const now=new Date().toISOString();
    const {error:uErr}=await state.client.from('listing_fu_schedules').update({completed:true,completed_at:now,updated_at:now}).eq('id',scheduleId).eq('listing_id',listingId);if(uErr)return toast(uErr.message);
    const content=[`예정 FU 완료`,`예정일: ${displayDate(data.scheduled_at)}`,data.reason&&data.reason!=='예정 FU'?`사유: ${data.reason}`:''].filter(Boolean).join('\n');
    const {error:hErr}=await state.client.from('interaction_history').insert({customer_id:null,listing_id:listingId,created_by:state.profile.id,follow_up_date:today(),contact_method:'예정 FU 완료',content,next_follow_up_at:null});if(hErr)return toast(hErr.message);
    await state.client.from('listings').update({last_follow_up_at:today()}).eq('id',listingId);
    await crm31016SyncListingNextFu(listingId);toast('완료 처리되어 FU 기록으로 이동했습니다.');await crm31016RenderListingSchedules(listingId);crm3830RefreshListingScreen();
  }
  async function crm31016DeleteListingSchedule(scheduleId,listingId){
    if(!confirm('이 예정 FU를 삭제할까요?'))return;
    const {error}=await state.client.from('listing_fu_schedules').delete().eq('id',scheduleId).eq('listing_id',listingId);if(error)return toast(error.message);
    await crm31016SyncListingNextFu(listingId);toast('예정 FU를 삭제했습니다.');await crm31016RenderListingSchedules(listingId);crm3830RefreshListingScreen();
  }

  const baseListingFu=crm361OpenListingFu;
  crm361OpenListingFu=async function(id){
    await baseListingFu(id);
    await crm31016RenderListingSchedules(id);
    const submit=q('#modalSubmit');
    const original=submit?.onclick;
    if(submit&&typeof original==='function'){
      submit.onclick=async function(e){
        const active=q('.crm361-fu-tab.active')?.dataset.tab||'record';
        if(active==='schedule'){e.preventDefault();return crm31016SaveListingSchedule(id);}
        return original.call(this,e);
      };
    }
  };

  Object.assign(window,{crm3888RenderSchedules,crm3888SaveSchedule,crm3888EditSchedule,crm3888ResetScheduleForm,crm3888CompleteSchedule,crm3888DeleteSchedule,crm361OpenListingFu,crm31016SaveListingSchedule,crm31016EditListingSchedule,crm31016ResetListingSchedule,crm31016CompleteListingSchedule,crm31016DeleteListingSchedule});
  console.info('CRM v3.10.16 고객·매물 예정 FU 간소화 및 수정 기능 복구 완료');
})();

/* ===== CRM v3.10.17 고객·매물 예정 FU 동일 UI + 수정 기능 확정 ===== */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const sameId=(a,b)=>String(a||'')===String(b||'');
  const isAdmin=()=>String(state?.profile?.role||'').toLowerCase()==='admin';
  const dateOnly=v=>{if(!v)return '';const d=new Date(v);if(Number.isNaN(d.getTime()))return String(v).slice(0,10);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`};
  const displayDate=v=>{const raw=dateOnly(v);if(!raw)return '-';const [y,m,d]=raw.split('-');return `${y}. ${Number(m)}. ${Number(d)}.`};
  const noonIso=date=>new Date(`${date}T12:00:00`).toISOString();

  function unifiedForm(kind,id,dateValue,reasonValue='',editId=''){
    const customer=kind==='customer';
    const dateId=customer?'crm3888ScheduleDate':'crm3830ScheduleDate';
    const reasonId=customer?'crm3888ScheduleReason':'crm31016ListingScheduleReason';
    const hiddenId=customer?'crm3888ScheduleId':'crm31016ListingScheduleId';
    const resetFn=customer?'crm3888ResetScheduleForm()':'crm31016ResetListingSchedule()';
    const saveFn=customer?`crm3888SaveSchedule('${id}')`:`crm31016SaveListingSchedule('${id}')`;
    return `<section class="crm31017-schedule-box">
      <input id="${hiddenId}" type="hidden" value="${escapeHtml(editId||'')}">
      <div class="crm31017-schedule-main">
        <div class="crm31017-schedule-copy">
          <strong>예정 FU 일정</strong>
          <span>FU 기록과 최종 FU에는 영향을 주지 않고, 목록의 예정 FU에만 표시됩니다.</span>
        </div>
        <label class="crm31017-date-field"><span>예정 FU</span><input id="${dateId}" type="date" value="${escapeHtml(dateValue||today())}"></label>
        <button type="button" class="ghost crm31017-clear" onclick="${resetFn}">일정 지우기</button>
      </div>
      <label class="crm31017-reason-field"><span>간단한 사유</span><input id="${reasonId}" value="${escapeHtml(reasonValue||'')}" placeholder="예: 조건 재확인, 방문 전 연락"></label>
      <div class="crm31017-form-actions">
        ${customer?'':`<button type="button" class="ghost" onclick="crm31014CopyListingIntro('${id}')">💬 카카오톡 매물 소개 복사</button>`}
        <div class="crm31017-form-actions-right"><button type="button" class="ghost" onclick="${resetFn}">입력 초기화</button><button type="button" class="primary" onclick="${saveFn}">${editId?'수정 저장':'예정 FU 저장'}</button></div>
      </div>
    </section>`;
  }

  function scheduleCards(rows,kind,parentId,canEdit){
    const now=Date.now();
    return (rows||[]).map(s=>{
      const overdue=!s.completed&&new Date(s.scheduled_at).getTime()<now;
      const editFn=kind==='customer'?`crm3888EditSchedule('${s.id}','${parentId}')`:`crm31016EditListingSchedule('${s.id}','${parentId}')`;
      const doneFn=kind==='customer'?`crm3888CompleteSchedule('${s.id}','${parentId}')`:`crm31016CompleteListingSchedule('${s.id}','${parentId}')`;
      const delFn=kind==='customer'?`crm3888DeleteSchedule('${s.id}','${parentId}')`:`crm31016DeleteListingSchedule('${s.id}','${parentId}')`;
      return `<article class="crm3888-schedule-item ${overdue?'overdue':''} ${s.completed?'completed':''}">
        <div class="crm3888-item-head">
          <div class="crm31017-card-copy">
            <div class="${overdue?'crm3888-due-red':'crm3888-due-normal'}">${displayDate(s.scheduled_at)}${overdue?' · 지남':''}</div>
            ${s.reason&&s.reason!=='예정 FU'?`<div class="crm31016-schedule-reason">${escapeHtml(s.reason)}</div>`:'<div class="crm31016-schedule-reason muted">사유 없음</div>'}
          </div>
          <div class="crm3888-item-actions">
            ${s.completed?badge('완료','green'):(canEdit?`<button type="button" class="ghost" onclick="${editFn}">수정</button><button type="button" class="success" onclick="${doneFn}">완료</button><button type="button" class="danger" onclick="${delFn}">삭제</button>`:'')}
          </div>
        </div>
      </article>`;
    }).join('');
  ...
  crm3888RenderSchedules=async function(customer){
    const {data,error}=await state.client.from('customer_fu_schedules').select('*').eq('customer_id',customer.id).order('completed',{ascending:true}).order('scheduled_at',{ascending:true});
    if(error){
      if(/customer_fu_schedules|schema cache/i.test(error.message||'')) return `<div class="crm3888-empty">고객 예정 FU SQL을 먼저 설치해 주세요.</div>`;
      return `<div class="crm3888-empty">${escapeHtml(error.message)}</div>`;
    }
    const canEdit=isAdmin()||sameId(customer.owner_id,state.profile?.id);
    const list=scheduleCards(data,'customer',customer.id,canEdit);
    return unifiedForm('customer',customer.id,today())+(list?`<div class="crm3888-schedule-list">${list}</div>`:'<div class="crm3888-empty">등록된 예정 FU가 없습니다.</div>');
  };

  async function renderListing(listingId){
    const listing=(state.listings||[]).find(x=>sameId(x.id,listingId));
    const panel=q('.crm361-fu-panel[data-panel="schedule"]');if(!panel)return;
    panel.innerHTML='<div class="crm3888-empty">불러오는 중입니다.</div>';
    const {data,error}=await state.client.from('listing_fu_schedules').select('*').eq('listing_id',listingId).order('completed',{ascending:true}).order('scheduled_at',{ascending:true});
    if(error){panel.innerHTML=`<div class="crm3888-empty">${/listing_fu_schedules|schema cache/i.test(error.message||'')?'매물 예정 FU SQL을 먼저 설치해 주세요.':escapeHtml(error.message)}</div>`;return;}
    const canEdit=isAdmin()||sameId(listing?.owner_id,state.profile?.id);
    const list=scheduleCards(data,'listing',listingId,canEdit);
    panel.innerHTML=unifiedForm('listing',listingId,today())+(list?`<div class="crm3888-schedule-list">${list}</div>`:'<div class="crm3888-empty">등록된 예정 FU가 없습니다.</div>');
  }
  ...
  crm3888EditSchedule=async function(scheduleId,customerId){
    ...
    if(error)return toast(`예정 FU를 불러오지 못했습니다: ${error.message}`);
    const idEl=q('#crm3888ScheduleId'),dateEl=q('#crm3888ScheduleDate'),reasonEl=q('#crm3888ScheduleReason');
    if(!idEl||!dateEl||!reasonEl)return toast('수정 입력창을 찾지 못했습니다. 예정 FU 탭을 다시 열어주세요.');
    idEl.value=data.id;dateEl.value=dateOnly(data.scheduled_at);reasonEl.value=data.reason==='예정 FU'?'':(data.reason||'');
    const btn=q('.crm31017-form-actions-right .primary');if(btn)btn.textContent='수정 저장';
    q('.crm31017-schedule-box')?.scrollIntoView({behavior:'smooth',block:'start'});reasonEl.focus();
  };

  crm31016EditListingSchedule=async function(scheduleId,listingId){
    const {data,error}=await state.client.from('listing_fu_schedules').select('*').eq('id',scheduleId).eq('listing_id',listingId).single();
    if(error)return toast(`예정 FU를 불러오지 못했습니다: ${error.message}`);
    const idEl=q('#crm31016ListingScheduleId'),dateEl=q('#crm3830ScheduleDate'),reasonEl=q('#crm31016ListingScheduleReason');
    if(!idEl||!dateEl||!reasonEl)return toast('수정 입력창을 찾지 못했습니다. 예정 FU 탭을 다시 열어주세요.');
    idEl.value=data.id;dateEl.value=dateOnly(data.scheduled_at);reasonEl.value=data.reason==='예정 FU'?'':(data.reason||'');
    const btn=q('.crm31017-form-actions-right .primary');if(btn)btn.textContent='수정 저장';
    q('.crm31017-schedule-box')?.scrollIntoView({behavior:'smooth',block:'start'});reasonEl.focus();
  ...
  crm3888ResetScheduleForm=function(){const id=q('#crm3888ScheduleId'),date=q('#crm3888ScheduleDate'),reason=q('#crm3888ScheduleReason');if(id)id.value='';if(date)date.value='';if(reason)reason.value='';const btn=q('.crm31017-form-actions-right .primary');if(btn)btn.textContent='예정 FU 저장';};
  crm31016ResetListingSchedule=function(){const id=q('#crm31016ListingScheduleId'),date=q('#crm3830ScheduleDate'),reason=q('#crm31016ListingScheduleReason');if(id)id.value='';if(date)date.value='';if(reason)reason.value='';const btn=q('.crm31017-form-actions-right .primary');if(btn)btn.textContent='예정 FU 저장';};

  // 완료 시 사유까지 FU 기록에 남기도록 보강
  crm3888CompleteSchedule=async function(scheduleId,customerId){
    const {data,error}=await state.client.from('customer_fu_schedules').select('*').eq('id',scheduleId).eq('customer_id',customerId).single();if(error)return toast(error.message);
    const now=new Date().toISOString();
    const {error:uErr}=await state.client.from('customer_fu_schedules').update({completed:true,completed_at:now,updated_at:now}).eq('id',scheduleId).eq('customer_id',customerId);if(uErr)return toast(uErr.message);
    const content=[`예정 FU 완료`,`예정일: ${displayDate(data.scheduled_at)}`,data.reason&&data.reason!=='예정 FU'?`사유: ${data.reason}`:''].filter(Boolean).join('\n');
    const {error:hErr}=await state.client.from('interaction_history').insert({customer_id:customerId,listing_id:null,created_by:state.profile.id,follow_up_date:today(),contact_method:'예정 FU 완료',content,next_follow_up_at:null});if(hErr)return toast(hErr.message);
    await state.client.from('customers').update({last_follow_up_at:today()}).eq('id',customerId);await crm31016SyncCustomerNextFu(customerId);
    toast('완료 처리되어 FU 기록으로 이동했습니다.');await openCustomerFuHub(customerId,'schedule');renderCustomers();
  };
  crm31016CompleteListingSchedule=async function(scheduleId,listingId){
    const {data,error}=await state.client.from('listing_fu_schedules').select('*').eq('id',scheduleId).eq('listing_id',listingId).single();if(error)return toast(error.message);
    const now=new Date().toISOString();
    const {error:uErr}=await state.client.from('listing_fu_schedules').update({completed:true,completed_at:now,updated_at:now}).eq('id',scheduleId).eq('listing_id',listingId);if(uErr)return toast(uErr.message);
    const content=[`예정 FU 완료`,`예정일: ${displayDate(data.scheduled_at)}`,data.reason&&data.reason!=='예정 FU'?`사유: ${data.reason}`:''].filter(Boolean).join('\n');
    const {error:hErr}=await state.client.from('interaction_history').insert({customer_id:null,listing_id:listingId,created_by:state.profile.id,follow_up_date:today(),contact_method:'예정 FU 완료',content,next_follow_up_at:null});if(hErr)return toast(hErr.message);
    await state.client.from('listings').update({last_follow_up_at:today()}).eq('id',listingId);await crm31016SyncListingNextFu(listingId);
    toast('완료 처리되어 FU 기록으로 이동했습니다.');await renderListing(listingId);crm3830RefreshListingScreen();
  };

  const baseListingFu=crm361OpenListingFu;
  crm361OpenListingFu=async function(id){await baseListingFu(id);await renderListing(id);};
  Object.assign(window,{crm3888RenderSchedules,crm3888EditSchedule,crm3888ResetScheduleForm,crm3888CompleteSchedule,crm31016EditListingSchedule,crm31016ResetListingSchedule,crm31016CompleteListingSchedule,crm361OpenListingFu});
  console.info('CRM v3.10.17 고객·매물 예정 FU 동일 UI 및 수정 기능 확정');
})();

/* ===== CRM v3.10.18 예정 FU UI 강제 통일 ===== */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const sameId=(a,b)=>String(a||'')===String(b||'');
  const isAdmin=()=>String(state?.profile?.role||'').toLowerCase()==='admin';
  ...
  const displayDate=v=>{const x=dateOnly(v);if(!x)return '-';const [y,m,d]=x.split('-');return `${y}. ${Number(m)}. ${Number(d)}.`;};
  const noonIso=date=>new Date(`${date}T12:00:00`).toISOString();

  function tabs(customerId,active){
    return `<div class="crm3888-fu-tabs"><button type="button" class="${active==='records'?'active':''}" onclick="openCustomerFuHub('${customerId}','records')">FU 기록</button><button type="button" class="${active==='schedule'?'active':''}" onclick="openCustomerFuHub('${customerId}','schedule')">예정 FU</button><button type="button" class="${active==='conditions'?'active':''}" onclick="openCustomerFuHub('${customerId}','conditions')">조건이력</button></div>`;
  }
  function compactForm(kind,id){
    const customer=kind==='customer';
    const hidden=customer?'crm3888ScheduleId':'crm31016ListingScheduleId';
    const dateId=customer?'crm3888ScheduleDate':'crm3830ScheduleDate';
    const reasonId=customer?'crm3888ScheduleReason':'crm31016ListingScheduleReason';
    const reset=customer?'crm31018ResetCustomerSchedule()':'crm31018ResetListingSchedule()';
    const save=customer?`crm31018SaveCustomerSchedule('${id}')`:`crm31018SaveListingSchedule('${id}')`;
    return `<section class="crm31018-compact-schedule">
      <input id="${hidden}" type="hidden">
      <div class="crm31018-compact-row">
        <div class="crm31018-copy"><strong>예정 FU 일정</strong><span>목록의 예정 FU에만 반영됩니다.</span></div>
        <label class="crm31018-date"><span>예정 FU</span><input id="${dateId}" type="date" value="${today()}"></label>
        <button type="button" class="ghost crm31018-clear" onclick="${reset}">일정 지우기</button>
      </div>
      <label class="crm31018-reason"><span>간단한 사유</span><input id="${reasonId}" placeholder="예: 조건 재확인, 방문 전 연락"></label>
      <div class="crm31018-actions">
        ${customer?'':`<button type="button" class="ghost" onclick="crm31014CopyListingIntro('${id}')">💬 카카오톡 매물 소개 복사</button>`}
        <button type="button" class="primary crm31018-save" onclick="${save}">예정 FU 저장</button>
      </div>
    </section>`;
  }
  function cards(rows,kind,parentId,canEdit){
    const now=Date.now();
    return (rows||[]).map(s=>{
      const overdue=!s.completed&&new Date(s.scheduled_at).getTime()<now;
      const edit=kind==='customer'?`crm31018EditCustomerSchedule('${s.id}','${parentId}')`:`crm31018EditListingSchedule('${s.id}','${parentId}')`;
      const done=kind==='customer'?`crm3888CompleteSchedule('${s.id}','${parentId}')`:`crm31016CompleteListingSchedule('${s.id}','${parentId}')`;
      const del=kind==='customer'?`crm3888DeleteSchedule('${s.id}','${parentId}')`:`crm31016DeleteListingSchedule('${s.id}','${parentId}')`;
      return `<article class="crm3888-schedule-item ${overdue?'overdue':''} ${s.completed?'completed':''}">
        <div class="crm3888-item-head"><div><div class="${overdue?'crm3888-due-red':'crm3888-due-normal'}">${displayDate(s.scheduled_at)}${overdue?' · 지남':''}</div><div class="crm31016-schedule-reason">${escapeHtml((s.reason&&s.reason!=='예정 FU')?s.reason:'사유 없음')}</div></div>
        <div class="crm3888-item-actions">${s.completed?badge('완료','green'):(canEdit?`<button type="button" class="ghost" onclick="${edit}">수정</button><button type="button" class="success" onclick="${done}">완료</button><button type="button" class="danger" onclick="${del}">삭제</button>`:'')}</div></div>
      </article>`;
    }).join('');
  }
  async function syncCustomer(customerId){
    const {data,error}=await state.client.from('customer_fu_schedules').select('scheduled_at').eq('customer_id',customerId).eq('completed',false).order('scheduled_at',{ascending:true}).limit(1);if(error)throw error;
    const next=data?.[0]?.scheduled_at?dateOnly(data[0].scheduled_at):null;
    await state.client.from('customers').update({next_follow_up_at:next}).eq('id',customerId);
    const row=(state.customers||[]).find(x=>sameId(x.id,customerId));if(row)row.next_follow_up_at=next;
  }
  async function syncListing(listingId){
    const {data,error}=await state.client.from('listing_fu_schedules').select('scheduled_at').eq('listing_id',listingId).eq('completed',false).order('scheduled_at',{ascending:true}).limit(1);if(error)throw error;
    const next=data?.[0]?.scheduled_at?dateOnly(data[0].scheduled_at):null;
    await state.client.from('listings').update({next_follow_up_at:next}).eq('id',listingId);
    const row=(state.listings||[]).find(x=>sameId(x.id,listingId));if(row)row.next_follow_up_at=next;
  }
  async function renderCustomerSchedules(customer){
    const {data,error}=await state.client.from('customer_fu_schedules').select('*').eq('customer_id',customer.id).order('completed',{ascending:true}).order('scheduled_at',{ascending:true});
    if(error)return `<div class="crm3888-empty">${/customer_fu_schedules|schema cache/i.test(error.message||'')?'고객 예정 FU SQL을 먼저 설치해 주세요.':escapeHtml(error.message)}</div>`;
    const canEdit=isAdmin()||sameId(customer.owner_id,state.profile?.id);
    const list=cards(data,'customer',customer.id,canEdit);
    return compactForm('customer',customer.id)+(list?`<div class="crm3888-schedule-list">${list}</div>`:'<div class="crm3888-empty">등록된 예정 FU가 없습니다.</div>');
  }
  async function renderListingSchedules(listingId){
    const listing=(state.listings||[]).find(x=>sameId(x.id,listingId));
    const panel=q('.crm361-fu-panel[data-panel="schedule"]');if(!panel)return;
    const {data,error}=await state.client.from('listing_fu_schedules').select('*').eq('listing_id',listingId).order('completed',{ascending:true}).order('scheduled_at',{ascending:true});
    if(error){panel.innerHTML=`<div class="crm3888-empty">${/listing_fu_schedules|schema cache/i.test(error.message||'')?'매물 예정 FU SQL을 먼저 설치해 주세요.':escapeHtml(error.message)}</div>`;return;}
    const canEdit=isAdmin()||sameId(listing?.owner_id,state.profile?.id);
    const list=cards(data,'listing',listingId,canEdit);
    panel.innerHTML=compactForm('listing',listingId)+(list?`<div class="crm3888-schedule-list">${list}</div>`:'<div class="crm3888-empty">등록된 예정 FU가 없습니다.</div>');
  }

  const baseOpenCustomerFuHub=window.openCustomerFuHub;
  window.openCustomerFuHub=openCustomerFuHub=async function(customerId,tab='records'){
    if(tab!=='schedule')return baseOpenCustomerFuHub(customerId,tab);
    const customer=(state.customers||[]).find(x=>sameId(x.id,customerId));if(!customer)return toast('고객을 찾지 못했습니다.');
    q('#modalTitle').textContent=`${customer.name} · 고객 FU 관리`;
    q('#modalSubmit').style.display='none';
    q('#modalBody').innerHTML=tabs(customerId,'schedule')+'<div class="crm3888-empty">불러오는 중입니다.</div>';
    q('#modal').showModal();
    q('#modalBody').innerHTML=tabs(customerId,'schedule')+await renderCustomerSchedules(customer);
    const restore=()=>{q('#modalSubmit').style.display='';q('#modal').removeEventListener('close',restore)};q('#modal').addEventListener('close',restore,{once:true});
  };

  const baseListingFu=window.crm361OpenListingFu||crm361OpenListingFu;
  window.crm361OpenListingFu=crm361OpenListingFu=async function(id){
    await baseListingFu(id);
    await renderListingSchedules(id);
  };
  window.openFollowUpModal=openFollowUpModal=function(entityType,id){return entityType==='listing'?window.crm361OpenListingFu(id):window.openCustomerFuHub(id,'records');};

  window.crm31018SaveCustomerSchedule=async function(customerId){
    const id=q('#crm3888ScheduleId')?.value||'',date=q('#crm3888ScheduleDate')?.value||'',reason=q('#crm3888ScheduleReason')?.value.trim()||'';if(!date)return toast('예정일을 입력하세요.');
    const row={customer_id:customerId,scheduled_at:noonIso(date),reason:reason||'예정 FU',assigned_to:state.profile.id,updated_at:new Date().toISOString()};
    const result=id?await state.client.from('customer_fu_schedules').update(row).eq('id',id).eq('customer_id',customerId):await state.client.from('customer_fu_schedules').insert({...row,created_by:state.profile.id});
    if(result.error)return toast(result.error.message);await syncCustomer(customerId);toast(id?'예정 FU를 수정했습니다.':'예정 FU를 등록했습니다.');await window.openCustomerFuHub(customerId,'schedule');renderCustomers();
  };
  window.crm31018SaveListingSchedule=async function(listingId){
    const id=q('#crm31016ListingScheduleId')?.value||'',date=q('#crm3830ScheduleDate')?.value||'',reason=q('#crm31016ListingScheduleReason')?.value.trim()||'';if(!date)return toast('예정일을 입력하세요.');
    const row={listing_id:listingId,scheduled_at:noonIso(date),reason:reason||'예정 FU',created_by:state.profile.id,updated_at:new Date().toISOString()};
    const result=id?await state.client.from('listing_fu_schedules').update(row).eq('id',id).eq('listing_id',listingId):await state.client.from('listing_fu_schedules').insert(row);
    if(result.error)return toast(result.error.message);await syncListing(listingId);toast(id?'예정 FU를 수정했습니다.':'예정 FU를 등록했습니다.');await renderListingSchedules(listingId);crm3830RefreshListingScreen();
  };
  window.crm31018EditCustomerSchedule=async function(scheduleId,customerId){
    const {data,error}=await state.client.from('customer_fu_schedules').select('*').eq('id',scheduleId).eq('customer_id',customerId).single();if(error)return toast(error.message);
    q('#crm3888ScheduleId').value=data.id;q('#crm3888ScheduleDate').value=dateOnly(data.scheduled_at);q('#crm3888ScheduleReason').value=data.reason==='예정 FU'?'':(data.reason||'');q('.crm31018-save').textContent='수정 저장';q('#crm3888ScheduleReason').focus();
  };
  window.crm31018EditListingSchedule=async function(scheduleId,listingId){
    const {data,error}=await state.client.from('listing_fu_schedules').select('*').eq('id',scheduleId).eq('listing_id',listingId).single();if(error)return toast(error.message);
    q('#crm31016ListingScheduleId').value=data.id;q('#crm3830ScheduleDate').value=dateOnly(data.scheduled_at);q('#crm31016ListingScheduleReason').value=data.reason==='예정 FU'?'':(data.reason||'');q('.crm31018-save').textContent='수정 저장';q('#crm31016ListingScheduleReason').focus();
  };
  window.crm31018ResetCustomerSchedule=function(){q('#crm3888ScheduleId').value='';q('#crm3888ScheduleDate').value='';q('#crm3888ScheduleReason').value='';q('.crm31018-save').textContent='예정 FU 저장';};
  window.crm31018ResetListingSchedule=function(){q('#crm31016ListingScheduleId').value='';q('#crm3830ScheduleDate').value='';q('#crm31016ListingScheduleReason').value='';q('.crm31018-save').textContent='예정 FU 저장';};
  console.info('CRM v3.10.18 예정 FU UI 강제 통일 적용');
})();

/* ===== CRM v3.10.19 매물 FU 기록 통합 + 리스트 히스토리 버튼 제거 ===== */
(function(){
  const q=(s,r=document)=>r.querySelector(s);
  const qa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const sameId=(a,b)=>String(a||'')===String(b||'');
  ...
  function listingById(id){return (state.listings||[]).find(x=>sameId(x.id,id));}

  async function renderListingFuRecords(listingId){
    const panel=q('.crm361-fu-panel[data-panel="record"]');
    if(!panel)return;
    let host=q('#crm31019ListingFuHistory',panel);
    if(!host){
      host=document.createElement('div');
      host.id='crm31019ListingFuHistory';
      host.className='crm31019-listing-fu-history';
      panel.appendChild(host);
    }
    host.innerHTML='<div class="crm3888-empty">FU 기록을 불러오는 중입니다.</div>';
    const {data,error}=await state.client.from('interaction_history')
      .select('*, writer:profiles!interaction_history_created_by_fkey(full_name)')
      .eq('listing_id',listingId)
      .order('follow_up_date',{ascending:false})
      .order('created_at',{ascending:false});
    if(error){host.innerHTML=`<div class="crm3888-empty">${escapeHtml(error.message)}</div>`;return;}
    const listing=listingById(listingId);
    const canDelete=isAdmin()||sameId(listing?.owner_id,state.profile?.id);
    host.innerHTML=(data||[]).length?`<div class="crm3888-record-list">${data.map(h=>`<article class="crm3888-record-item crm31019-listing-record"><div class="crm3888-item-head"><div><strong>${escapeHtml(h.contact_method||'FU')}</strong> · ${fmtDate(h.follow_up_date)}</div><div class="crm31019-record-actions"><span class="muted">${escapeHtml(h.writer?.full_name||'')}</span>${canDelete?`<button type="button" class="danger crm31019-delete" onclick="crm31019DeleteListingFu('${h.id}','${listingId}')">삭제</button>`:''}</div></div><div>${escapeHtml(h.content||'').replace(/\n/g,'<br>')}</div></article>`).join('')}</div>`:'<div class="crm3888-empty">아직 FU 기록이 없습니다.</div>';
  }

  async function crm31019DeleteListingFu(historyId,listingId){
    const listing=listingById(listingId);
    if(!listing)return toast('매물 정보를 찾지 못했습니다.');
    if(!(isAdmin()||sameId(listing.owner_id,state.profile?.id)))return toast('해당 매물 담당 중개사와 관리자만 FU 기록을 삭제할 수 있습니다.');
    if(!confirm('이 FU 기록을 삭제할까요?'))return;
    const {error}=await state.client.from('interaction_history').delete().eq('id',historyId).eq('listing_id',listingId);
    if(error)return toast(`FU 삭제 실패: ${error.message}`);
    toast('FU 기록을 삭제했습니다.');
    await renderListingFuRecords(listingId);
  }

  const baseListingFu=window.crm361OpenListingFu||globalThis.crm361OpenListingFu;
  if(typeof baseListingFu==='function'&&!baseListingFu.__crm31019){
    const wrapped=async function(id){
      const result=await baseListingFu.apply(this,arguments);
      await renderListingFuRecords(id);
      return result;
    };
    wrapped.__crm31019=true;
    window.crm361OpenListingFu=wrapped;
    try{crm361OpenListingFu=wrapped}catch(_){ }
  }

  function removeListingHistoryButtons(root=document){
    qa('button',root).forEach(btn=>{
      const txt=(btn.textContent||'').trim();
      const code=btn.getAttribute('onclick')||'';
      if(txt==='히스토리'&&/openHistoryModal\(\s*['"]listing['"]/.test(code))btn.remove();
    });
  }

  const baseRenderListingTable=window.renderListingTable||globalThis.renderListingTable;
  if(typeof baseRenderListingTable==='function'&&!baseRenderListingTable.__crm31019){
    const wrapped=function(){
      const result=baseRenderListingTable.apply(this,arguments);
      const target=arguments[1];
      removeListingHistoryButtons(target?document.getElementById(target):document);
      return result;
    };
    wrapped.__crm31019=true;
    window.renderListingTable=wrapped;
    try{renderListingTable=wrapped}catch(_){ }
  }

  removeListingHistoryButtons();
  Object.assign(window,{crm31019DeleteListingFu,crm31019RenderListingFuRecords:renderListingFuRecords});
  console.info('CRM v3.10.19 매물 FU 기록 통합 및 히스토리 버튼 제거 적용');
})();

/* ===== CRM v3.10.20 매물 관리 버튼 2줄 배치 ===== */
(function(){
  function arrangeListingActions(root=document){
    root.querySelectorAll('.row-actions').forEach(actions=>{
      const buttons=[...actions.querySelectorAll(':scope > button, :scope > .crm31020-action-row > button')];
      const byText=text=>buttons.find(btn=>(btn.textContent||'').trim()===text);
      const fu=byText('FU');
      const progress=byText('진행상황');
      const copy=byText('매물복사');
      const edit=byText('수정');
      const del=byText('삭제');
      if(!fu||!progress||!copy||!edit||!del)return;

      const known=new Set([fu,progress,copy,edit,del]);
      const extra=buttons.filter(btn=>!known.has(btn));
      actions.querySelectorAll(':scope > .crm31020-action-row').forEach(row=>row.remove());

      const row1=document.createElement('div');
      row1.className='crm31020-action-row crm31020-action-row-primary';
      [fu,progress,copy].forEach(btn=>row1.appendChild(btn));

      const row2=document.createElement('div');
      row2.className='crm31020-action-row crm31020-action-row-secondary';
      [edit,del,...extra].forEach(btn=>row2.appendChild(btn));

      actions.replaceChildren(row1,row2);
      actions.classList.add('crm31020-listing-actions');
    });
  }

  const baseRender=window.renderListingTable||globalThis.renderListingTable;
  if(typeof baseRender==='function'&&!baseRender.__crm31020){
    const wrapped=function(){
      const result=baseRender.apply(this,arguments);
      const target=arguments[1];
      arrangeListingActions(target?document.getElementById(target):document);
      return result;
    };
    wrapped.__crm31020=true;
    window.renderListingTable=wrapped;
    try{renderListingTable=wrapped}catch(_){ }
  }

  arrangeListingActions();
  window.crm31020ArrangeListingActions=arrangeListingActions;
  console.info('CRM v3.10.20 매물 관리 버튼 배치 적용 완료');
})();

/* ===== CRM v3.10.22 고객 목록 입주희망일 표시 확정 ===== */
(function(){
  function moveInText(customer){
    const period=String(customer?.desired_move_in_period||'').trim();
    if(period)return period;
    const date=String(customer?.desired_move_in_date||'').trim();
    return date?fmtDate(date):'-';
  }
  ...
  function customerIdFromRow(row){
    const btn=row.querySelector("button[onclick*='openCustomerModal']");
    const code=btn?.getAttribute('onclick')||'';
    return code.match(/openCustomerModal\(['\"]([^'\"]+)['\"]\)/)?.[1]||'';
  }

  function patchCustomerMoveInColumn(){
    const table=document.querySelector('#customerTable table');
    if(!table)return;

    // 이전 버전에서 잘못 삽입된 열이 있다면 한 번 정리한다.
    const headers=[...table.querySelectorAll('thead th')];
    const existingIndexes=[];
    headers.forEach((th,i)=>{
      if(th.dataset.crm31022MoveIn==='1'||th.classList.contains('crm3894-move-in-col')||th.textContent.trim()==='입주희망일')existingIndexes.push(i);
    });
    if(existingIndexes.length>1){
      existingIndexes.slice(1).reverse().forEach(i=>{
        table.querySelectorAll('tr').forEach(tr=>tr.children[i]?.remove());
      });
    }

    let currentHeaders=[...table.querySelectorAll('thead th')];
    let moveIndex=currentHeaders.findIndex(th=>th.dataset.crm31022MoveIn==='1'||th.classList.contains('crm3894-move-in-col')||th.textContent.trim()==='입주희망일');
    const featureIndex=currentHeaders.findIndex(th=>th.textContent.trim()==='희망특징');
    if(featureIndex<0)return;

    if(moveIndex<0){
      const th=document.createElement('th');
      th.textContent='입주희망일';
      th.className='crm3894-move-in-col crm31022-move-in-col';
      th.dataset.crm31022MoveIn='1';
      currentHeaders[featureIndex].before(th);
      moveIndex=featureIndex;
    }else{
      const th=currentHeaders[moveIndex];
      th.textContent='입주희망일';
      th.dataset.crm31022MoveIn='1';
      th.classList.add('crm3894-move-in-col','crm31022-move-in-col');
      // 항상 희망특징 바로 왼쪽에 배치
      currentHeaders=[...table.querySelectorAll('thead th')];
      const newFeatureIndex=currentHeaders.findIndex(x=>x.textContent.trim()==='희망특징');
      const newMoveIndex=currentHeaders.indexOf(th);
      if(newFeatureIndex>=0&&newMoveIndex!==newFeatureIndex-1)currentHeaders[newFeatureIndex].before(th);
    }
    ...
    rows.forEach(row=>{
      ...
      let cell=row.querySelector('td[data-crm31022-move-in="1"]');
      if(!cell){
        cell=document.createElement('td');
        cell.dataset.crm31022MoveIn='1';
        cell.className='crm3894-move-in-col crm31022-move-in-col';
        const rowHeaders=[...table.querySelectorAll('thead th')];
        const featurePos=rowHeaders.findIndex(th=>th.textContent.trim()==='희망특징');
        if(featurePos>=0&&row.children[featurePos])row.children[featurePos].before(cell);
        else row.appendChild(cell);
      }
      const text=moveInText(customer);
      cell.innerHTML=`<span class="crm3894-move-in-text" title="${escapeHtml(text)}">${escapeHtml(text)}</span>`;
    });
  }

  // 렌더링 완료 후 1회만 보정한다.
  const baseRender=window.renderCustomers||globalThis.renderCustomers;
  if(typeof baseRender==='function'&&!baseRender.__crm31022){
    const wrapped=async function(){
      const result=await baseRender.apply(this,arguments);
      requestAnimationFrame(patchCustomerMoveInColumn);
      return result;
    };
    wrapped.__crm31022=true;
    window.renderCustomers=wrapped;
    try{renderCustomers=wrapped}catch(_){ }
  }

  const baseFilter=window.filterCustomers||globalThis.filterCustomers;
  if(typeof baseFilter==='function'&&!baseFilter.__crm31022){
    const wrapped=function(){
  ...
  const observer=new MutationObserver(mutations=>{
    if(mutations.some(m=>[...m.addedNodes].some(n=>n.nodeType===1&&(n.matches?.('#customerTable, #customerTable *')||n.querySelector?.('#customerTable'))))){
      requestAnimationFrame(patchCustomerMoveInColumn);
    }
  });
  observer.observe(document.body,{childList:true,subtree:true});
  setTimeout(patchCustomerMoveInColumn,100);
  window.crm31022PatchCustomerMoveInColumn=patchCustomerMoveInColumn;
  console.info('CRM v3.10.22 고객 목록 입주희망일 표시 확정 적용');
})();

/* ===== CRM v3.10.23 최신 등록순 · 전 열 정렬 · 고객 등록일 · 최초 FU 자동기입 ===== */
(function(){
  ...
  const cleanHeader=s=>String(s||'').replace(/[↕↑↓]/g,'').trim();
  const dateValue=v=>{
    const s=String(v||'').trim();
    if(!s||s==='-')return NaN;
    const normalized=s.replace(/\./g,'-').replace(/\s+/g,' ').replace(/-(\s|$)/g,'$1');
    const m=normalized.match(/(20\d{2})[-\s](\d{1,2})[-\s](\d{1,2})(?:\s+(\d{1,2}):(\d{2}))?/);
    if(m)return new Date(+m[1],+m[2]-1,+m[3],+(m[4]||0),+(m[5]||0)).getTime();
    const t=Date.parse(s);return Number.isFinite(t)?t:NaN;
  };
  const numberValue=v=>{
    const s=String(v||'').replace(/,/g,'');
    const m=s.match(/-?\d+(?:\.\d+)?/);return m?Number(m[0]):NaN;
  };
  function compareVisible(a,b){
    const av=(a||'').trim(), bv=(b||'').trim();
    const ad=dateValue(av),bd=dateValue(bv);
    if(Number.isFinite(ad)&&Number.isFinite(bd))return ad-bd;
    const an=numberValue(av),bn=numberValue(bv);
    if(Number.isFinite(an)&&Number.isFinite(bn)&&(/\d/.test(av)&&/\d/.test(bv)))return an-bn;
    return av.localeCompare(bv,'ko',{numeric:true,sensitivity:'base'});
  }
  function entityIdFromRow(row,type){
    const code=[...row.querySelectorAll('[onclick]')].map(el=>el.getAttribute('onclick')||'').join(' ');
    const patterns=type==='customer'
      ?[/openCustomerModal\(['"]([^'"]+)['"]\)/,/openFollowUpModal\(['"]customer['"],['"]([^'"]+)['"]\)/,/openCustomerFuHub\(['"]([^'"]+)['"]/]
      :[/openListingModal\(['"]([^'"]+)['"]\)/,/openListingPhotos\(['"]([^'"]+)['"]\)/,/openFollowUpModal\(['"]listing['"],['"]([^'"]+)['"]\)/,/crm361OpenListingFu\(['"]([^'"]+)['"]\)/];
    for(const re of patterns){const m=code.match(re);if(m)return m[1];}
    return '';
  }
  function tableType(table){
    if(table.closest('#customerTable'))return 'customer';
    if(table.classList.contains('customer-table'))return 'customer';
    if(table.classList.contains('listing-table')||table.closest('#myListingTable,#networkTable,#adminListingTable'))return 'listing';
    return '';
  }
  function buildGroups(table,type){
    const rows=[...table.tBodies[0]?.rows||[]];
    if(type!=='listing')return rows.map((r,i)=>({rows:[r],main:r,original:i,id:entityIdFromRow(r,type)}));
    const groups=[];let pending=[];
    rows.forEach(r=>{
      const isAddress=r.cells.length===1||[...r.cells].some(c=>Number(c.colSpan)>1);
      if(isAddress){
        if(pending.length)groups.push({rows:pending,main:pending[pending.length-1],original:groups.length,id:entityIdFromRow(pending[pending.length-1],type)});
        pending=[r];
      }else{
        pending.push(r);
        groups.push({rows:pending,main:r,original:groups.length,id:entityIdFromRow(r,type)});
        pending=[];
      }
    });
    if(pending.length)groups.push({rows:pending,main:pending[pending.length-1],original:groups.length,id:entityIdFromRow(pending[pending.length-1],type)});
  ...
  function createdAtFor(group,type){
    const list=type==='customer'?(state.customers||[]):(state.listings||[]);
    const row=list.find(x=>same(x.id,group.id));
    return row?.created_at?new Date(row.created_at).getTime():0;
  }
  function updateSequence(groups,type){
    groups.forEach((g,i)=>{
      const row=g.main;
      const headers=[...row.closest('table').querySelectorAll('thead th')].map(th=>cleanHeader(th.textContent));
      const idx=headers.indexOf('순번');
      if(idx>=0&&row.cells[idx])row.cells[idx].textContent=String(i+1);
    });
  }
  function applyGroups(table,groups,type){
    const frag=document.createDocumentFragment();
    groups.forEach(g=>g.rows.forEach(r=>frag.appendChild(r)));
    table.tBodies[0].appendChild(frag);updateSequence(groups,type);
  }
  function ensureNewestFirst(table,type){
    if(type!=='listing')return;
    const groups=buildGroups(table,type);
    groups.sort((a,b)=>createdAtFor(b,type)-createdAtFor(a,type));
    groups.forEach((g,i)=>g.original=i);
    applyGroups(table,groups,type);
    tableStates.set(table,{key:'__newest__',dir:'desc',groups:[...groups]});
  }
  function installSort(table,type){
    const headers=[...table.querySelectorAll('thead th')];
    if(!headers.length||!table.tBodies.length)return;
    headers.forEach((th,index)=>{
      const label=cleanHeader(th.textContent);
      if(!label||label==='관리'||label==='선택')return;
      th.classList.add('crm31023-sortable');
      th.dataset.crm31023Label=label;
      if(!th.querySelector('.crm31023-sort-icon'))th.insertAdjacentHTML('beforeend','<span class="crm31023-sort-icon">↕</span>');
      if(th.dataset.crm31023Bound==='1')return;
      th.dataset.crm31023Bound='1';
      th.addEventListener('click',()=>{
        const stateNow=tableStates.get(table)||{key:'',dir:'',groups:buildGroups(table,type)};
        let dir='asc';
        if(stateNow.key===label&&stateNow.dir==='asc')dir='desc';
        else if(stateNow.key===label&&stateNow.dir==='desc')dir='original';
        let groups=buildGroups(table,type);
        groups.forEach((g,i)=>{if(g.original==null)g.original=i;});
        if(dir==='original')groups.sort((a,b)=>{
          if(type==='listing')return createdAtFor(b,type)-createdAtFor(a,type);
          return a.original-b.original;
        });
        else groups.sort((a,b)=>{
          const av=a.main.cells[index]?.innerText||'';
          const bv=b.main.cells[index]?.innerText||'';
          const c=compareVisible(av,bv);return dir==='asc'?c:-c;
        });
        applyGroups(table,groups,type);
        headers.forEach(h=>{const icon=h.querySelector('.crm31023-sort-icon');if(icon)icon.textContent='↕';h.classList.remove('crm31023-sort-active');});
        const icon=th.querySelector('.crm31023-sort-icon');if(icon)icon.textContent=dir==='asc'?'↑':dir==='desc'?'↓':'↕';
        if(dir!=='original')th.classList.add('crm31023-sort-active');
        tableStates.set(table,{key:label,dir,groups});
      });
    });
  }
  function addCustomerRegisteredAt(table){
    if(tableType(table)!=='customer')return;
    [...table.tBodies[0]?.rows||[]].forEach(row=>{
      const id=entityIdFromRow(row,'customer');if(!id)return;
      const c=(state.customers||[]).find(x=>same(x.id,id));if(!c?.created_at)return;
      const actions=row.querySelector('.row-actions');if(!actions)return;
      let stamp=actions.parentElement.querySelector('.crm31023-created-at');
      if(!stamp){stamp=document.createElement('div');stamp.className='crm31023-created-at';actions.before(stamp);}
      const d=new Date(c.created_at);
      stamp.textContent=`등록 ${String(d.getFullYear()).slice(2)}.${String(d.getMonth()+1).padStart(2,'0')}.${String(d.getDate()).padStart(2,'0')} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
    });
  }
  function processTables(){
    scheduled=false;
    document.querySelectorAll('table').forEach(table=>{
      const type=tableType(table);if(!type)return;
      if(table.dataset.crm31023Init!=='1'){
        table.dataset.crm31023Init='1';
        ensureNewestFirst(table,type);
      }
      installSort(table,type);
      addCustomerRegisteredAt(table);
    });
  }
  function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(processTables);}

  // 신규 고객·매물 등록 시 최종 FU를 오늘 날짜로 자동 입력한다.
  function installInsertPatch(){
    if(!state?.client||state.client.__crm31023InsertPatch)return false;
    const client=state.client;
    const originalFrom=client.from.bind(client);
    client.from=function(table){
      const builder=originalFrom(table);
      if((table==='customers'||table==='listings')&&builder&&typeof builder.insert==='function'&&!builder.__crm31023Insert){
        const originalInsert=builder.insert.bind(builder);
        builder.insert=function(values,options){
          const todayValue=new Date().toISOString().slice(0,10);
          const patch=row=>{
            if(!row||typeof row!=='object')return row;
            return row.last_follow_up_at?row:{...row,last_follow_up_at:todayValue};
          };
          return originalInsert(Array.isArray(values)?values.map(patch):patch(values),options);
        };
        builder.__crm31023Insert=true;
      }
      return builder;
    };
    client.__crm31023InsertPatch=true;return true;
  }
  if(!installInsertPatch()){
    const timer=setInterval(()=>{if(installInsertPatch())clearInterval(timer)},100);
    setTimeout(()=>clearInterval(timer),10000);
  }

  const observer=new MutationObserver(mutations=>{
    if(mutations.some(m=>[...m.addedNodes].some(n=>n.nodeType===1&&(n.matches?.('table,tbody,tr')||n.querySelector?.('table')))))schedule();
  });
  observer.observe(document.body,{childList:true,subtree:true});
  schedule();
...
(() => {
  ...
  function buildRecordFooter(id) {
    ...
    briefing.className = 'ghost crm-r318-fu-only crm-r324-briefing';
    briefing.innerHTML = '📋 브리핑시트';
    briefing.onclick = () => window.crmR324OpenBriefingSheet?.(id);
...
(() => {
  ...
  function brochureAddress(x){
    ...
    // 고객용 소개서에는 호수를 노출하지 않는다. 주소 원문에 이미 들어간 호수도 제거한다.
    const unit=String(x?.unit_no||'').trim();
    if(unit){
      s=s.replaceAll(`${unit}호`,' ').replaceAll(unit,' ').replace(/\s+/g,' ').trim();
    }
    s=s.replace(/\s+[0-9A-Za-z가-힣-]+호(?:\s.*)?$/,'').trim();
    // 동 정보는 유지/보완한다.
    if(x?.building_no){
      const b=String(x.building_no).trim();
      const label=/동$/.test(b)?b:`${b}동`;
      if(b&&!s.includes(label)) s=`${s} ${label}`.trim();
    }
    return s||'-';
  }
  function brochureFloor(x){
    const cur=x?.current_floor, total=x?.total_floors;
    if(cur!==null&&cur!==undefined&&cur!==''&&total!==null&&total!==undefined&&total!=='') return `${cur}층 / ${total}층`;
    if(cur!==null&&cur!==undefined&&cur!=='') return `${cur}층`;
    if(total!==null&&total!==undefined&&total!=='') return `총 ${total}층`;
    return '-';
  }
  function brochureBuiltYear(x){
    const y=String(x?.built_year||'').trim();
    return y ? `${y}년` : '-';
  }
  ...
  function card(x,urls,i){
    const facts=[['매물유형',x.property_type||'-'],['방 / 욕실',`${listingRoomText(x)||'-'}개 / ${x.bathroom_count??'-'}개`],['전용면적',brochureArea(x)],['입주가능일',brochureMoveIn(x)],['층수',brochureFloor(x)],['사용승인일',brochureBuiltYear(x)]];
  ...
  async function imageCanvas(x,urls){
    ...
    ctx.fillStyle='#64748b';ctx.font='500 17px sans-serif';ctx.fillText('매물 조건은 실시간으로 변경될 수 있으므로 계약·방문 전 재확인해 주세요.',90,1378);
    return c;
  }
...
/* =========================================================
   CRM v3.10.24R3.24 · 매물 브리핑시트
   ========================================================= */
(() => {
  const VERSION='3.10.24R3.24';
  const esc=v=>escapeHtml(String(v??''));
  const txt=v=>String(v??'').trim();
  const clean=v=>String(v??'').replace(/<br\s*\/?>/gi,' · ').replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim();
  const yn=v=>v===true?'가능':v===false?'불가':'미확인';
  function moveIn(x){
    const a=[];
    if(x?.move_in_immediate)a.push('즉시입주');
    else if(x?.move_in_period){const m=txt(x.move_in_period).match(/^(\d{4})-(\d{2})\s*(초순|중순|말)$/);a.push(m?`${m[1]}년 ${Number(m[2])}월 ${m[3]}`:txt(x.move_in_period));}
    else if(x?.move_in_date)a.push(fmtDate(x.move_in_date));
    if(x?.move_in_negotiable)a.push('협의 가능');
    return a.length?a.join(' · '):'-';
  }
  function address(x){
    const base=txt(x?.address),district=txt(x?.district);let s=base||district||'-';
    if(district&&base&&!base.includes(district))s=`${district} ${base}`;
    const b=txt(x?.building_no),u=txt(x?.unit_no);
    if(b){const bl=/동$/.test(b)?b:`${b}동`;if(!s.includes(bl))s+=` ${bl}`}
    if(u){const ul=/호$/.test(u)?u:`${u}호`;if(!s.includes(ul))s+=` ${ul}`}
    return s.replace(/\s+/g,' ').trim();
  }
  function floor(x){const c=x?.current_floor,t=x?.total_floors;if(c!==null&&c!==undefined&&c!==''&&t!==null&&t!==undefined&&t!=='')return`${c}층 / 총 ${t}층`;if(c!==null&&c!==undefined&&c!=='')return`${c}층`;if(t!==null&&t!==undefined&&t!=='')return`총 ${t}층`;return'-'}
  function area(x){const n=Number(x?.area_m2||0);return n?`${n.toLocaleString('ko-KR',{maximumFractionDigits:2})}㎡ · 약 ${(n/3.3058).toFixed(2)}평`:'-'}
  function contacts(x){const out=[];const seen=new Set();const extras=Array.isArray(x?.additional_contacts)?x.additional_contacts:[];if(x?.contact_phone){const phone=crm381FormatPhone(x.contact_phone);const matched=extras.find(c=>c.phone&&crm381FormatPhone(c.phone)===phone);out.push({role:'소유주',name:matched?.contact_name||x?.contact_name||'',phone});seen.add(phone)}extras.forEach(c=>{if(!c.phone)return;const phone=crm381FormatPhone(c.phone);if(seen.has(phone))return;out.push({role:c.contact_role||'기타',name:c.contact_name||'',phone});seen.add(phone)});return out}
  function rows(x){return[
    ['매물유형',x.property_type||'-'],['거래유형',clean(crm38DealTypeText(x)||x.transaction_type||'-')],['금액',clean(listingPriceText(x))||'-'],['전용면적',area(x)],['방 / 화장실',`${listingRoomText(x)||'-'}개 / ${x.bathroom_count??'-'}개`],['층수',floor(x)],['사용승인일',crmR344ApprovalDateText(x)],['관리비',x.management_fee?fmtMoney(x.management_fee):'-'],['입주가능일',moveIn(x)],['옵션',x.options||'-'],['대출 가능 여부',yn(x.loan_available)],['공시지가/기준시가',x.official_price?fmtMoney(x.official_price):'-'],['매물 특징',(crm36Array(x.feature_tags)||[]).join(', ')||'-'],['공개 여부',x.is_public===false?'비공개':'공개'],['최종 확인일',x.last_confirmed_at?fmtDate(x.last_confirmed_at):'-'],['다음 확인 예정일',x.next_confirm_at?fmtDate(x.next_confirm_at):'-'],['예정 FU',x.next_follow_up_at?fmtDate(x.next_follow_up_at):'-']
  ]}
  async function photoUrls(x,limit=6){try{const {data,error}=await state.client.from('listing_photos').select('*').eq('listing_id',x.id).order('sort_order').order('created_at');if(error)throw error;const arr=[...(data||[])];const ci=arr.findIndex(p=>p.id===x.cover_photo_id);if(ci>0)arr.unshift(arr.splice(ci,1)[0]);const urls=[];for(const p of arr.slice(0,limit)){const u=await signedPhotoUrl(p.storage_path);if(u)urls.push(u)}return urls}catch(e){console.warn('briefing photo load failed',e);return[]}}
  function textVersion(x){const l=[`[매물 브리핑시트]`,`매물명: ${x.title||'-'}`,`주소: ${address(x)}`,``];rows(x).forEach(([k,v])=>l.push(`• ${k}: ${v}`));const cs=contacts(x);if(cs.length){l.push('',`[연락처]`);cs.forEach(c=>l.push(`• ${c.role}${c.name?` (${c.name})`:''}: ${c.phone}`))}if(x.description)l.push('',`[상세설명 / 비밀메모]`,x.description);return l.join('\n')}
  const pcss=`*{box-sizing:border-box}body{margin:0;background:#eef2f6;color:#111827;font-family:Pretendard,'Noto Sans KR',Arial,sans-serif}.tb{position:sticky;top:0;z-index:10;display:flex;justify-content:flex-end;gap:8px;padding:12px 18px;background:#fff;border-bottom:1px solid #dbe2ea}.tb button{border:1px solid #d5dce5;background:#fff;border-radius:10px;padding:10px 15px;font-weight:800}.tb .pri{background:#2563eb;color:#fff;border-color:#2563eb}.wrap{width:min(1040px,calc(100% - 32px));margin:24px auto}.sheet{background:#fff;border-radius:22px;padding:32px;box-shadow:0 18px 55px rgba(15,23,42,.1)}.eyebrow{font-size:11px;font-weight:900;letter-spacing:.16em;color:#64748b;border-bottom:2px solid #111827;padding-bottom:10px}.hero{display:grid;grid-template-columns:1fr auto;gap:22px;align-items:end;padding:24px 0 20px}.hero h1{font-size:34px;margin:5px 0 8px}.hero p{margin:0;color:#64748b}.price{text-align:right}.price strong{display:block;font-size:23px}.photos{display:grid;grid-template-columns:2fr 1fr 1fr;grid-template-rows:205px 205px;gap:7px;border-radius:15px;overflow:hidden;background:#e5e7eb}.photos img{width:100%;height:100%;object-fit:cover}.photos img:first-child{grid-row:1/3}.none{height:230px;background:#f8fafc;border:1px dashed #cbd5e1;border-radius:14px;display:grid;place-items:center;color:#94a3b8;font-weight:800}.section{margin-top:22px}.section h2{font-size:16px;margin:0 0 10px}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.item{border:1px solid #e2e8f0;border-radius:12px;padding:12px;min-height:74px}.item span{font-size:11px;color:#64748b;font-weight:800;display:block;margin-bottom:5px}.item strong{font-size:14px;line-height:1.45}.contacts{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.contact{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:12px}.memo{white-space:pre-wrap;background:#fff8ed;border:1px solid #fed7aa;border-radius:12px;padding:14px;line-height:1.65}.foot{margin-top:22px;padding-top:12px;border-top:1px solid #e2e8f0;color:#94a3b8;font-size:11px}@page{size:A4;margin:8mm}@media print{body{background:#fff}.tb{display:none}.wrap{width:100%;margin:0}.sheet{box-shadow:none;border-radius:0;padding:7mm}.photos{grid-template-rows:54mm 54mm}*{-webkit-print-color-adjust:exact;print-color-adjust:exact}}`;
  async function printSheet(x){const w=window.open('','_blank');if(!w)return toast('팝업이 차단되었습니다.');w.document.write('<p style="font-family:sans-serif;padding:30px">브리핑시트를 준비하고 있습니다...</p>');const urls=await photoUrls(x,5),cs=contacts(x),rr=rows(x);const ph=urls.length?`<div class="photos">${urls.map(u=>`<img src="${u}">`).join('')}</div>`:`<div class="none">등록된 사진이 없습니다</div>`;const body=`<div class="sheet"><div class="eyebrow">INTERNAL PROPERTY BRIEFING SHEET</div><div class="hero"><div><h1>${esc(x.title||'매물 브리핑')}</h1><p>${esc(address(x))}</p></div><div class="price"><strong>${esc(clean(listingPriceText(x))||'-')}</strong></div></div>${ph}<section class="section"><h2>매물 상세 정보</h2><div class="grid">${rr.map(([k,v])=>`<div class="item"><span>${esc(k)}</span><strong>${esc(v)}</strong></div>`).join('')}</div></section>${cs.length?`<section class="section"><h2>연락처</h2><div class="contacts">${cs.map(c=>`<div class="contact"><b>${esc(c.role)}</b>${c.name?` · ${esc(c.name)}`:''}<div>${esc(c.phone)}</div></div>`).join('')}</div></section>`:''}${x.description?`<section class="section"><h2>상세설명 / 비밀메모</h2><div class="memo">${esc(x.description)}</div></section>`:''}<div class="foot">중개 실무용 브리핑 자료 · 등록된 매물 정보를 기준으로 자동 생성</div></div>`;w.document.open();w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(x.title||'매물')} 브리핑시트</title><style>${pcss}</style></head><body><div class="tb"><button onclick="window.close()">닫기</button><button class="pri" onclick="window.print()">인쇄 / PDF 저장</button></div><main class="wrap">${body}</main></body></html>`);w.document.close()}
  async function copyText(t){let ok=false;try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(t);ok=true}}catch(_){}if(!ok){const ta=document.createElement('textarea');ta.value=t;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();try{ok=document.execCommand('copy')}catch(_){}ta.remove()}toast(ok?'브리핑시트 텍스트를 복사했습니다.':'복사에 실패했습니다.')}
  async function imageSheet(x){const c=document.createElement('canvas');c.width=1200;c.height=1760;const ctx=c.getContext('2d');ctx.fillStyle='#f8fafc';ctx.fillRect(0,0,1200,1760);ctx.fillStyle='#fff';ctx.fillRect(50,50,1100,1660);ctx.fillStyle='#64748b';ctx.font='800 20px sans-serif';ctx.fillText('INTERNAL PROPERTY BRIEFING SHEET',85,105);ctx.fillStyle='#111827';ctx.fillRect(85,128,1030,3);ctx.font='900 48px sans-serif';ctx.fillText(String(x.title||'매물 브리핑').slice(0,28),85,205);ctx.fillStyle='#64748b';ctx.font='500 22px sans-serif';ctx.fillText(address(x).slice(0,54),85,248);ctx.fillStyle='#111827';ctx.font='900 30px sans-serif';ctx.fillText(clean(listingPriceText(x)).slice(0,30),85,300);let y=365;for(const [k,v] of rows(x).slice(0,12)){ctx.fillStyle='#64748b';ctx.font='800 17px sans-serif';ctx.fillText(k,85,y);ctx.fillStyle='#111827';ctx.font='800 19px sans-serif';ctx.fillText(String(v).slice(0,52),330,y);y+=58}const cs=contacts(x);if(cs.length){ctx.fillStyle='#111827';ctx.font='900 24px sans-serif';ctx.fillText('소유주 / 연락처',85,y+16);y+=54;cs.slice(0,4).forEach((c2)=>{ctx.fillStyle='#64748b';ctx.font='800 17px sans-serif';ctx.fillText(c2.role||'연락처',85,y);ctx.fillStyle='#111827';ctx.font='800 19px sans-serif';const contactText=`${c2.name?`${c2.name} · `:''}${c2.phone||'-'}`;ctx.fillText(String(contactText).slice(0,52),330,y);y+=46})}if(x.description){ctx.fillStyle='#111827';ctx.font='900 24px sans-serif';ctx.fillText('상세설명 / 비밀메모',85,y+36);ctx.fillStyle='#334155';ctx.font='500 18px sans-serif';const s=String(x.description);for(let i=0;i<Math.min(6,Math.ceil(s.length/52));i++)ctx.fillText(s.slice(i*52,(i+1)*52),85,y+76+i*28)}return c}
  async function crmR324OpenBriefingSheet(id){const x=(state.listings||[]).find(v=>String(v.id)===String(id));if(!x)return toast('매물 정보를 찾을 수 없습니다.');const d=document.createElement('dialog');d.className='crm-r324-brief-dialog';d.innerHTML=`<div class="crm-r324-brief-card"><div class="crm-r324-brief-head"><div><strong>브리핑시트</strong><span>${esc(x.title||'')}</span></div><button class="icon-btn" type="button" data-close>×</button></div><div class="crm-r324-brief-summary"><b>${esc(address(x))}</b><span>${esc(clean(listingPriceText(x))||'-')}</span></div><div class="crm-r324-brief-grid">${rows(x).slice(0,12).map(([k,v])=>`<div><span>${esc(k)}</span><strong>${esc(v)}</strong></div>`).join('')}</div><div class="crm-r324-brief-actions"><button class="ghost" data-copy>📋 텍스트 복사</button><button class="ghost" data-png>🖼 PNG 저장</button><button class="primary" data-pdf>인쇄 / PDF</button></div></div>`;document.body.appendChild(d);const close=()=>{d.close();setTimeout(()=>d.remove(),0)};d.querySelector('[data-close]').onclick=close;d.addEventListener('cancel',e=>{e.preventDefault();close()});d.querySelector('[data-copy]').onclick=()=>copyText(textVersion(x));d.querySelector('[data-pdf]').onclick=()=>printSheet(x);d.querySelector('[data-png]').onclick=async()=>{const c=await imageSheet(x);c.toBlob(blob=>{if(!blob)return toast('이미지 생성에 실패했습니다.');const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${String(x.title||'매물').replace(/[\\/:*?"<>|]/g,'_')}_브리핑시트.png`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1500)},'image/png',.95)};d.showModal()}
...
/* =========================================================
   CRM v3.10.24R3.44 · 소개자료 사용승인일 통일
   ========================================================= */
(() => {
  const VERSION='3.10.24R3.44';

  function approvalDateText(x){
    if(x?.usage_approval_date){
      const raw=String(x.usage_approval_date).slice(0,10);
      const [y,m,d]=raw.split('-');
      if(y&&m&&d)return `${y}.${m}.${d}`;
      return raw;
    }
    if(x?.built_year)return `${x.built_year}.01.01 (일자 확인 필요)`;
    return '-';
  }

  window.crmR344ApprovalDateText=approvalDateText;
  try{crmR344ApprovalDateText=approvalDateText}catch(_){}

  // 카카오톡 소개문구에 정확한 사용승인일 추가
  const oldKakaoText=window.crm384KakaoText||globalThis.crm384KakaoText;
  if(typeof oldKakaoText==='function'){
    const wrapped=function(customer,rows){
      let text=oldKakaoText.call(this,customer,rows);
      (rows||[]).forEach((x,i)=>{
        const title=`${i+1}. ${x.title}`;
        const approval=`• 사용승인일: ${approvalDateText(x)}`;
        if(text.includes(title) && !text.includes(approval)){
          const start=text.indexOf(title);
          const next=text.indexOf(`\n${i+2}. `,start+title.length);
          const end=next>=0?next:text.length;
          const block=text.slice(start,end);
          const insertAt=block.indexOf('\n• 특징:');
          const relative=insertAt>=0?insertAt:block.length;
          const updated=block.slice(0,relative)+`\n${approval}`+block.slice(relative);
          text=text.slice(0,start)+updated+text.slice(end);
        }
      });
      return text;
    };
    window.crm384KakaoText=wrapped;
    try{crm384KakaoText=wrapped}catch(_){}
  }

  // 추천 카카오톡 문구 함수가 직접 문장을 만드는 버전도 보정
  const oldKakao=window.crm36KakaoMessage||globalThis.crm36KakaoMessage;
  if(typeof oldKakao==='function'&&!oldKakao.__crmR344){
    const wrapped=async function(customerId,...args){
      const result=await oldKakao.call(this,customerId,...args);
      const ta=document.querySelector('#crm36KakaoText,#crm384KakaoText');
      if(ta){
        const rows=(state.listings||[]).filter(x=>state.matchSelection?.has(x.id));
        let text=ta.value;
        rows.forEach((x,i)=>{
          const title=`${i+1}. ${x.title}`;
          if(text.includes(title)&&!text.includes(`• 사용승인일: ${approvalDateText(x)}`)){
            text=text.replace(title,`${title}\n• 사용승인일: ${approvalDateText(x)}`);
          }
        });
        ta.value=text;
      }
    ...
    wrapped.__crmR344=true;
    window.crm36KakaoMessage=wrapped;
    try{crm36KakaoMessage=wrapped}catch(_){}
  }
...
(()=>{
  ...
  function installCustomer(id){
    ...
    monthly.insertAdjacentHTML('afterend',`<div class="crm-r346-adjust" id="crmR346CustomerAdjust"><label class="inline-check crm-r346-check"><input type="checkbox" id="crmR346CustomerCheck" name="deposit_adjustable" ${c.deposit_adjustable?'checked':''}> 보증금 조정 가능</label><div class="crm-r346-alt-fields" id="crmR346CustomerFields"><label>다른 보증금(만원)<input id="crmR346CustomerDeposit" name="alternate_deposit" type="number" min="0" value="${c.alternate_deposit??''}"></label><label>다른 월세(만원)<input id="crmR346CustomerRent" name="alternate_monthly_rent" type="number" min="0" value="${c.alternate_monthly_rent??''}"></label></div></div>`);
  ...
  function installListing(id){
    ...
    f.insertAdjacentHTML('beforeend',`<div class="crm-r346-adjust" id="crmR346ListingAdjust"><label class="inline-check crm-r346-check"><input type="checkbox" id="crmR346ListingCheck" name="deposit_adjustable" ${l.deposit_adjustable?'checked':''}> 보증금 조정 가능</label><div class="crm-r346-alt-fields" id="crmR346ListingFields"><label>다른 보증금(만원)<input id="crmR346ListingDeposit" name="alternate_deposit" type="number" min="0" value="${l.alternate_deposit??''}"></label><label>다른 월세(만원)<input id="crmR346ListingRent" name="alternate_monthly_rent" type="number" min="0" value="${l.alternate_monthly_rent??''}"></label></div></div>`);
...
/* =========================================================
   CRM v3.10.24R3.47 · 대안 월세 저장/목록 반영 확정
   ========================================================= */
(() => {
  const VERSION='3.10.24R3.47';
  ...
  const pair=(d,r)=>{
    const dep=val(d), rent=val(r);
    if(!dep&&!rent)return '';
    return `${dep?dep.toLocaleString('ko-KR'):'0'} / ${rent?rent.toLocaleString('ko-KR'):'0'}`;
  };

  const listingBase=window.listingPriceText||globalThis.listingPriceText;
  if(typeof listingBase==='function'){
    const wrapped=function(x){
      let base=String(listingBase.call(this,x))
        .replace(/<br><span class="crm-r345-adjust-list">.*?<\/span>/g,'')
        .replace(/<br><span class="crm-r346-alt-line">.*?<\/span>/g,'');
      const alt=x?.deposit_adjustable?pair(x.alternate_deposit,x.alternate_monthly_rent):'';
      return alt?`${base}<br><span class="crm-r347-alt-line">${escapeHtml(alt)}</span>`:base;
    };
    window.listingPriceText=wrapped;
    try{listingPriceText=wrapped}catch(_){}
  }

  const customerBase=window.customerBudgetText||globalThis.customerBudgetText;
  if(typeof customerBase==='function'){
    const wrapped=function(x){
      let base=String(customerBase.call(this,x))
        .replace(/<br><span class="crm-r345-adjust-list">.*?<\/span>/g,'')
        .replace(/<br><span class="crm-r346-alt-line">.*?<\/span>/g,'');
      const alt=x?.deposit_adjustable?pair(x.alternate_deposit,x.alternate_monthly_rent):'';
      return alt?`${base}<br><span class="crm-r347-alt-line">${escapeHtml(alt)}</span>`:base;
...
/* =========================================================
   CRM v3.10.24R3.51 · 반지하 호수 영문 + 공용면적
   ========================================================= */
(() => {
  const VERSION='3.10.24R3.51';
  const q=(s,r=document)=>r.querySelector(s);

  function installCommonArea(id){
    const body=q('#modalBody');
    if(!body||!q('[name="title"]',body)||q('[name="common_area_m2"]',body))return;

    const listing=(state.listings||[]).find(x=>String(x.id)===String(id))||{};
    const areaInput=q('[name="area_m2"]',body);
    const areaLabel=areaInput?.closest('label');
    if(!areaInput||!areaLabel)return;

    const label=document.createElement('label');
    label.className=areaLabel.className||'';
    label.innerHTML=`공용면적(㎡)
      <input name="common_area_m2" type="number" min="0" step="0.01"
        value="${listing.common_area_m2??''}" placeholder="예: 12.5">
      <small class="crm-r351-pyeong">-</small>`;

    areaLabel.insertAdjacentElement('afterend',label);

    const input=q('[name="common_area_m2"]',label);
    const helper=q('.crm-r351-pyeong',label);
    const sync=()=>{
      const v=Number(input.value);
      helper.textContent=Number.isFinite(v)&&v>0?`약 ${(v/3.3058).toFixed(2)}평`:'';
    };
  ...
  const base=window.openListingModal||globalThis.openListingModal;
  if(typeof base==='function'&&!base.__crmR351){
    const wrapped=function(id,...args){
      const result=base.call(this,id,...args);
      [50,140,320].forEach(ms=>setTimeout(()=>installCommonArea(id),ms));
      Promise.resolve(result).finally(()=>setTimeout(()=>installCommonArea(id),80));
      return result;
    };
    wrapped.__crmR351=true;
    window.openListingModal=wrapped;
    try{openListingModal=wrapped}catch(_){}
  }

  console.info(`CRM v${VERSION} 반지하 호수·공용면적 적용`);
})();
