...
(()=>{
  ...
  // 저장 버튼을 직접 감싸 변경요약 후 기존 저장 함수를 정확히 한 번 실행한다.
  async function bindChangeSummary393(ctx){
    const {type,id,form,btn}=ctx;
    if(!id||!btn||btn.__crm395SummaryBound)return;
    btn.__crm395SummaryBound=true;
    ...
    btn.onclick=async function(e){
      ...
      e?.preventDefault?.();
      e?.stopPropagation?.();
      showChangeSummary(changes,async()=>{
        await saveSnapshot393(type,id);
        ctx.bypass=true;
        localStorage.removeItem(draftKey(type,id));
        localStorage.removeItem(oldDraftKey(type,id));
        try{
          if(typeof originalHandler==='function') await originalHandler.call(btn,e);
        }finally{
          ctx.bypass=false;
        }
      });
