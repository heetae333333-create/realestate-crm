...
(()=>{
  ...
  async function installFormSafety(type,id){
    ...
    modal.addEventListener('close',()=>{clearTimeout(timer);modalContext=null;localStorage.removeItem(oldDraftKey(type,id));},{once:true});
  }

  // 저장 버튼을 캡처 단계에서 가로채므로 뒤에서 onclick이 다시 지정돼도 변경요약이 작동한다.
  document.addEventListener('click',e=>{
    const btn=e.target.closest?.('#modalSubmit');if(!btn||!modalContext||modalContext.btn!==btn||modalContext.bypass)return;
    const {type,id,form,initial}=modalContext;
    if(!id)return; // 신규등록은 변경요약 제외
    const changes=diff(initial||{},serialize(form),form);if(!changes.length)return;
    e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
    showChangeSummary(changes,async()=>{
      await saveSnapshot393(type,id);
      modalContext.bypass=true;
      localStorage.removeItem(draftKey(type,id));localStorage.removeItem(oldDraftKey(type,id));
      btn.click();
      setTimeout(()=>{if(modalContext)modalContext.bypass=false},500);
    });
  },true);
