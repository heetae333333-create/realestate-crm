...
(()=>{
  ...
  function addTopScrollbar(wrapper,table){
    if(wrapper.dataset.crm399Scroll==='1')return;
    wrapper.dataset.crm399Scroll='1';wrapper.classList.add('crm396-unified-scroll');
    let top=wrapper.previousElementSibling;
    if(!top?.classList.contains('crm396-top-scroll')){top=document.createElement('div');top.className='crm396-top-scroll';top.innerHTML='<div></div>';wrapper.parentNode.insertBefore(top,wrapper);}
    const inner=top.firstElementChild;
    const pair={top,inner,wrapper,table};scrollPairs.add(pair);
    let syncing=false;
    top.addEventListener('scroll',()=>{if(syncing)return;syncing=true;wrapper.scrollLeft=top.scrollLeft;requestAnimationFrame(()=>syncing=false)},{passive:true});
    wrapper.addEventListener('scroll',()=>{if(syncing)return;syncing=true;top.scrollLeft=wrapper.scrollLeft;requestAnimationFrame(()=>syncing=false)},{passive:true});
    wrapper.addEventListener('wheel',e=>{
      if(!e.shiftKey)return;const amount=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;if(!amount)return;
      e.preventDefault();wrapper.scrollLeft+=amount;top.scrollLeft=wrapper.scrollLeft;
    },{passive:false});
    requestAnimationFrame(()=>syncScrollWidth(pair));setTimeout(()=>syncScrollWidth(pair),250);
  }
...
(function(){
  ...
  function ensureNetworkScrollbar(){
    ...
    const table=host?.querySelector('table.listing-table');if(!host||!table)return;
    const wrap=table.closest('.table-wrap,.crm3890-listing-scroll')||table.parentElement;
    if(!wrap)return;
    wrap.classList.add('crm3106-network-scroll');
    let top=wrap.previousElementSibling;
    if(!top?.classList.contains('crm3106-network-top-scroll')){
      top=document.createElement('div');top.className='crm3106-network-top-scroll';top.innerHTML='<div></div>';
      wrap.parentNode.insertBefore(top,wrap);
      let lock=false;
      top.addEventListener('scroll',()=>{if(lock)return;lock=true;wrap.scrollLeft=top.scrollLeft;requestAnimationFrame(()=>lock=false)},{passive:true});
      wrap.addEventListener('scroll',()=>{if(lock)return;lock=true;top.scrollLeft=wrap.scrollLeft;requestAnimationFrame(()=>lock=false)},{passive:true});
      wrap.addEventListener('wheel',e=>{if(!e.shiftKey)return;const d=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;if(!d)return;e.preventDefault();wrap.scrollLeft+=d;top.scrollLeft=wrap.scrollLeft},{passive:false});
    }
    const inner=top.firstElementChild;
    const sync=()=>{const width=Math.max(table.scrollWidth,wrap.clientWidth+1);inner.style.width=`${width}px`;top.style.display='block';top.scrollLeft=wrap.scrollLeft};
    requestAnimationFrame(sync);setTimeout(sync,120);setTimeout(sync,400);
  }
